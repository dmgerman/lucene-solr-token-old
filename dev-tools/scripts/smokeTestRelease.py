begin_unit
comment|'# Licensed to the Apache Software Foundation (ASF) under one or more'
nl|'\n'
comment|'# contributor license agreements.  See the NOTICE file distributed with'
nl|'\n'
comment|'# this work for additional information regarding copyright ownership.'
nl|'\n'
comment|'# The ASF licenses this file to You under the Apache License, Version 2.0'
nl|'\n'
comment|'# (the "License"); you may not use this file except in compliance with'
nl|'\n'
comment|'# the License.  You may obtain a copy of the License at'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#     http://www.apache.org/licenses/LICENSE-2.0'
nl|'\n'
comment|'#'
nl|'\n'
comment|'# Unless required by applicable law or agreed to in writing, software'
nl|'\n'
comment|'# distributed under the License is distributed on an "AS IS" BASIS,'
nl|'\n'
comment|'# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.'
nl|'\n'
comment|'# See the License for the specific language governing permissions and'
nl|'\n'
comment|'# limitations under the License.'
nl|'\n'
nl|'\n'
name|'import'
name|'argparse'
newline|'\n'
name|'import'
name|'os'
newline|'\n'
name|'import'
name|'zipfile'
newline|'\n'
name|'import'
name|'codecs'
newline|'\n'
name|'import'
name|'tarfile'
newline|'\n'
name|'import'
name|'zipfile'
newline|'\n'
name|'import'
name|'threading'
newline|'\n'
name|'import'
name|'traceback'
newline|'\n'
name|'import'
name|'datetime'
newline|'\n'
name|'import'
name|'time'
newline|'\n'
name|'import'
name|'subprocess'
newline|'\n'
name|'import'
name|'signal'
newline|'\n'
name|'import'
name|'shutil'
newline|'\n'
name|'import'
name|'hashlib'
newline|'\n'
name|'import'
name|'http'
op|'.'
name|'client'
newline|'\n'
name|'import'
name|'re'
newline|'\n'
name|'import'
name|'urllib'
op|'.'
name|'request'
op|','
name|'urllib'
op|'.'
name|'error'
op|','
name|'urllib'
op|'.'
name|'parse'
newline|'\n'
name|'import'
name|'urllib'
op|'.'
name|'parse'
newline|'\n'
name|'import'
name|'sys'
newline|'\n'
name|'import'
name|'html'
op|'.'
name|'parser'
newline|'\n'
name|'from'
name|'collections'
name|'import'
name|'defaultdict'
newline|'\n'
name|'import'
name|'xml'
op|'.'
name|'etree'
op|'.'
name|'ElementTree'
name|'as'
name|'ET'
newline|'\n'
name|'import'
name|'filecmp'
newline|'\n'
name|'import'
name|'platform'
newline|'\n'
name|'import'
name|'checkJavaDocs'
newline|'\n'
name|'import'
name|'checkJavadocLinks'
newline|'\n'
name|'import'
name|'io'
newline|'\n'
name|'import'
name|'codecs'
newline|'\n'
name|'import'
name|'textwrap'
newline|'\n'
name|'from'
name|'collections'
name|'import'
name|'namedtuple'
newline|'\n'
nl|'\n'
comment|'# This tool expects to find /lucene and /solr off the base URL.  You'
nl|'\n'
comment|'# must have a working gpg, tar, unzip in your path.  This has been'
nl|'\n'
comment|'# tested on Linux and on Cygwin under Windows 7.'
nl|'\n'
nl|'\n'
DECL|variable|cygwin
name|'cygwin'
op|'='
name|'platform'
op|'.'
name|'system'
op|'('
op|')'
op|'.'
name|'lower'
op|'('
op|')'
op|'.'
name|'startswith'
op|'('
string|"'cygwin'"
op|')'
newline|'\n'
DECL|variable|cygwinWindowsRoot
name|'cygwinWindowsRoot'
op|'='
name|'os'
op|'.'
name|'popen'
op|'('
string|"'cygpath -w /'"
op|')'
op|'.'
name|'read'
op|'('
op|')'
op|'.'
name|'strip'
op|'('
op|')'
op|'.'
name|'replace'
op|'('
string|"'\\\\'"
op|','
string|"'/'"
op|')'
name|'if'
name|'cygwin'
name|'else'
string|"''"
newline|'\n'
nl|'\n'
DECL|function|unshortenURL
name|'def'
name|'unshortenURL'
op|'('
name|'url'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'parsed'
op|'='
name|'urllib'
op|'.'
name|'parse'
op|'.'
name|'urlparse'
op|'('
name|'url'
op|')'
newline|'\n'
name|'if'
name|'parsed'
op|'['
number|'0'
op|']'
name|'in'
op|'('
string|"'http'"
op|','
string|"'https'"
op|')'
op|':'
newline|'\n'
indent|'    '
name|'h'
op|'='
name|'http'
op|'.'
name|'client'
op|'.'
name|'HTTPConnection'
op|'('
name|'parsed'
op|'.'
name|'netloc'
op|')'
newline|'\n'
name|'h'
op|'.'
name|'request'
op|'('
string|"'HEAD'"
op|','
name|'parsed'
op|'.'
name|'path'
op|')'
newline|'\n'
name|'response'
op|'='
name|'h'
op|'.'
name|'getresponse'
op|'('
op|')'
newline|'\n'
name|'if'
name|'int'
op|'('
name|'response'
op|'.'
name|'status'
op|'/'
number|'100'
op|')'
op|'=='
number|'3'
name|'and'
name|'response'
op|'.'
name|'getheader'
op|'('
string|"'Location'"
op|')'
op|':'
newline|'\n'
indent|'      '
name|'return'
name|'response'
op|'.'
name|'getheader'
op|'('
string|"'Location'"
op|')'
newline|'\n'
dedent|''
dedent|''
name|'return'
name|'url'
newline|'\n'
nl|'\n'
comment|'# TODO'
nl|'\n'
comment|'#   + verify KEYS contains key that signed the release'
nl|'\n'
comment|'#   + make sure changes HTML looks ok'
nl|'\n'
comment|'#   - verify license/notice of all dep jars'
nl|'\n'
comment|'#   - check maven'
nl|'\n'
comment|'#   - check JAR manifest version'
nl|'\n'
comment|'#   - check license/notice exist'
nl|'\n'
comment|'#   - check no "extra" files'
nl|'\n'
comment|'#   - make sure jars exist inside bin release'
nl|'\n'
comment|'#   - run "ant test"'
nl|'\n'
comment|'#   - make sure docs exist'
nl|'\n'
comment|'#   - use java5 for lucene/modules'
nl|'\n'
nl|'\n'
DECL|variable|reHREF
dedent|''
name|'reHREF'
op|'='
name|'re'
op|'.'
name|'compile'
op|'('
string|'\'<a href="(.*?)">(.*?)</a>\''
op|')'
newline|'\n'
nl|'\n'
comment|'# Set to False to avoid re-downloading the packages...'
nl|'\n'
DECL|variable|FORCE_CLEAN
name|'FORCE_CLEAN'
op|'='
name|'True'
newline|'\n'
nl|'\n'
DECL|function|getHREFs
name|'def'
name|'getHREFs'
op|'('
name|'urlString'
op|')'
op|':'
newline|'\n'
nl|'\n'
comment|'# Deref any redirects'
nl|'\n'
indent|'  '
name|'while'
name|'True'
op|':'
newline|'\n'
indent|'    '
name|'url'
op|'='
name|'urllib'
op|'.'
name|'parse'
op|'.'
name|'urlparse'
op|'('
name|'urlString'
op|')'
newline|'\n'
name|'if'
name|'url'
op|'.'
name|'scheme'
op|'=='
string|'"http"'
op|':'
newline|'\n'
indent|'      '
name|'h'
op|'='
name|'http'
op|'.'
name|'client'
op|'.'
name|'HTTPConnection'
op|'('
name|'url'
op|'.'
name|'netloc'
op|')'
newline|'\n'
dedent|''
name|'elif'
name|'url'
op|'.'
name|'scheme'
op|'=='
string|'"https"'
op|':'
newline|'\n'
indent|'      '
name|'h'
op|'='
name|'http'
op|'.'
name|'client'
op|'.'
name|'HTTPSConnection'
op|'('
name|'url'
op|'.'
name|'netloc'
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'      '
name|'raise'
name|'RuntimeError'
op|'('
string|'"Unknown protocol: %s"'
op|'%'
name|'url'
op|'.'
name|'scheme'
op|')'
newline|'\n'
dedent|''
name|'h'
op|'.'
name|'request'
op|'('
string|"'HEAD'"
op|','
name|'url'
op|'.'
name|'path'
op|')'
newline|'\n'
name|'r'
op|'='
name|'h'
op|'.'
name|'getresponse'
op|'('
op|')'
newline|'\n'
name|'newLoc'
op|'='
name|'r'
op|'.'
name|'getheader'
op|'('
string|"'location'"
op|')'
newline|'\n'
name|'if'
name|'newLoc'
name|'is'
name|'not'
name|'None'
op|':'
newline|'\n'
indent|'      '
name|'urlString'
op|'='
name|'newLoc'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'      '
name|'break'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'links'
op|'='
op|'['
op|']'
newline|'\n'
name|'try'
op|':'
newline|'\n'
indent|'    '
name|'html'
op|'='
name|'load'
op|'('
name|'urlString'
op|')'
newline|'\n'
dedent|''
name|'except'
op|':'
newline|'\n'
indent|'    '
name|'print'
op|'('
string|"'\\nFAILED to open url %s'"
op|'%'
name|'urlString'
op|')'
newline|'\n'
name|'traceback'
op|'.'
name|'print_exc'
op|'('
op|')'
newline|'\n'
name|'raise'
newline|'\n'
nl|'\n'
dedent|''
name|'for'
name|'subUrl'
op|','
name|'text'
name|'in'
name|'reHREF'
op|'.'
name|'findall'
op|'('
name|'html'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'fullURL'
op|'='
name|'urllib'
op|'.'
name|'parse'
op|'.'
name|'urljoin'
op|'('
name|'urlString'
op|','
name|'subUrl'
op|')'
newline|'\n'
name|'links'
op|'.'
name|'append'
op|'('
op|'('
name|'text'
op|','
name|'fullURL'
op|')'
op|')'
newline|'\n'
dedent|''
name|'return'
name|'links'
newline|'\n'
nl|'\n'
DECL|function|download
dedent|''
name|'def'
name|'download'
op|'('
name|'name'
op|','
name|'urlString'
op|','
name|'tmpDir'
op|','
name|'quiet'
op|'='
name|'False'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'startTime'
op|'='
name|'time'
op|'.'
name|'time'
op|'('
op|')'
newline|'\n'
name|'fileName'
op|'='
string|"'%s/%s'"
op|'%'
op|'('
name|'tmpDir'
op|','
name|'name'
op|')'
newline|'\n'
name|'if'
name|'not'
name|'FORCE_CLEAN'
name|'and'
name|'os'
op|'.'
name|'path'
op|'.'
name|'exists'
op|'('
name|'fileName'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'if'
name|'not'
name|'quiet'
name|'and'
name|'fileName'
op|'.'
name|'find'
op|'('
string|"'.asc'"
op|')'
op|'=='
op|'-'
number|'1'
op|':'
newline|'\n'
indent|'      '
name|'print'
op|'('
string|"'    already done: %.1f MB'"
op|'%'
op|'('
name|'os'
op|'.'
name|'path'
op|'.'
name|'getsize'
op|'('
name|'fileName'
op|')'
op|'/'
number|'1024.'
op|'/'
number|'1024.'
op|')'
op|')'
newline|'\n'
dedent|''
name|'return'
newline|'\n'
dedent|''
name|'try'
op|':'
newline|'\n'
indent|'    '
name|'attemptDownload'
op|'('
name|'urlString'
op|','
name|'fileName'
op|')'
newline|'\n'
dedent|''
name|'except'
name|'Exception'
name|'as'
name|'e'
op|':'
newline|'\n'
indent|'    '
name|'print'
op|'('
string|"'Retrying download of url %s after exception: %s'"
op|'%'
op|'('
name|'urlString'
op|','
name|'e'
op|')'
op|')'
newline|'\n'
name|'try'
op|':'
newline|'\n'
indent|'      '
name|'attemptDownload'
op|'('
name|'urlString'
op|','
name|'fileName'
op|')'
newline|'\n'
dedent|''
name|'except'
name|'Exception'
name|'as'
name|'e'
op|':'
newline|'\n'
indent|'      '
name|'raise'
name|'RuntimeError'
op|'('
string|'\'failed to download url "%s"\''
op|'%'
name|'urlString'
op|')'
name|'from'
name|'e'
newline|'\n'
dedent|''
dedent|''
name|'if'
name|'not'
name|'quiet'
name|'and'
name|'fileName'
op|'.'
name|'find'
op|'('
string|"'.asc'"
op|')'
op|'=='
op|'-'
number|'1'
op|':'
newline|'\n'
indent|'    '
name|'t'
op|'='
name|'time'
op|'.'
name|'time'
op|'('
op|')'
op|'-'
name|'startTime'
newline|'\n'
name|'sizeMB'
op|'='
name|'os'
op|'.'
name|'path'
op|'.'
name|'getsize'
op|'('
name|'fileName'
op|')'
op|'/'
number|'1024.'
op|'/'
number|'1024.'
newline|'\n'
name|'print'
op|'('
string|"'    %.1f MB in %.2f sec (%.1f MB/sec)'"
op|'%'
op|'('
name|'sizeMB'
op|','
name|'t'
op|','
name|'sizeMB'
op|'/'
name|'t'
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|function|attemptDownload
dedent|''
dedent|''
name|'def'
name|'attemptDownload'
op|'('
name|'urlString'
op|','
name|'fileName'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'fIn'
op|'='
name|'urllib'
op|'.'
name|'request'
op|'.'
name|'urlopen'
op|'('
name|'urlString'
op|')'
newline|'\n'
name|'fOut'
op|'='
name|'open'
op|'('
name|'fileName'
op|','
string|"'wb'"
op|')'
newline|'\n'
name|'success'
op|'='
name|'False'
newline|'\n'
name|'try'
op|':'
newline|'\n'
indent|'    '
name|'while'
name|'True'
op|':'
newline|'\n'
indent|'      '
name|'s'
op|'='
name|'fIn'
op|'.'
name|'read'
op|'('
number|'65536'
op|')'
newline|'\n'
name|'if'
name|'s'
op|'=='
string|"b''"
op|':'
newline|'\n'
indent|'        '
name|'break'
newline|'\n'
dedent|''
name|'fOut'
op|'.'
name|'write'
op|'('
name|'s'
op|')'
newline|'\n'
dedent|''
name|'fOut'
op|'.'
name|'close'
op|'('
op|')'
newline|'\n'
name|'fIn'
op|'.'
name|'close'
op|'('
op|')'
newline|'\n'
name|'success'
op|'='
name|'True'
newline|'\n'
dedent|''
name|'finally'
op|':'
newline|'\n'
indent|'    '
name|'fIn'
op|'.'
name|'close'
op|'('
op|')'
newline|'\n'
name|'fOut'
op|'.'
name|'close'
op|'('
op|')'
newline|'\n'
name|'if'
name|'not'
name|'success'
op|':'
newline|'\n'
indent|'      '
name|'os'
op|'.'
name|'remove'
op|'('
name|'fileName'
op|')'
newline|'\n'
nl|'\n'
DECL|function|load
dedent|''
dedent|''
dedent|''
name|'def'
name|'load'
op|'('
name|'urlString'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'try'
op|':'
newline|'\n'
indent|'    '
name|'content'
op|'='
name|'urllib'
op|'.'
name|'request'
op|'.'
name|'urlopen'
op|'('
name|'urlString'
op|')'
op|'.'
name|'read'
op|'('
op|')'
op|'.'
name|'decode'
op|'('
string|"'utf-8'"
op|')'
newline|'\n'
dedent|''
name|'except'
name|'Exception'
name|'as'
name|'e'
op|':'
newline|'\n'
indent|'    '
name|'print'
op|'('
string|"'Retrying download of url %s after exception: %s'"
op|'%'
op|'('
name|'urlString'
op|','
name|'e'
op|')'
op|')'
newline|'\n'
name|'content'
op|'='
name|'urllib'
op|'.'
name|'request'
op|'.'
name|'urlopen'
op|'('
name|'urlString'
op|')'
op|'.'
name|'read'
op|'('
op|')'
op|'.'
name|'decode'
op|'('
string|"'utf-8'"
op|')'
newline|'\n'
dedent|''
name|'return'
name|'content'
newline|'\n'
nl|'\n'
DECL|function|noJavaPackageClasses
dedent|''
name|'def'
name|'noJavaPackageClasses'
op|'('
name|'desc'
op|','
name|'file'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'with'
name|'zipfile'
op|'.'
name|'ZipFile'
op|'('
name|'file'
op|')'
name|'as'
name|'z2'
op|':'
newline|'\n'
indent|'    '
name|'for'
name|'name2'
name|'in'
name|'z2'
op|'.'
name|'namelist'
op|'('
op|')'
op|':'
newline|'\n'
indent|'      '
name|'if'
name|'name2'
op|'.'
name|'endswith'
op|'('
string|"'.class'"
op|')'
name|'and'
op|'('
name|'name2'
op|'.'
name|'startswith'
op|'('
string|"'java/'"
op|')'
name|'or'
name|'name2'
op|'.'
name|'startswith'
op|'('
string|"'javax/'"
op|')'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'RuntimeError'
op|'('
string|'\'%s contains sheisty class "%s"\''
op|'%'
op|'('
name|'desc'
op|','
name|'name2'
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|function|decodeUTF8
dedent|''
dedent|''
dedent|''
dedent|''
name|'def'
name|'decodeUTF8'
op|'('
name|'bytes'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'return'
name|'codecs'
op|'.'
name|'getdecoder'
op|'('
string|"'UTF-8'"
op|')'
op|'('
name|'bytes'
op|')'
op|'['
number|'0'
op|']'
newline|'\n'
nl|'\n'
DECL|variable|MANIFEST_FILE_NAME
dedent|''
name|'MANIFEST_FILE_NAME'
op|'='
string|"'META-INF/MANIFEST.MF'"
newline|'\n'
DECL|variable|NOTICE_FILE_NAME
name|'NOTICE_FILE_NAME'
op|'='
string|"'META-INF/NOTICE.txt'"
newline|'\n'
DECL|variable|LICENSE_FILE_NAME
name|'LICENSE_FILE_NAME'
op|'='
string|"'META-INF/LICENSE.txt'"
newline|'\n'
nl|'\n'
DECL|function|checkJARMetaData
name|'def'
name|'checkJARMetaData'
op|'('
name|'desc'
op|','
name|'jarFile'
op|','
name|'gitRevision'
op|','
name|'version'
op|')'
op|':'
newline|'\n'
nl|'\n'
indent|'  '
name|'with'
name|'zipfile'
op|'.'
name|'ZipFile'
op|'('
name|'jarFile'
op|','
string|"'r'"
op|')'
name|'as'
name|'z'
op|':'
newline|'\n'
indent|'    '
name|'for'
name|'name'
name|'in'
op|'('
name|'MANIFEST_FILE_NAME'
op|','
name|'NOTICE_FILE_NAME'
op|','
name|'LICENSE_FILE_NAME'
op|')'
op|':'
newline|'\n'
indent|'      '
name|'try'
op|':'
newline|'\n'
comment|'# The Python docs state a KeyError is raised ... so this None'
nl|'\n'
comment|'# check is just defensive:'
nl|'\n'
indent|'        '
name|'if'
name|'z'
op|'.'
name|'getinfo'
op|'('
name|'name'
op|')'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'          '
name|'raise'
name|'RuntimeError'
op|'('
string|"'%s is missing %s'"
op|'%'
op|'('
name|'desc'
op|','
name|'name'
op|')'
op|')'
newline|'\n'
dedent|''
dedent|''
name|'except'
name|'KeyError'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'RuntimeError'
op|'('
string|"'%s is missing %s'"
op|'%'
op|'('
name|'desc'
op|','
name|'name'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'s'
op|'='
name|'decodeUTF8'
op|'('
name|'z'
op|'.'
name|'read'
op|'('
name|'MANIFEST_FILE_NAME'
op|')'
op|')'
newline|'\n'
nl|'\n'
name|'for'
name|'verify'
name|'in'
op|'('
nl|'\n'
string|"'Specification-Vendor: The Apache Software Foundation'"
op|','
nl|'\n'
string|"'Implementation-Vendor: The Apache Software Foundation'"
op|','
nl|'\n'
comment|'# Make sure 1.8 compiler was used to build release bits:'
nl|'\n'
string|"'X-Compile-Source-JDK: 8'"
op|','
nl|'\n'
comment|'# Make sure 1.8 ant was used to build release bits: (this will match 1.8+)'
nl|'\n'
string|"'Ant-Version: Apache Ant 1.8'"
op|','
nl|'\n'
comment|'# Make sure .class files are 1.8 format:'
nl|'\n'
string|"'X-Compile-Target-JDK: 8'"
op|','
nl|'\n'
string|"'Specification-Version: %s'"
op|'%'
name|'version'
op|','
nl|'\n'
comment|'# Make sure the release was compiled with 1.8:'
nl|'\n'
string|"'Created-By: 1.8'"
op|')'
op|':'
newline|'\n'
indent|'      '
name|'if'
name|'s'
op|'.'
name|'find'
op|'('
name|'verify'
op|')'
op|'=='
op|'-'
number|'1'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'RuntimeError'
op|'('
string|'\'%s is missing "%s" inside its META-INF/MANIFEST.MF\''
op|'%'
op|'('
name|'desc'
op|','
name|'verify'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'if'
name|'gitRevision'
op|'!='
string|"'skip'"
op|':'
newline|'\n'
comment|'# Make sure this matches the version and git revision we think we are releasing:'
nl|'\n'
comment|"# TODO: LUCENE-7023: is it OK that Implementation-Version's value now spans two lines?"
nl|'\n'
indent|'      '
name|'verifyRevision'
op|'='
string|"'Implementation-Version: %s %s'"
op|'%'
op|'('
name|'version'
op|','
name|'gitRevision'
op|')'
newline|'\n'
name|'if'
name|'s'
op|'.'
name|'find'
op|'('
name|'verifyRevision'
op|')'
op|'=='
op|'-'
number|'1'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'RuntimeError'
op|'('
string|'\'%s is missing "%s" inside its META-INF/MANIFEST.MF (wrong git revision?)\''
op|'%'
op|'('
name|'desc'
op|','
name|'verifyRevision'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'notice'
op|'='
name|'decodeUTF8'
op|'('
name|'z'
op|'.'
name|'read'
op|'('
name|'NOTICE_FILE_NAME'
op|')'
op|')'
newline|'\n'
name|'license'
op|'='
name|'decodeUTF8'
op|'('
name|'z'
op|'.'
name|'read'
op|'('
name|'LICENSE_FILE_NAME'
op|')'
op|')'
newline|'\n'
nl|'\n'
name|'justFileName'
op|'='
name|'os'
op|'.'
name|'path'
op|'.'
name|'split'
op|'('
name|'desc'
op|')'
op|'['
number|'1'
op|']'
newline|'\n'
nl|'\n'
name|'if'
name|'justFileName'
op|'.'
name|'lower'
op|'('
op|')'
op|'.'
name|'find'
op|'('
string|"'solr'"
op|')'
op|'!='
op|'-'
number|'1'
op|':'
newline|'\n'
indent|'      '
name|'if'
name|'SOLR_LICENSE'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'RuntimeError'
op|'('
string|"'BUG in smokeTestRelease!'"
op|')'
newline|'\n'
dedent|''
name|'if'
name|'SOLR_NOTICE'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'RuntimeError'
op|'('
string|"'BUG in smokeTestRelease!'"
op|')'
newline|'\n'
dedent|''
name|'if'
name|'notice'
op|'!='
name|'SOLR_NOTICE'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'RuntimeError'
op|'('
string|"'%s: %s contents doesn\\'t match main NOTICE.txt'"
op|'%'
op|'('
name|'desc'
op|','
name|'NOTICE_FILE_NAME'
op|')'
op|')'
newline|'\n'
dedent|''
name|'if'
name|'license'
op|'!='
name|'SOLR_LICENSE'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'RuntimeError'
op|'('
string|"'%s: %s contents doesn\\'t match main LICENSE.txt'"
op|'%'
op|'('
name|'desc'
op|','
name|'LICENSE_FILE_NAME'
op|')'
op|')'
newline|'\n'
dedent|''
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'      '
name|'if'
name|'LUCENE_LICENSE'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'RuntimeError'
op|'('
string|"'BUG in smokeTestRelease!'"
op|')'
newline|'\n'
dedent|''
name|'if'
name|'LUCENE_NOTICE'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'RuntimeError'
op|'('
string|"'BUG in smokeTestRelease!'"
op|')'
newline|'\n'
dedent|''
name|'if'
name|'notice'
op|'!='
name|'LUCENE_NOTICE'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'RuntimeError'
op|'('
string|"'%s: %s contents doesn\\'t match main NOTICE.txt'"
op|'%'
op|'('
name|'desc'
op|','
name|'NOTICE_FILE_NAME'
op|')'
op|')'
newline|'\n'
dedent|''
name|'if'
name|'license'
op|'!='
name|'LUCENE_LICENSE'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'RuntimeError'
op|'('
string|"'%s: %s contents doesn\\'t match main LICENSE.txt'"
op|'%'
op|'('
name|'desc'
op|','
name|'LICENSE_FILE_NAME'
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|function|normSlashes
dedent|''
dedent|''
dedent|''
dedent|''
name|'def'
name|'normSlashes'
op|'('
name|'path'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'return'
name|'path'
op|'.'
name|'replace'
op|'('
name|'os'
op|'.'
name|'sep'
op|','
string|"'/'"
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|checkAllJARs
dedent|''
name|'def'
name|'checkAllJARs'
op|'('
name|'topDir'
op|','
name|'project'
op|','
name|'gitRevision'
op|','
name|'version'
op|','
name|'tmpDir'
op|','
name|'baseURL'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'print'
op|'('
string|"'    verify JAR metadata/identity/no javax.* or java.* classes...'"
op|')'
newline|'\n'
name|'if'
name|'project'
op|'=='
string|"'solr'"
op|':'
newline|'\n'
indent|'    '
name|'luceneDistFilenames'
op|'='
name|'dict'
op|'('
op|')'
newline|'\n'
name|'for'
name|'file'
name|'in'
name|'getBinaryDistFiles'
op|'('
string|"'lucene'"
op|','
name|'tmpDir'
op|','
name|'version'
op|','
name|'baseURL'
op|')'
op|':'
newline|'\n'
indent|'      '
name|'luceneDistFilenames'
op|'['
name|'os'
op|'.'
name|'path'
op|'.'
name|'basename'
op|'('
name|'file'
op|')'
op|']'
op|'='
name|'file'
newline|'\n'
dedent|''
dedent|''
name|'for'
name|'root'
op|','
name|'dirs'
op|','
name|'files'
name|'in'
name|'os'
op|'.'
name|'walk'
op|'('
name|'topDir'
op|')'
op|':'
newline|'\n'
nl|'\n'
indent|'    '
name|'normRoot'
op|'='
name|'normSlashes'
op|'('
name|'root'
op|')'
newline|'\n'
nl|'\n'
name|'if'
name|'project'
op|'=='
string|"'solr'"
name|'and'
name|'normRoot'
op|'.'
name|'endswith'
op|'('
string|"'/server/lib'"
op|')'
op|':'
newline|'\n'
comment|"# Solr's example intentionally ships servlet JAR:"
nl|'\n'
indent|'      '
name|'continue'
newline|'\n'
nl|'\n'
dedent|''
name|'for'
name|'file'
name|'in'
name|'files'
op|':'
newline|'\n'
indent|'      '
name|'if'
name|'file'
op|'.'
name|'lower'
op|'('
op|')'
op|'.'
name|'endswith'
op|'('
string|"'.jar'"
op|')'
op|':'
newline|'\n'
indent|'        '
name|'if'
name|'project'
op|'=='
string|"'solr'"
op|':'
newline|'\n'
indent|'          '
name|'if'
op|'('
name|'normRoot'
op|'.'
name|'endswith'
op|'('
string|"'/contrib/dataimporthandler-extras/lib'"
op|')'
name|'and'
op|'('
name|'file'
op|'.'
name|'startswith'
op|'('
string|"'javax.mail-'"
op|')'
name|'or'
name|'file'
op|'.'
name|'startswith'
op|'('
string|"'activation-'"
op|')'
op|')'
op|')'
name|'or'
op|'('
name|'normRoot'
op|'.'
name|'endswith'
op|'('
string|"'/test-framework/lib'"
op|')'
name|'and'
name|'file'
op|'.'
name|'startswith'
op|'('
string|"'jersey-'"
op|')'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'print'
op|'('
string|"'      **WARNING**: skipping check of %s/%s: it has javax.* classes'"
op|'%'
op|'('
name|'root'
op|','
name|'file'
op|')'
op|')'
newline|'\n'
name|'continue'
newline|'\n'
dedent|''
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'          '
name|'if'
name|'normRoot'
op|'.'
name|'endswith'
op|'('
string|"'/replicator/lib'"
op|')'
name|'and'
name|'file'
op|'.'
name|'startswith'
op|'('
string|"'javax.servlet'"
op|')'
op|':'
newline|'\n'
indent|'            '
name|'continue'
newline|'\n'
dedent|''
dedent|''
name|'fullPath'
op|'='
string|"'%s/%s'"
op|'%'
op|'('
name|'root'
op|','
name|'file'
op|')'
newline|'\n'
name|'noJavaPackageClasses'
op|'('
string|'\'JAR file "%s"\''
op|'%'
name|'fullPath'
op|','
name|'fullPath'
op|')'
newline|'\n'
name|'if'
name|'file'
op|'.'
name|'lower'
op|'('
op|')'
op|'.'
name|'find'
op|'('
string|"'lucene'"
op|')'
op|'!='
op|'-'
number|'1'
name|'or'
name|'file'
op|'.'
name|'lower'
op|'('
op|')'
op|'.'
name|'find'
op|'('
string|"'solr'"
op|')'
op|'!='
op|'-'
number|'1'
op|':'
newline|'\n'
indent|'          '
name|'checkJARMetaData'
op|'('
string|'\'JAR file "%s"\''
op|'%'
name|'fullPath'
op|','
name|'fullPath'
op|','
name|'gitRevision'
op|','
name|'version'
op|')'
newline|'\n'
dedent|''
name|'if'
name|'project'
op|'=='
string|"'solr'"
name|'and'
name|'file'
op|'.'
name|'lower'
op|'('
op|')'
op|'.'
name|'find'
op|'('
string|"'lucene'"
op|')'
op|'!='
op|'-'
number|'1'
op|':'
newline|'\n'
indent|'          '
name|'jarFilename'
op|'='
name|'os'
op|'.'
name|'path'
op|'.'
name|'basename'
op|'('
name|'file'
op|')'
newline|'\n'
name|'if'
name|'jarFilename'
name|'not'
name|'in'
name|'luceneDistFilenames'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'RuntimeError'
op|'('
string|"'Artifact %s is not present in Lucene binary distribution'"
op|'%'
name|'fullPath'
op|')'
newline|'\n'
dedent|''
name|'identical'
op|'='
name|'filecmp'
op|'.'
name|'cmp'
op|'('
name|'fullPath'
op|','
name|'luceneDistFilenames'
op|'['
name|'jarFilename'
op|']'
op|','
name|'shallow'
op|'='
name|'False'
op|')'
newline|'\n'
name|'if'
name|'not'
name|'identical'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'RuntimeError'
op|'('
string|"'Artifact %s is not identical to %s in Lucene binary distribution'"
nl|'\n'
op|'%'
op|'('
name|'fullPath'
op|','
name|'luceneDistFilenames'
op|'['
name|'jarFilename'
op|']'
op|')'
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|checkSigs
dedent|''
dedent|''
dedent|''
dedent|''
dedent|''
dedent|''
name|'def'
name|'checkSigs'
op|'('
name|'project'
op|','
name|'urlString'
op|','
name|'version'
op|','
name|'tmpDir'
op|','
name|'isSigned'
op|')'
op|':'
newline|'\n'
nl|'\n'
indent|'  '
name|'print'
op|'('
string|"'  test basics...'"
op|')'
newline|'\n'
name|'ents'
op|'='
name|'getDirEntries'
op|'('
name|'urlString'
op|')'
newline|'\n'
name|'artifact'
op|'='
name|'None'
newline|'\n'
name|'keysURL'
op|'='
name|'None'
newline|'\n'
name|'changesURL'
op|'='
name|'None'
newline|'\n'
name|'mavenURL'
op|'='
name|'None'
newline|'\n'
name|'expectedSigs'
op|'='
op|'['
op|']'
newline|'\n'
name|'if'
name|'isSigned'
op|':'
newline|'\n'
indent|'    '
name|'expectedSigs'
op|'.'
name|'append'
op|'('
string|"'asc'"
op|')'
newline|'\n'
dedent|''
name|'expectedSigs'
op|'.'
name|'extend'
op|'('
op|'['
string|"'md5'"
op|','
string|"'sha1'"
op|']'
op|')'
newline|'\n'
nl|'\n'
name|'artifacts'
op|'='
op|'['
op|']'
newline|'\n'
name|'for'
name|'text'
op|','
name|'subURL'
name|'in'
name|'ents'
op|':'
newline|'\n'
indent|'    '
name|'if'
name|'text'
op|'=='
string|"'KEYS'"
op|':'
newline|'\n'
indent|'      '
name|'keysURL'
op|'='
name|'subURL'
newline|'\n'
dedent|''
name|'elif'
name|'text'
op|'=='
string|"'maven/'"
op|':'
newline|'\n'
indent|'      '
name|'mavenURL'
op|'='
name|'subURL'
newline|'\n'
dedent|''
name|'elif'
name|'text'
op|'.'
name|'startswith'
op|'('
string|"'changes'"
op|')'
op|':'
newline|'\n'
indent|'      '
name|'if'
name|'text'
name|'not'
name|'in'
op|'('
string|"'changes/'"
op|','
string|"'changes-%s/'"
op|'%'
name|'version'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'RuntimeError'
op|'('
string|"'%s: found %s vs expected changes-%s/'"
op|'%'
op|'('
name|'project'
op|','
name|'text'
op|','
name|'version'
op|')'
op|')'
newline|'\n'
dedent|''
name|'changesURL'
op|'='
name|'subURL'
newline|'\n'
dedent|''
name|'elif'
name|'artifact'
op|'=='
name|'None'
op|':'
newline|'\n'
indent|'      '
name|'artifact'
op|'='
name|'text'
newline|'\n'
name|'artifactURL'
op|'='
name|'subURL'
newline|'\n'
name|'if'
name|'project'
op|'=='
string|"'solr'"
op|':'
newline|'\n'
indent|'        '
name|'expected'
op|'='
string|"'solr-%s'"
op|'%'
name|'version'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'        '
name|'expected'
op|'='
string|"'lucene-%s'"
op|'%'
name|'version'
newline|'\n'
dedent|''
name|'if'
name|'not'
name|'artifact'
op|'.'
name|'startswith'
op|'('
name|'expected'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'RuntimeError'
op|'('
string|"'%s: unknown artifact %s: expected prefix %s'"
op|'%'
op|'('
name|'project'
op|','
name|'text'
op|','
name|'expected'
op|')'
op|')'
newline|'\n'
dedent|''
name|'sigs'
op|'='
op|'['
op|']'
newline|'\n'
dedent|''
name|'elif'
name|'text'
op|'.'
name|'startswith'
op|'('
name|'artifact'
op|'+'
string|"'.'"
op|')'
op|':'
newline|'\n'
indent|'      '
name|'sigs'
op|'.'
name|'append'
op|'('
name|'text'
op|'['
name|'len'
op|'('
name|'artifact'
op|')'
op|'+'
number|'1'
op|':'
op|']'
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'      '
name|'if'
name|'sigs'
op|'!='
name|'expectedSigs'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'RuntimeError'
op|'('
string|"'%s: artifact %s has wrong sigs: expected %s but got %s'"
op|'%'
op|'('
name|'project'
op|','
name|'artifact'
op|','
name|'expectedSigs'
op|','
name|'sigs'
op|')'
op|')'
newline|'\n'
dedent|''
name|'artifacts'
op|'.'
name|'append'
op|'('
op|'('
name|'artifact'
op|','
name|'artifactURL'
op|')'
op|')'
newline|'\n'
name|'artifact'
op|'='
name|'text'
newline|'\n'
name|'artifactURL'
op|'='
name|'subURL'
newline|'\n'
name|'sigs'
op|'='
op|'['
op|']'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'if'
name|'sigs'
op|'!='
op|'['
op|']'
op|':'
newline|'\n'
indent|'    '
name|'artifacts'
op|'.'
name|'append'
op|'('
op|'('
name|'artifact'
op|','
name|'artifactURL'
op|')'
op|')'
newline|'\n'
name|'if'
name|'sigs'
op|'!='
name|'expectedSigs'
op|':'
newline|'\n'
indent|'      '
name|'raise'
name|'RuntimeError'
op|'('
string|"'%s: artifact %s has wrong sigs: expected %s but got %s'"
op|'%'
op|'('
name|'project'
op|','
name|'artifact'
op|','
name|'expectedSigs'
op|','
name|'sigs'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'if'
name|'project'
op|'=='
string|"'lucene'"
op|':'
newline|'\n'
indent|'    '
name|'expected'
op|'='
op|'['
string|"'lucene-%s-src.tgz'"
op|'%'
name|'version'
op|','
nl|'\n'
string|"'lucene-%s.tgz'"
op|'%'
name|'version'
op|','
nl|'\n'
string|"'lucene-%s.zip'"
op|'%'
name|'version'
op|']'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'    '
name|'expected'
op|'='
op|'['
string|"'solr-%s-src.tgz'"
op|'%'
name|'version'
op|','
nl|'\n'
string|"'solr-%s.tgz'"
op|'%'
name|'version'
op|','
nl|'\n'
string|"'solr-%s.zip'"
op|'%'
name|'version'
op|']'
newline|'\n'
nl|'\n'
dedent|''
name|'actual'
op|'='
op|'['
name|'x'
op|'['
number|'0'
op|']'
name|'for'
name|'x'
name|'in'
name|'artifacts'
op|']'
newline|'\n'
name|'if'
name|'expected'
op|'!='
name|'actual'
op|':'
newline|'\n'
indent|'    '
name|'raise'
name|'RuntimeError'
op|'('
string|"'%s: wrong artifacts: expected %s but got %s'"
op|'%'
op|'('
name|'project'
op|','
name|'expected'
op|','
name|'actual'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'if'
name|'keysURL'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'    '
name|'raise'
name|'RuntimeError'
op|'('
string|"'%s is missing KEYS'"
op|'%'
name|'project'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'print'
op|'('
string|"'  get KEYS'"
op|')'
newline|'\n'
name|'download'
op|'('
string|"'%s.KEYS'"
op|'%'
name|'project'
op|','
name|'keysURL'
op|','
name|'tmpDir'
op|')'
newline|'\n'
nl|'\n'
name|'keysFile'
op|'='
string|"'%s/%s.KEYS'"
op|'%'
op|'('
name|'tmpDir'
op|','
name|'project'
op|')'
newline|'\n'
nl|'\n'
comment|'# Set up clean gpg world; import keys file:'
nl|'\n'
name|'gpgHomeDir'
op|'='
string|"'%s/%s.gpg'"
op|'%'
op|'('
name|'tmpDir'
op|','
name|'project'
op|')'
newline|'\n'
name|'if'
name|'os'
op|'.'
name|'path'
op|'.'
name|'exists'
op|'('
name|'gpgHomeDir'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'shutil'
op|'.'
name|'rmtree'
op|'('
name|'gpgHomeDir'
op|')'
newline|'\n'
dedent|''
name|'os'
op|'.'
name|'makedirs'
op|'('
name|'gpgHomeDir'
op|','
number|'0o700'
op|')'
newline|'\n'
name|'run'
op|'('
string|"'gpg --homedir %s --import %s'"
op|'%'
op|'('
name|'gpgHomeDir'
op|','
name|'keysFile'
op|')'
op|','
nl|'\n'
string|"'%s/%s.gpg.import.log'"
op|'%'
op|'('
name|'tmpDir'
op|','
name|'project'
op|')'
op|')'
newline|'\n'
nl|'\n'
name|'if'
name|'mavenURL'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'    '
name|'raise'
name|'RuntimeError'
op|'('
string|"'%s is missing maven'"
op|'%'
name|'project'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'if'
name|'changesURL'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'    '
name|'raise'
name|'RuntimeError'
op|'('
string|"'%s is missing changes-%s'"
op|'%'
op|'('
name|'project'
op|','
name|'version'
op|')'
op|')'
newline|'\n'
dedent|''
name|'testChanges'
op|'('
name|'project'
op|','
name|'version'
op|','
name|'changesURL'
op|')'
newline|'\n'
nl|'\n'
name|'for'
name|'artifact'
op|','
name|'urlString'
name|'in'
name|'artifacts'
op|':'
newline|'\n'
indent|'    '
name|'print'
op|'('
string|"'  download %s...'"
op|'%'
name|'artifact'
op|')'
newline|'\n'
name|'download'
op|'('
name|'artifact'
op|','
name|'urlString'
op|','
name|'tmpDir'
op|')'
newline|'\n'
name|'verifyDigests'
op|'('
name|'artifact'
op|','
name|'urlString'
op|','
name|'tmpDir'
op|')'
newline|'\n'
nl|'\n'
name|'if'
name|'isSigned'
op|':'
newline|'\n'
indent|'      '
name|'print'
op|'('
string|"'    verify sig'"
op|')'
newline|'\n'
comment|'# Test sig (this is done with a clean brand-new GPG world)'
nl|'\n'
name|'download'
op|'('
name|'artifact'
op|'+'
string|"'.asc'"
op|','
name|'urlString'
op|'+'
string|"'.asc'"
op|','
name|'tmpDir'
op|')'
newline|'\n'
name|'sigFile'
op|'='
string|"'%s/%s.asc'"
op|'%'
op|'('
name|'tmpDir'
op|','
name|'artifact'
op|')'
newline|'\n'
name|'artifactFile'
op|'='
string|"'%s/%s'"
op|'%'
op|'('
name|'tmpDir'
op|','
name|'artifact'
op|')'
newline|'\n'
name|'logFile'
op|'='
string|"'%s/%s.%s.gpg.verify.log'"
op|'%'
op|'('
name|'tmpDir'
op|','
name|'project'
op|','
name|'artifact'
op|')'
newline|'\n'
name|'run'
op|'('
string|"'gpg --homedir %s --verify %s %s'"
op|'%'
op|'('
name|'gpgHomeDir'
op|','
name|'sigFile'
op|','
name|'artifactFile'
op|')'
op|','
nl|'\n'
name|'logFile'
op|')'
newline|'\n'
comment|"# Forward any GPG warnings, except the expected one (since it's a clean world)"
nl|'\n'
name|'f'
op|'='
name|'open'
op|'('
name|'logFile'
op|','
name|'encoding'
op|'='
string|"'UTF-8'"
op|')'
newline|'\n'
name|'for'
name|'line'
name|'in'
name|'f'
op|'.'
name|'readlines'
op|'('
op|')'
op|':'
newline|'\n'
indent|'        '
name|'if'
name|'line'
op|'.'
name|'lower'
op|'('
op|')'
op|'.'
name|'find'
op|'('
string|"'warning'"
op|')'
op|'!='
op|'-'
number|'1'
name|'and'
name|'line'
op|'.'
name|'find'
op|'('
string|"'WARNING: This key is not certified with a trusted signature'"
op|')'
op|'=='
op|'-'
number|'1'
op|':'
newline|'\n'
indent|'          '
name|'print'
op|'('
string|"'      GPG: %s'"
op|'%'
name|'line'
op|'.'
name|'strip'
op|'('
op|')'
op|')'
newline|'\n'
dedent|''
dedent|''
name|'f'
op|'.'
name|'close'
op|'('
op|')'
newline|'\n'
nl|'\n'
comment|'# Test trust (this is done with the real users config)'
nl|'\n'
name|'run'
op|'('
string|"'gpg --import %s'"
op|'%'
op|'('
name|'keysFile'
op|')'
op|','
nl|'\n'
string|"'%s/%s.gpg.trust.import.log'"
op|'%'
op|'('
name|'tmpDir'
op|','
name|'project'
op|')'
op|')'
newline|'\n'
name|'print'
op|'('
string|"'    verify trust'"
op|')'
newline|'\n'
name|'logFile'
op|'='
string|"'%s/%s.%s.gpg.trust.log'"
op|'%'
op|'('
name|'tmpDir'
op|','
name|'project'
op|','
name|'artifact'
op|')'
newline|'\n'
name|'run'
op|'('
string|"'gpg --verify %s %s'"
op|'%'
op|'('
name|'sigFile'
op|','
name|'artifactFile'
op|')'
op|','
name|'logFile'
op|')'
newline|'\n'
comment|'# Forward any GPG warnings:'
nl|'\n'
name|'f'
op|'='
name|'open'
op|'('
name|'logFile'
op|','
name|'encoding'
op|'='
string|"'UTF-8'"
op|')'
newline|'\n'
name|'for'
name|'line'
name|'in'
name|'f'
op|'.'
name|'readlines'
op|'('
op|')'
op|':'
newline|'\n'
indent|'        '
name|'if'
name|'line'
op|'.'
name|'lower'
op|'('
op|')'
op|'.'
name|'find'
op|'('
string|"'warning'"
op|')'
op|'!='
op|'-'
number|'1'
op|':'
newline|'\n'
indent|'          '
name|'print'
op|'('
string|"'      GPG: %s'"
op|'%'
name|'line'
op|'.'
name|'strip'
op|'('
op|')'
op|')'
newline|'\n'
dedent|''
dedent|''
name|'f'
op|'.'
name|'close'
op|'('
op|')'
newline|'\n'
nl|'\n'
DECL|function|testChanges
dedent|''
dedent|''
dedent|''
name|'def'
name|'testChanges'
op|'('
name|'project'
op|','
name|'version'
op|','
name|'changesURLString'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'print'
op|'('
string|"'  check changes HTML...'"
op|')'
newline|'\n'
name|'changesURL'
op|'='
name|'None'
newline|'\n'
name|'for'
name|'text'
op|','
name|'subURL'
name|'in'
name|'getDirEntries'
op|'('
name|'changesURLString'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'if'
name|'text'
op|'=='
string|"'Changes.html'"
op|':'
newline|'\n'
indent|'      '
name|'changesURL'
op|'='
name|'subURL'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'if'
name|'changesURL'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'    '
name|'raise'
name|'RuntimeError'
op|'('
string|"'did not see Changes.html link from %s'"
op|'%'
name|'changesURLString'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'s'
op|'='
name|'load'
op|'('
name|'changesURL'
op|')'
newline|'\n'
name|'checkChangesContent'
op|'('
name|'s'
op|','
name|'version'
op|','
name|'changesURL'
op|','
name|'project'
op|','
name|'True'
op|')'
newline|'\n'
nl|'\n'
DECL|function|testChangesText
dedent|''
name|'def'
name|'testChangesText'
op|'('
name|'dir'
op|','
name|'version'
op|','
name|'project'
op|')'
op|':'
newline|'\n'
indent|'  '
string|'"Checks all CHANGES.txt under this dir."'
newline|'\n'
name|'for'
name|'root'
op|','
name|'dirs'
op|','
name|'files'
name|'in'
name|'os'
op|'.'
name|'walk'
op|'('
name|'dir'
op|')'
op|':'
newline|'\n'
nl|'\n'
comment|'# NOTE: O(N) but N should be smallish:'
nl|'\n'
indent|'    '
name|'if'
string|"'CHANGES.txt'"
name|'in'
name|'files'
op|':'
newline|'\n'
indent|'      '
name|'fullPath'
op|'='
string|"'%s/CHANGES.txt'"
op|'%'
name|'root'
newline|'\n'
comment|"#print 'CHECK %s' % fullPath"
nl|'\n'
name|'checkChangesContent'
op|'('
name|'open'
op|'('
name|'fullPath'
op|','
name|'encoding'
op|'='
string|"'UTF-8'"
op|')'
op|'.'
name|'read'
op|'('
op|')'
op|','
name|'version'
op|','
name|'fullPath'
op|','
name|'project'
op|','
name|'False'
op|')'
newline|'\n'
nl|'\n'
DECL|variable|reChangesSectionHREF
dedent|''
dedent|''
dedent|''
name|'reChangesSectionHREF'
op|'='
name|'re'
op|'.'
name|'compile'
op|'('
string|'\'<a id="(.*?)".*?>(.*?)</a>\''
op|','
name|'re'
op|'.'
name|'IGNORECASE'
op|')'
newline|'\n'
DECL|variable|reUnderbarNotDashHTML
name|'reUnderbarNotDashHTML'
op|'='
name|'re'
op|'.'
name|'compile'
op|'('
string|"r'<li>(\\s*(LUCENE|SOLR)_\\d\\d\\d\\d+)'"
op|')'
newline|'\n'
DECL|variable|reUnderbarNotDashTXT
name|'reUnderbarNotDashTXT'
op|'='
name|'re'
op|'.'
name|'compile'
op|'('
string|"r'\\s+((LUCENE|SOLR)_\\d\\d\\d\\d+)'"
op|','
name|'re'
op|'.'
name|'MULTILINE'
op|')'
newline|'\n'
DECL|function|checkChangesContent
name|'def'
name|'checkChangesContent'
op|'('
name|'s'
op|','
name|'version'
op|','
name|'name'
op|','
name|'project'
op|','
name|'isHTML'
op|')'
op|':'
newline|'\n'
nl|'\n'
indent|'  '
name|'if'
name|'isHTML'
name|'and'
name|'s'
op|'.'
name|'find'
op|'('
string|"'Release %s'"
op|'%'
name|'version'
op|')'
op|'=='
op|'-'
number|'1'
op|':'
newline|'\n'
indent|'    '
name|'raise'
name|'RuntimeError'
op|'('
string|'\'did not see "Release %s" in %s\''
op|'%'
op|'('
name|'version'
op|','
name|'name'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'if'
name|'isHTML'
op|':'
newline|'\n'
indent|'    '
name|'r'
op|'='
name|'reUnderbarNotDashHTML'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'    '
name|'r'
op|'='
name|'reUnderbarNotDashTXT'
newline|'\n'
nl|'\n'
dedent|''
name|'m'
op|'='
name|'r'
op|'.'
name|'search'
op|'('
name|'s'
op|')'
newline|'\n'
name|'if'
name|'m'
name|'is'
name|'not'
name|'None'
op|':'
newline|'\n'
indent|'    '
name|'raise'
name|'RuntimeError'
op|'('
string|"'incorrect issue (_ instead of -) in %s: %s'"
op|'%'
op|'('
name|'name'
op|','
name|'m'
op|'.'
name|'group'
op|'('
number|'1'
op|')'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'if'
name|'s'
op|'.'
name|'lower'
op|'('
op|')'
op|'.'
name|'find'
op|'('
string|"'not yet released'"
op|')'
op|'!='
op|'-'
number|'1'
op|':'
newline|'\n'
indent|'    '
name|'raise'
name|'RuntimeError'
op|'('
string|'\'saw "not yet released" in %s\''
op|'%'
name|'name'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'if'
name|'not'
name|'isHTML'
op|':'
newline|'\n'
indent|'    '
name|'if'
name|'project'
op|'=='
string|"'lucene'"
op|':'
newline|'\n'
indent|'      '
name|'sub'
op|'='
string|"'Lucene %s'"
op|'%'
name|'version'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'      '
name|'sub'
op|'='
name|'version'
newline|'\n'
nl|'\n'
dedent|''
name|'if'
name|'s'
op|'.'
name|'find'
op|'('
name|'sub'
op|')'
op|'=='
op|'-'
number|'1'
op|':'
newline|'\n'
comment|'# benchmark never seems to include release info:'
nl|'\n'
indent|'      '
name|'if'
name|'name'
op|'.'
name|'find'
op|'('
string|"'/benchmark/'"
op|')'
op|'=='
op|'-'
number|'1'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'RuntimeError'
op|'('
string|'\'did not see "%s" in %s\''
op|'%'
op|'('
name|'sub'
op|','
name|'name'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
dedent|''
name|'if'
name|'isHTML'
op|':'
newline|'\n'
comment|'# Make sure a section only appears once under each release:'
nl|'\n'
indent|'    '
name|'seenIDs'
op|'='
name|'set'
op|'('
op|')'
newline|'\n'
name|'seenText'
op|'='
name|'set'
op|'('
op|')'
newline|'\n'
nl|'\n'
name|'release'
op|'='
name|'None'
newline|'\n'
name|'for'
name|'id'
op|','
name|'text'
name|'in'
name|'reChangesSectionHREF'
op|'.'
name|'findall'
op|'('
name|'s'
op|')'
op|':'
newline|'\n'
indent|'      '
name|'if'
name|'text'
op|'.'
name|'lower'
op|'('
op|')'
op|'.'
name|'startswith'
op|'('
string|"'release '"
op|')'
op|':'
newline|'\n'
indent|'        '
name|'release'
op|'='
name|'text'
op|'['
number|'8'
op|':'
op|']'
op|'.'
name|'strip'
op|'('
op|')'
newline|'\n'
name|'seenText'
op|'.'
name|'clear'
op|'('
op|')'
newline|'\n'
dedent|''
name|'if'
name|'id'
name|'in'
name|'seenIDs'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'RuntimeError'
op|'('
string|'\'%s has duplicate section "%s" under release "%s"\''
op|'%'
op|'('
name|'name'
op|','
name|'text'
op|','
name|'release'
op|')'
op|')'
newline|'\n'
dedent|''
name|'seenIDs'
op|'.'
name|'add'
op|'('
name|'id'
op|')'
newline|'\n'
name|'if'
name|'text'
name|'in'
name|'seenText'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'RuntimeError'
op|'('
string|'\'%s has duplicate section "%s" under release "%s"\''
op|'%'
op|'('
name|'name'
op|','
name|'text'
op|','
name|'release'
op|')'
op|')'
newline|'\n'
dedent|''
name|'seenText'
op|'.'
name|'add'
op|'('
name|'text'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
dedent|''
name|'reUnixPath'
op|'='
name|'re'
op|'.'
name|'compile'
op|'('
string|'r\'\\b[a-zA-Z_]+=(?:"(?:\\\\"|[^"])*"\''
op|'+'
string|'\'|(?:\\\\\\\\.|[^"\\\'\\\\s])*\''
op|'+'
string|'r"|\'(?:\\\\\'|[^\'])*\')"'
op|'+'
string|'r\'|(/(?:\\\\.|[^"\\\'\\s])*)\''
op|'+'
string|'r\'|("/(?:\\\\.|[^"])*")\''
DECL|variable|reUnixPath
op|'+'
string|'r"|(\'/(?:\\\\.|[^\'])*\')"'
op|')'
newline|'\n'
nl|'\n'
DECL|function|unix2win
name|'def'
name|'unix2win'
op|'('
name|'matchobj'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'if'
name|'matchobj'
op|'.'
name|'group'
op|'('
number|'1'
op|')'
name|'is'
name|'not'
name|'None'
op|':'
name|'return'
name|'cygwinWindowsRoot'
op|'+'
name|'matchobj'
op|'.'
name|'group'
op|'('
op|')'
newline|'\n'
name|'if'
name|'matchobj'
op|'.'
name|'group'
op|'('
number|'2'
op|')'
name|'is'
name|'not'
name|'None'
op|':'
name|'return'
string|'\'"%s%s\''
op|'%'
op|'('
name|'cygwinWindowsRoot'
op|','
name|'matchobj'
op|'.'
name|'group'
op|'('
op|')'
op|'.'
name|'lstrip'
op|'('
string|'\'"\''
op|')'
op|')'
newline|'\n'
name|'if'
name|'matchobj'
op|'.'
name|'group'
op|'('
number|'3'
op|')'
name|'is'
name|'not'
name|'None'
op|':'
name|'return'
string|'"\'%s%s"'
op|'%'
op|'('
name|'cygwinWindowsRoot'
op|','
name|'matchobj'
op|'.'
name|'group'
op|'('
op|')'
op|'.'
name|'lstrip'
op|'('
string|'"\'"'
op|')'
op|')'
newline|'\n'
name|'return'
name|'matchobj'
op|'.'
name|'group'
op|'('
op|')'
newline|'\n'
nl|'\n'
DECL|function|cygwinifyPaths
dedent|''
name|'def'
name|'cygwinifyPaths'
op|'('
name|'command'
op|')'
op|':'
newline|'\n'
comment|'# The problem: Native Windows applications running under Cygwin'
nl|'\n'
comment|"# (e.g. Ant, which isn't available as a Cygwin package) can't"
nl|'\n'
comment|"# handle Cygwin's Unix-style paths.  However, environment variable"
nl|'\n'
comment|'# values are automatically converted, so only paths outside of'
nl|'\n'
comment|'# environment variable values should be converted to Windows paths.'
nl|'\n'
comment|'# Assumption: all paths will be absolute.'
nl|'\n'
indent|'  '
name|'if'
string|"'; ant '"
name|'in'
name|'command'
op|':'
name|'command'
op|'='
name|'reUnixPath'
op|'.'
name|'sub'
op|'('
name|'unix2win'
op|','
name|'command'
op|')'
newline|'\n'
name|'return'
name|'command'
newline|'\n'
nl|'\n'
DECL|function|printFileContents
dedent|''
name|'def'
name|'printFileContents'
op|'('
name|'fileName'
op|')'
op|':'
newline|'\n'
nl|'\n'
comment|"# Assume log file was written in system's default encoding, but"
nl|'\n'
comment|'# even if we are wrong, we replace errors ... the ASCII chars'
nl|'\n'
comment|'# (which is what we mostly care about eg for the test seed) should'
nl|'\n'
comment|'# still survive:'
nl|'\n'
indent|'  '
name|'txt'
op|'='
name|'codecs'
op|'.'
name|'open'
op|'('
name|'fileName'
op|','
string|"'r'"
op|','
name|'encoding'
op|'='
name|'sys'
op|'.'
name|'getdefaultencoding'
op|'('
op|')'
op|','
name|'errors'
op|'='
string|"'replace'"
op|')'
op|'.'
name|'read'
op|'('
op|')'
newline|'\n'
nl|'\n'
comment|"# Encode to our output encoding (likely also system's default"
nl|'\n'
comment|'# encoding):'
nl|'\n'
name|'bytes'
op|'='
name|'txt'
op|'.'
name|'encode'
op|'('
name|'sys'
op|'.'
name|'stdout'
op|'.'
name|'encoding'
op|','
name|'errors'
op|'='
string|"'replace'"
op|')'
newline|'\n'
nl|'\n'
comment|'# Decode back to string and print... we should hit no exception here'
nl|'\n'
comment|'# since all errors have been replaced:'
nl|'\n'
name|'print'
op|'('
name|'codecs'
op|'.'
name|'getdecoder'
op|'('
name|'sys'
op|'.'
name|'stdout'
op|'.'
name|'encoding'
op|')'
op|'('
name|'bytes'
op|')'
op|'['
number|'0'
op|']'
op|')'
newline|'\n'
name|'print'
op|'('
op|')'
newline|'\n'
nl|'\n'
DECL|function|run
dedent|''
name|'def'
name|'run'
op|'('
name|'command'
op|','
name|'logFile'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'if'
name|'cygwin'
op|':'
name|'command'
op|'='
name|'cygwinifyPaths'
op|'('
name|'command'
op|')'
newline|'\n'
name|'if'
name|'os'
op|'.'
name|'system'
op|'('
string|"'%s > %s 2>&1'"
op|'%'
op|'('
name|'command'
op|','
name|'logFile'
op|')'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'logPath'
op|'='
name|'os'
op|'.'
name|'path'
op|'.'
name|'abspath'
op|'('
name|'logFile'
op|')'
newline|'\n'
name|'print'
op|'('
string|'\'\\ncommand "%s" failed:\''
op|'%'
name|'command'
op|')'
newline|'\n'
name|'printFileContents'
op|'('
name|'logFile'
op|')'
newline|'\n'
name|'raise'
name|'RuntimeError'
op|'('
string|'\'command "%s" failed; see log file %s\''
op|'%'
op|'('
name|'command'
op|','
name|'logPath'
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|function|verifyDigests
dedent|''
dedent|''
name|'def'
name|'verifyDigests'
op|'('
name|'artifact'
op|','
name|'urlString'
op|','
name|'tmpDir'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'print'
op|'('
string|"'    verify md5/sha1 digests'"
op|')'
newline|'\n'
name|'md5Expected'
op|','
name|'t'
op|'='
name|'load'
op|'('
name|'urlString'
op|'+'
string|"'.md5'"
op|')'
op|'.'
name|'strip'
op|'('
op|')'
op|'.'
name|'split'
op|'('
op|')'
newline|'\n'
name|'if'
name|'t'
op|'!='
string|"'*'"
op|'+'
name|'artifact'
op|':'
newline|'\n'
indent|'    '
name|'raise'
name|'RuntimeError'
op|'('
string|"'MD5 %s.md5 lists artifact %s but expected *%s'"
op|'%'
op|'('
name|'urlString'
op|','
name|'t'
op|','
name|'artifact'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'sha1Expected'
op|','
name|'t'
op|'='
name|'load'
op|'('
name|'urlString'
op|'+'
string|"'.sha1'"
op|')'
op|'.'
name|'strip'
op|'('
op|')'
op|'.'
name|'split'
op|'('
op|')'
newline|'\n'
name|'if'
name|'t'
op|'!='
string|"'*'"
op|'+'
name|'artifact'
op|':'
newline|'\n'
indent|'    '
name|'raise'
name|'RuntimeError'
op|'('
string|"'SHA1 %s.sha1 lists artifact %s but expected *%s'"
op|'%'
op|'('
name|'urlString'
op|','
name|'t'
op|','
name|'artifact'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'m'
op|'='
name|'hashlib'
op|'.'
name|'md5'
op|'('
op|')'
newline|'\n'
name|'s'
op|'='
name|'hashlib'
op|'.'
name|'sha1'
op|'('
op|')'
newline|'\n'
name|'f'
op|'='
name|'open'
op|'('
string|"'%s/%s'"
op|'%'
op|'('
name|'tmpDir'
op|','
name|'artifact'
op|')'
op|','
string|"'rb'"
op|')'
newline|'\n'
name|'while'
name|'True'
op|':'
newline|'\n'
indent|'    '
name|'x'
op|'='
name|'f'
op|'.'
name|'read'
op|'('
number|'65536'
op|')'
newline|'\n'
name|'if'
name|'len'
op|'('
name|'x'
op|')'
op|'=='
number|'0'
op|':'
newline|'\n'
indent|'      '
name|'break'
newline|'\n'
dedent|''
name|'m'
op|'.'
name|'update'
op|'('
name|'x'
op|')'
newline|'\n'
name|'s'
op|'.'
name|'update'
op|'('
name|'x'
op|')'
newline|'\n'
dedent|''
name|'f'
op|'.'
name|'close'
op|'('
op|')'
newline|'\n'
name|'md5Actual'
op|'='
name|'m'
op|'.'
name|'hexdigest'
op|'('
op|')'
newline|'\n'
name|'sha1Actual'
op|'='
name|'s'
op|'.'
name|'hexdigest'
op|'('
op|')'
newline|'\n'
name|'if'
name|'md5Actual'
op|'!='
name|'md5Expected'
op|':'
newline|'\n'
indent|'    '
name|'raise'
name|'RuntimeError'
op|'('
string|"'MD5 digest mismatch for %s: expected %s but got %s'"
op|'%'
op|'('
name|'artifact'
op|','
name|'md5Expected'
op|','
name|'md5Actual'
op|')'
op|')'
newline|'\n'
dedent|''
name|'if'
name|'sha1Actual'
op|'!='
name|'sha1Expected'
op|':'
newline|'\n'
indent|'    '
name|'raise'
name|'RuntimeError'
op|'('
string|"'SHA1 digest mismatch for %s: expected %s but got %s'"
op|'%'
op|'('
name|'artifact'
op|','
name|'sha1Expected'
op|','
name|'sha1Actual'
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|function|getDirEntries
dedent|''
dedent|''
name|'def'
name|'getDirEntries'
op|'('
name|'urlString'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'if'
name|'urlString'
op|'.'
name|'startswith'
op|'('
string|"'file:/'"
op|')'
name|'and'
name|'not'
name|'urlString'
op|'.'
name|'startswith'
op|'('
string|"'file://'"
op|')'
op|':'
newline|'\n'
comment|'# stupid bogus ant URI'
nl|'\n'
indent|'    '
name|'urlString'
op|'='
string|'"file:///"'
op|'+'
name|'urlString'
op|'['
number|'6'
op|':'
op|']'
newline|'\n'
nl|'\n'
dedent|''
name|'if'
name|'urlString'
op|'.'
name|'startswith'
op|'('
string|"'file://'"
op|')'
op|':'
newline|'\n'
indent|'    '
name|'path'
op|'='
name|'urlString'
op|'['
number|'7'
op|':'
op|']'
newline|'\n'
name|'if'
name|'path'
op|'.'
name|'endswith'
op|'('
string|"'/'"
op|')'
op|':'
newline|'\n'
indent|'      '
name|'path'
op|'='
name|'path'
op|'['
op|':'
op|'-'
number|'1'
op|']'
newline|'\n'
dedent|''
name|'if'
name|'cygwin'
op|':'
comment|'# Convert Windows path to Cygwin path'
newline|'\n'
indent|'      '
name|'path'
op|'='
name|'re'
op|'.'
name|'sub'
op|'('
string|"r'^/([A-Za-z]):/'"
op|','
string|"r'/cygdrive/\\1/'"
op|','
name|'path'
op|')'
newline|'\n'
dedent|''
name|'l'
op|'='
op|'['
op|']'
newline|'\n'
name|'for'
name|'ent'
name|'in'
name|'os'
op|'.'
name|'listdir'
op|'('
name|'path'
op|')'
op|':'
newline|'\n'
indent|'      '
name|'entPath'
op|'='
string|"'%s/%s'"
op|'%'
op|'('
name|'path'
op|','
name|'ent'
op|')'
newline|'\n'
name|'if'
name|'os'
op|'.'
name|'path'
op|'.'
name|'isdir'
op|'('
name|'entPath'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'entPath'
op|'+='
string|"'/'"
newline|'\n'
name|'ent'
op|'+='
string|"'/'"
newline|'\n'
dedent|''
name|'l'
op|'.'
name|'append'
op|'('
op|'('
name|'ent'
op|','
string|"'file://%s'"
op|'%'
name|'entPath'
op|')'
op|')'
newline|'\n'
dedent|''
name|'l'
op|'.'
name|'sort'
op|'('
op|')'
newline|'\n'
name|'return'
name|'l'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'    '
name|'links'
op|'='
name|'getHREFs'
op|'('
name|'urlString'
op|')'
newline|'\n'
name|'for'
name|'i'
op|','
op|'('
name|'text'
op|','
name|'subURL'
op|')'
name|'in'
name|'enumerate'
op|'('
name|'links'
op|')'
op|':'
newline|'\n'
indent|'      '
name|'if'
name|'text'
op|'=='
string|"'Parent Directory'"
name|'or'
name|'text'
op|'=='
string|"'..'"
op|':'
newline|'\n'
indent|'        '
name|'return'
name|'links'
op|'['
op|'('
name|'i'
op|'+'
number|'1'
op|')'
op|':'
op|']'
newline|'\n'
nl|'\n'
DECL|function|unpackAndVerify
dedent|''
dedent|''
dedent|''
dedent|''
name|'def'
name|'unpackAndVerify'
op|'('
name|'java'
op|','
name|'project'
op|','
name|'tmpDir'
op|','
name|'artifact'
op|','
name|'gitRevision'
op|','
name|'version'
op|','
name|'testArgs'
op|','
name|'baseURL'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'destDir'
op|'='
string|"'%s/unpack'"
op|'%'
name|'tmpDir'
newline|'\n'
name|'if'
name|'os'
op|'.'
name|'path'
op|'.'
name|'exists'
op|'('
name|'destDir'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'shutil'
op|'.'
name|'rmtree'
op|'('
name|'destDir'
op|')'
newline|'\n'
dedent|''
name|'os'
op|'.'
name|'makedirs'
op|'('
name|'destDir'
op|')'
newline|'\n'
name|'os'
op|'.'
name|'chdir'
op|'('
name|'destDir'
op|')'
newline|'\n'
name|'print'
op|'('
string|"'  unpack %s...'"
op|'%'
name|'artifact'
op|')'
newline|'\n'
name|'unpackLogFile'
op|'='
string|"'%s/%s-unpack-%s.log'"
op|'%'
op|'('
name|'tmpDir'
op|','
name|'project'
op|','
name|'artifact'
op|')'
newline|'\n'
name|'if'
name|'artifact'
op|'.'
name|'endswith'
op|'('
string|"'.tar.gz'"
op|')'
name|'or'
name|'artifact'
op|'.'
name|'endswith'
op|'('
string|"'.tgz'"
op|')'
op|':'
newline|'\n'
indent|'    '
name|'run'
op|'('
string|"'tar xzf %s/%s'"
op|'%'
op|'('
name|'tmpDir'
op|','
name|'artifact'
op|')'
op|','
name|'unpackLogFile'
op|')'
newline|'\n'
dedent|''
name|'elif'
name|'artifact'
op|'.'
name|'endswith'
op|'('
string|"'.zip'"
op|')'
op|':'
newline|'\n'
indent|'    '
name|'run'
op|'('
string|"'unzip %s/%s'"
op|'%'
op|'('
name|'tmpDir'
op|','
name|'artifact'
op|')'
op|','
name|'unpackLogFile'
op|')'
newline|'\n'
nl|'\n'
comment|'# make sure it unpacks to proper subdir'
nl|'\n'
dedent|''
name|'l'
op|'='
name|'os'
op|'.'
name|'listdir'
op|'('
name|'destDir'
op|')'
newline|'\n'
name|'expected'
op|'='
string|"'%s-%s'"
op|'%'
op|'('
name|'project'
op|','
name|'version'
op|')'
newline|'\n'
name|'if'
name|'l'
op|'!='
op|'['
name|'expected'
op|']'
op|':'
newline|'\n'
indent|'    '
name|'raise'
name|'RuntimeError'
op|'('
string|"'unpack produced entries %s; expected only %s'"
op|'%'
op|'('
name|'l'
op|','
name|'expected'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'unpackPath'
op|'='
string|"'%s/%s'"
op|'%'
op|'('
name|'destDir'
op|','
name|'expected'
op|')'
newline|'\n'
name|'verifyUnpacked'
op|'('
name|'java'
op|','
name|'project'
op|','
name|'artifact'
op|','
name|'unpackPath'
op|','
name|'gitRevision'
op|','
name|'version'
op|','
name|'testArgs'
op|','
name|'tmpDir'
op|','
name|'baseURL'
op|')'
newline|'\n'
name|'return'
name|'unpackPath'
newline|'\n'
nl|'\n'
DECL|variable|LUCENE_NOTICE
dedent|''
name|'LUCENE_NOTICE'
op|'='
name|'None'
newline|'\n'
DECL|variable|LUCENE_LICENSE
name|'LUCENE_LICENSE'
op|'='
name|'None'
newline|'\n'
DECL|variable|SOLR_NOTICE
name|'SOLR_NOTICE'
op|'='
name|'None'
newline|'\n'
DECL|variable|SOLR_LICENSE
name|'SOLR_LICENSE'
op|'='
name|'None'
newline|'\n'
nl|'\n'
DECL|function|verifyUnpacked
name|'def'
name|'verifyUnpacked'
op|'('
name|'java'
op|','
name|'project'
op|','
name|'artifact'
op|','
name|'unpackPath'
op|','
name|'gitRevision'
op|','
name|'version'
op|','
name|'testArgs'
op|','
name|'tmpDir'
op|','
name|'baseURL'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'global'
name|'LUCENE_NOTICE'
newline|'\n'
name|'global'
name|'LUCENE_LICENSE'
newline|'\n'
name|'global'
name|'SOLR_NOTICE'
newline|'\n'
name|'global'
name|'SOLR_LICENSE'
newline|'\n'
nl|'\n'
name|'os'
op|'.'
name|'chdir'
op|'('
name|'unpackPath'
op|')'
newline|'\n'
name|'isSrc'
op|'='
name|'artifact'
op|'.'
name|'find'
op|'('
string|"'-src'"
op|')'
op|'!='
op|'-'
number|'1'
newline|'\n'
nl|'\n'
name|'l'
op|'='
name|'os'
op|'.'
name|'listdir'
op|'('
name|'unpackPath'
op|')'
newline|'\n'
name|'textFiles'
op|'='
op|'['
string|"'LICENSE'"
op|','
string|"'NOTICE'"
op|','
string|"'README'"
op|']'
newline|'\n'
name|'if'
name|'project'
op|'=='
string|"'lucene'"
op|':'
newline|'\n'
indent|'    '
name|'textFiles'
op|'.'
name|'extend'
op|'('
op|'('
string|"'JRE_VERSION_MIGRATION'"
op|','
string|"'CHANGES'"
op|','
string|"'MIGRATE'"
op|','
string|"'SYSTEM_REQUIREMENTS'"
op|')'
op|')'
newline|'\n'
name|'if'
name|'isSrc'
op|':'
newline|'\n'
indent|'      '
name|'textFiles'
op|'.'
name|'append'
op|'('
string|"'BUILD'"
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'for'
name|'fileName'
name|'in'
name|'textFiles'
op|':'
newline|'\n'
indent|'    '
name|'fileName'
op|'+='
string|"'.txt'"
newline|'\n'
name|'if'
name|'fileName'
name|'not'
name|'in'
name|'l'
op|':'
newline|'\n'
indent|'      '
name|'raise'
name|'RuntimeError'
op|'('
string|'\'file "%s" is missing from artifact %s\''
op|'%'
op|'('
name|'fileName'
op|','
name|'artifact'
op|')'
op|')'
newline|'\n'
dedent|''
name|'l'
op|'.'
name|'remove'
op|'('
name|'fileName'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'if'
name|'project'
op|'=='
string|"'lucene'"
op|':'
newline|'\n'
indent|'    '
name|'if'
name|'LUCENE_NOTICE'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'      '
name|'LUCENE_NOTICE'
op|'='
name|'open'
op|'('
string|"'%s/NOTICE.txt'"
op|'%'
name|'unpackPath'
op|','
name|'encoding'
op|'='
string|"'UTF-8'"
op|')'
op|'.'
name|'read'
op|'('
op|')'
newline|'\n'
dedent|''
name|'if'
name|'LUCENE_LICENSE'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'      '
name|'LUCENE_LICENSE'
op|'='
name|'open'
op|'('
string|"'%s/LICENSE.txt'"
op|'%'
name|'unpackPath'
op|','
name|'encoding'
op|'='
string|"'UTF-8'"
op|')'
op|'.'
name|'read'
op|'('
op|')'
newline|'\n'
dedent|''
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'    '
name|'if'
name|'SOLR_NOTICE'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'      '
name|'SOLR_NOTICE'
op|'='
name|'open'
op|'('
string|"'%s/NOTICE.txt'"
op|'%'
name|'unpackPath'
op|','
name|'encoding'
op|'='
string|"'UTF-8'"
op|')'
op|'.'
name|'read'
op|'('
op|')'
newline|'\n'
dedent|''
name|'if'
name|'SOLR_LICENSE'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'      '
name|'SOLR_LICENSE'
op|'='
name|'open'
op|'('
string|"'%s/LICENSE.txt'"
op|'%'
name|'unpackPath'
op|','
name|'encoding'
op|'='
string|"'UTF-8'"
op|')'
op|'.'
name|'read'
op|'('
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'if'
name|'not'
name|'isSrc'
op|':'
newline|'\n'
comment|'# TODO: we should add verifyModule/verifySubmodule (e.g. analysis) here and recurse through'
nl|'\n'
indent|'    '
name|'if'
name|'project'
op|'=='
string|"'lucene'"
op|':'
newline|'\n'
indent|'      '
name|'expectedJARs'
op|'='
op|'('
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'      '
name|'expectedJARs'
op|'='
op|'('
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'for'
name|'fileName'
name|'in'
name|'expectedJARs'
op|':'
newline|'\n'
indent|'      '
name|'fileName'
op|'+='
string|"'.jar'"
newline|'\n'
name|'if'
name|'fileName'
name|'not'
name|'in'
name|'l'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'RuntimeError'
op|'('
string|'\'%s: file "%s" is missing from artifact %s\''
op|'%'
op|'('
name|'project'
op|','
name|'fileName'
op|','
name|'artifact'
op|')'
op|')'
newline|'\n'
dedent|''
name|'l'
op|'.'
name|'remove'
op|'('
name|'fileName'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'if'
name|'project'
op|'=='
string|"'lucene'"
op|':'
newline|'\n'
comment|'# TODO: clean this up to not be a list of modules that we must maintain'
nl|'\n'
indent|'    '
name|'extras'
op|'='
op|'('
string|"'analysis'"
op|','
string|"'backward-codecs'"
op|','
string|"'benchmark'"
op|','
string|"'classification'"
op|','
string|"'codecs'"
op|','
string|"'core'"
op|','
string|"'demo'"
op|','
string|"'docs'"
op|','
string|"'expressions'"
op|','
string|"'facet'"
op|','
string|"'grouping'"
op|','
string|"'highlighter'"
op|','
string|"'join'"
op|','
string|"'memory'"
op|','
string|"'misc'"
op|','
string|"'queries'"
op|','
string|"'queryparser'"
op|','
string|"'replicator'"
op|','
string|"'sandbox'"
op|','
string|"'spatial'"
op|','
string|"'spatial-extras'"
op|','
string|"'spatial3d'"
op|','
string|"'suggest'"
op|','
string|"'test-framework'"
op|','
string|"'licenses'"
op|')'
newline|'\n'
name|'if'
name|'isSrc'
op|':'
newline|'\n'
indent|'      '
name|'extras'
op|'+='
op|'('
string|"'build.xml'"
op|','
string|"'common-build.xml'"
op|','
string|"'module-build.xml'"
op|','
string|"'top-level-ivy-settings.xml'"
op|','
string|"'default-nested-ivy-settings.xml'"
op|','
string|"'ivy-versions.properties'"
op|','
string|"'ivy-ignore-conflicts.properties'"
op|','
string|"'version.properties'"
op|','
string|"'tools'"
op|','
string|"'site'"
op|')'
newline|'\n'
dedent|''
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'    '
name|'extras'
op|'='
op|'('
op|')'
newline|'\n'
nl|'\n'
comment|'# TODO: if solr, verify lucene/licenses, solr/licenses are present'
nl|'\n'
nl|'\n'
dedent|''
name|'for'
name|'e'
name|'in'
name|'extras'
op|':'
newline|'\n'
indent|'    '
name|'if'
name|'e'
name|'not'
name|'in'
name|'l'
op|':'
newline|'\n'
indent|'      '
name|'raise'
name|'RuntimeError'
op|'('
string|"'%s: %s missing from artifact %s'"
op|'%'
op|'('
name|'project'
op|','
name|'e'
op|','
name|'artifact'
op|')'
op|')'
newline|'\n'
dedent|''
name|'l'
op|'.'
name|'remove'
op|'('
name|'e'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'if'
name|'project'
op|'=='
string|"'lucene'"
op|':'
newline|'\n'
indent|'    '
name|'if'
name|'len'
op|'('
name|'l'
op|')'
op|'>'
number|'0'
op|':'
newline|'\n'
indent|'      '
name|'raise'
name|'RuntimeError'
op|'('
string|"'%s: unexpected files/dirs in artifact %s: %s'"
op|'%'
op|'('
name|'project'
op|','
name|'artifact'
op|','
name|'l'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'if'
name|'isSrc'
op|':'
newline|'\n'
indent|'    '
name|'print'
op|'('
string|"'    make sure no JARs/WARs in src dist...'"
op|')'
newline|'\n'
name|'lines'
op|'='
name|'os'
op|'.'
name|'popen'
op|'('
string|"'find . -name \\\\*.jar'"
op|')'
op|'.'
name|'readlines'
op|'('
op|')'
newline|'\n'
name|'if'
name|'len'
op|'('
name|'lines'
op|')'
op|'!='
number|'0'
op|':'
newline|'\n'
indent|'      '
name|'print'
op|'('
string|"'    FAILED:'"
op|')'
newline|'\n'
name|'for'
name|'line'
name|'in'
name|'lines'
op|':'
newline|'\n'
indent|'        '
name|'print'
op|'('
string|"'      %s'"
op|'%'
name|'line'
op|'.'
name|'strip'
op|'('
op|')'
op|')'
newline|'\n'
dedent|''
name|'raise'
name|'RuntimeError'
op|'('
string|"'source release has JARs...'"
op|')'
newline|'\n'
dedent|''
name|'lines'
op|'='
name|'os'
op|'.'
name|'popen'
op|'('
string|"'find . -name \\\\*.war'"
op|')'
op|'.'
name|'readlines'
op|'('
op|')'
newline|'\n'
name|'if'
name|'len'
op|'('
name|'lines'
op|')'
op|'!='
number|'0'
op|':'
newline|'\n'
indent|'      '
name|'print'
op|'('
string|"'    FAILED:'"
op|')'
newline|'\n'
name|'for'
name|'line'
name|'in'
name|'lines'
op|':'
newline|'\n'
indent|'        '
name|'print'
op|'('
string|"'      %s'"
op|'%'
name|'line'
op|'.'
name|'strip'
op|'('
op|')'
op|')'
newline|'\n'
dedent|''
name|'raise'
name|'RuntimeError'
op|'('
string|"'source release has WARs...'"
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'print'
op|'('
string|'\'    run "ant validate"\''
op|')'
newline|'\n'
name|'java'
op|'.'
name|'run_java8'
op|'('
string|"'ant validate'"
op|','
string|"'%s/validate.log'"
op|'%'
name|'unpackPath'
op|')'
newline|'\n'
nl|'\n'
name|'if'
name|'project'
op|'=='
string|"'lucene'"
op|':'
newline|'\n'
indent|'      '
name|'print'
op|'('
string|'"    run tests w/ Java 8 and testArgs=\'%s\'..."'
op|'%'
name|'testArgs'
op|')'
newline|'\n'
name|'java'
op|'.'
name|'run_java8'
op|'('
string|"'ant clean test %s'"
op|'%'
name|'testArgs'
op|','
string|"'%s/test.log'"
op|'%'
name|'unpackPath'
op|')'
newline|'\n'
name|'java'
op|'.'
name|'run_java8'
op|'('
string|"'ant jar'"
op|','
string|"'%s/compile.log'"
op|'%'
name|'unpackPath'
op|')'
newline|'\n'
name|'testDemo'
op|'('
name|'java'
op|'.'
name|'run_java8'
op|','
name|'isSrc'
op|','
name|'version'
op|','
string|"'1.8'"
op|')'
newline|'\n'
nl|'\n'
name|'print'
op|'('
string|"'    generate javadocs w/ Java 8...'"
op|')'
newline|'\n'
name|'java'
op|'.'
name|'run_java8'
op|'('
string|"'ant javadocs'"
op|','
string|"'%s/javadocs.log'"
op|'%'
name|'unpackPath'
op|')'
newline|'\n'
name|'checkJavadocpathFull'
op|'('
string|"'%s/build/docs'"
op|'%'
name|'unpackPath'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'      '
name|'os'
op|'.'
name|'chdir'
op|'('
string|"'solr'"
op|')'
newline|'\n'
nl|'\n'
name|'print'
op|'('
string|'"    run tests w/ Java 8 and testArgs=\'%s\'..."'
op|'%'
name|'testArgs'
op|')'
newline|'\n'
name|'java'
op|'.'
name|'run_java8'
op|'('
string|"'ant clean test -Dtests.slow=false %s'"
op|'%'
name|'testArgs'
op|','
string|"'%s/test.log'"
op|'%'
name|'unpackPath'
op|')'
newline|'\n'
nl|'\n'
comment|'# test javadocs'
nl|'\n'
name|'print'
op|'('
string|"'    generate javadocs w/ Java 8...'"
op|')'
newline|'\n'
name|'java'
op|'.'
name|'run_java8'
op|'('
string|"'ant clean javadocs'"
op|','
string|"'%s/javadocs.log'"
op|'%'
name|'unpackPath'
op|')'
newline|'\n'
name|'checkJavadocpathFull'
op|'('
string|"'%s/solr/build/docs'"
op|'%'
name|'unpackPath'
op|','
name|'False'
op|')'
newline|'\n'
nl|'\n'
name|'print'
op|'('
string|"'    test solr example w/ Java 8...'"
op|')'
newline|'\n'
name|'java'
op|'.'
name|'run_java8'
op|'('
string|"'ant clean server'"
op|','
string|"'%s/antexample.log'"
op|'%'
name|'unpackPath'
op|')'
newline|'\n'
name|'testSolrExample'
op|'('
name|'unpackPath'
op|','
name|'java'
op|'.'
name|'java8_home'
op|','
name|'True'
op|')'
newline|'\n'
nl|'\n'
name|'os'
op|'.'
name|'chdir'
op|'('
string|"'..'"
op|')'
newline|'\n'
name|'print'
op|'('
string|"'    check NOTICE'"
op|')'
newline|'\n'
name|'testNotice'
op|'('
name|'unpackPath'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'else'
op|':'
newline|'\n'
nl|'\n'
indent|'    '
name|'checkAllJARs'
op|'('
name|'os'
op|'.'
name|'getcwd'
op|'('
op|')'
op|','
name|'project'
op|','
name|'gitRevision'
op|','
name|'version'
op|','
name|'tmpDir'
op|','
name|'baseURL'
op|')'
newline|'\n'
nl|'\n'
name|'if'
name|'project'
op|'=='
string|"'lucene'"
op|':'
newline|'\n'
indent|'      '
name|'testDemo'
op|'('
name|'java'
op|'.'
name|'run_java8'
op|','
name|'isSrc'
op|','
name|'version'
op|','
string|"'1.8'"
op|')'
newline|'\n'
nl|'\n'
name|'print'
op|'('
string|"'    check Lucene\\'s javadoc JAR'"
op|')'
newline|'\n'
name|'checkJavadocpath'
op|'('
string|"'%s/docs'"
op|'%'
name|'unpackPath'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'      '
name|'print'
op|'('
string|"'    copying unpacked distribution for Java 8 ...'"
op|')'
newline|'\n'
name|'java8UnpackPath'
op|'='
string|"'%s-java8'"
op|'%'
name|'unpackPath'
newline|'\n'
name|'if'
name|'os'
op|'.'
name|'path'
op|'.'
name|'exists'
op|'('
name|'java8UnpackPath'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'shutil'
op|'.'
name|'rmtree'
op|'('
name|'java8UnpackPath'
op|')'
newline|'\n'
dedent|''
name|'shutil'
op|'.'
name|'copytree'
op|'('
name|'unpackPath'
op|','
name|'java8UnpackPath'
op|')'
newline|'\n'
name|'os'
op|'.'
name|'chdir'
op|'('
name|'java8UnpackPath'
op|')'
newline|'\n'
name|'print'
op|'('
string|"'    test solr example w/ Java 8...'"
op|')'
newline|'\n'
name|'testSolrExample'
op|'('
name|'java8UnpackPath'
op|','
name|'java'
op|'.'
name|'java8_home'
op|','
name|'False'
op|')'
newline|'\n'
nl|'\n'
name|'os'
op|'.'
name|'chdir'
op|'('
name|'unpackPath'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'testChangesText'
op|'('
string|"'.'"
op|','
name|'version'
op|','
name|'project'
op|')'
newline|'\n'
nl|'\n'
name|'if'
name|'project'
op|'=='
string|"'lucene'"
name|'and'
name|'isSrc'
op|':'
newline|'\n'
indent|'    '
name|'print'
op|'('
string|"'  confirm all releases have coverage in TestBackwardsCompatibility'"
op|')'
newline|'\n'
name|'confirmAllReleasesAreTestedForBackCompat'
op|'('
name|'version'
op|','
name|'unpackPath'
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|testNotice
dedent|''
dedent|''
name|'def'
name|'testNotice'
op|'('
name|'unpackPath'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'solrNotice'
op|'='
name|'open'
op|'('
string|"'%s/NOTICE.txt'"
op|'%'
name|'unpackPath'
op|','
name|'encoding'
op|'='
string|"'UTF-8'"
op|')'
op|'.'
name|'read'
op|'('
op|')'
newline|'\n'
name|'luceneNotice'
op|'='
name|'open'
op|'('
string|"'%s/lucene/NOTICE.txt'"
op|'%'
name|'unpackPath'
op|','
name|'encoding'
op|'='
string|"'UTF-8'"
op|')'
op|'.'
name|'read'
op|'('
op|')'
newline|'\n'
nl|'\n'
name|'expected'
op|'='
string|'"""\n=========================================================================\n==  Apache Lucene Notice                                               ==\n=========================================================================\n\n"""'
op|'+'
name|'luceneNotice'
op|'+'
string|'"""---\n"""'
newline|'\n'
nl|'\n'
name|'if'
name|'solrNotice'
op|'.'
name|'find'
op|'('
name|'expected'
op|')'
op|'=='
op|'-'
number|'1'
op|':'
newline|'\n'
indent|'    '
name|'raise'
name|'RuntimeError'
op|'('
string|"'Solr\\'s NOTICE.txt does not have the verbatim copy, plus header/footer, of Lucene\\'s NOTICE.txt'"
op|')'
newline|'\n'
nl|'\n'
DECL|function|readSolrOutput
dedent|''
dedent|''
name|'def'
name|'readSolrOutput'
op|'('
name|'p'
op|','
name|'startupEvent'
op|','
name|'failureEvent'
op|','
name|'logFile'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'f'
op|'='
name|'open'
op|'('
name|'logFile'
op|','
string|"'wb'"
op|')'
newline|'\n'
name|'try'
op|':'
newline|'\n'
indent|'    '
name|'while'
name|'True'
op|':'
newline|'\n'
indent|'      '
name|'line'
op|'='
name|'p'
op|'.'
name|'stdout'
op|'.'
name|'readline'
op|'('
op|')'
newline|'\n'
name|'if'
name|'len'
op|'('
name|'line'
op|')'
op|'=='
number|'0'
op|':'
newline|'\n'
indent|'        '
name|'p'
op|'.'
name|'poll'
op|'('
op|')'
newline|'\n'
name|'if'
name|'not'
name|'startupEvent'
op|'.'
name|'isSet'
op|'('
op|')'
op|':'
newline|'\n'
indent|'          '
name|'failureEvent'
op|'.'
name|'set'
op|'('
op|')'
newline|'\n'
name|'startupEvent'
op|'.'
name|'set'
op|'('
op|')'
newline|'\n'
dedent|''
name|'break'
newline|'\n'
dedent|''
name|'f'
op|'.'
name|'write'
op|'('
name|'line'
op|')'
newline|'\n'
name|'f'
op|'.'
name|'flush'
op|'('
op|')'
newline|'\n'
comment|"#print('SOLR: %s' % line.strip())"
nl|'\n'
name|'if'
name|'not'
name|'startupEvent'
op|'.'
name|'isSet'
op|'('
op|')'
op|':'
newline|'\n'
indent|'        '
name|'if'
name|'line'
op|'.'
name|'find'
op|'('
string|"b'Started ServerConnector@'"
op|')'
op|'!='
op|'-'
number|'1'
name|'and'
name|'line'
op|'.'
name|'find'
op|'('
string|"b'{HTTP/1.1}{0.0.0.0:8983}'"
op|')'
op|'!='
op|'-'
number|'1'
op|':'
newline|'\n'
indent|'          '
name|'startupEvent'
op|'.'
name|'set'
op|'('
op|')'
newline|'\n'
dedent|''
name|'elif'
name|'p'
op|'.'
name|'poll'
op|'('
op|')'
name|'is'
name|'not'
name|'None'
op|':'
newline|'\n'
indent|'          '
name|'failureEvent'
op|'.'
name|'set'
op|'('
op|')'
newline|'\n'
name|'startupEvent'
op|'.'
name|'set'
op|'('
op|')'
newline|'\n'
name|'break'
newline|'\n'
dedent|''
dedent|''
dedent|''
dedent|''
name|'except'
op|':'
newline|'\n'
indent|'    '
name|'print'
op|'('
op|')'
newline|'\n'
name|'print'
op|'('
string|"'Exception reading Solr output:'"
op|')'
newline|'\n'
name|'traceback'
op|'.'
name|'print_exc'
op|'('
op|')'
newline|'\n'
name|'failureEvent'
op|'.'
name|'set'
op|'('
op|')'
newline|'\n'
name|'startupEvent'
op|'.'
name|'set'
op|'('
op|')'
newline|'\n'
dedent|''
name|'finally'
op|':'
newline|'\n'
indent|'    '
name|'f'
op|'.'
name|'close'
op|'('
op|')'
newline|'\n'
nl|'\n'
DECL|function|testSolrExample
dedent|''
dedent|''
name|'def'
name|'testSolrExample'
op|'('
name|'unpackPath'
op|','
name|'javaPath'
op|','
name|'isSrc'
op|')'
op|':'
newline|'\n'
comment|'# test solr using some examples it comes with'
nl|'\n'
indent|'  '
name|'logFile'
op|'='
string|"'%s/solr-example.log'"
op|'%'
name|'unpackPath'
newline|'\n'
name|'if'
name|'isSrc'
op|':'
newline|'\n'
indent|'    '
name|'os'
op|'.'
name|'chdir'
op|'('
name|'unpackPath'
op|'+'
string|"'/solr'"
op|')'
newline|'\n'
name|'subprocess'
op|'.'
name|'call'
op|'('
op|'['
string|"'chmod'"
op|','
string|"'+x'"
op|','
name|'unpackPath'
op|'+'
string|"'/solr/bin/solr'"
op|']'
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'    '
name|'os'
op|'.'
name|'chdir'
op|'('
name|'unpackPath'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'print'
op|'('
string|"'      start Solr instance (log=%s)...'"
op|'%'
name|'logFile'
op|')'
newline|'\n'
name|'env'
op|'='
op|'{'
op|'}'
newline|'\n'
name|'env'
op|'.'
name|'update'
op|'('
name|'os'
op|'.'
name|'environ'
op|')'
newline|'\n'
name|'env'
op|'['
string|"'JAVA_HOME'"
op|']'
op|'='
name|'javaPath'
newline|'\n'
name|'env'
op|'['
string|"'PATH'"
op|']'
op|'='
string|"'%s/bin:%s'"
op|'%'
op|'('
name|'javaPath'
op|','
name|'env'
op|'['
string|"'PATH'"
op|']'
op|')'
newline|'\n'
nl|'\n'
comment|"# Stop Solr running on port 8983 (in case a previous run didn't shutdown cleanly)"
nl|'\n'
name|'try'
op|':'
newline|'\n'
indent|'      '
name|'subprocess'
op|'.'
name|'call'
op|'('
op|'['
string|"'bin/solr'"
op|','
string|"'stop'"
op|','
string|"'-p'"
op|','
string|"'8983'"
op|']'
op|')'
newline|'\n'
dedent|''
name|'except'
op|':'
newline|'\n'
indent|'      '
name|'print'
op|'('
string|"'      Stop failed due to: '"
op|'+'
name|'sys'
op|'.'
name|'exc_info'
op|'('
op|')'
op|'['
number|'0'
op|']'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'print'
op|'('
string|"'      Running techproducts example on port 8983 from %s'"
op|'%'
name|'unpackPath'
op|')'
newline|'\n'
name|'try'
op|':'
newline|'\n'
indent|'    '
name|'runExampleStatus'
op|'='
name|'subprocess'
op|'.'
name|'call'
op|'('
op|'['
string|"'bin/solr'"
op|','
string|"'-e'"
op|','
string|"'techproducts'"
op|']'
op|')'
newline|'\n'
name|'if'
name|'runExampleStatus'
op|'!='
number|'0'
op|':'
newline|'\n'
indent|'      '
name|'raise'
name|'RuntimeError'
op|'('
string|"'Failed to run the techproducts example, check log for previous errors.'"
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'os'
op|'.'
name|'chdir'
op|'('
string|"'example'"
op|')'
newline|'\n'
name|'print'
op|'('
string|"'      test utf8...'"
op|')'
newline|'\n'
name|'run'
op|'('
string|"'sh ./exampledocs/test_utf8.sh http://localhost:8983/solr/techproducts'"
op|','
string|"'utf8.log'"
op|')'
newline|'\n'
name|'print'
op|'('
string|"'      run query...'"
op|')'
newline|'\n'
name|'s'
op|'='
name|'load'
op|'('
string|"'http://localhost:8983/solr/techproducts/select/?q=video'"
op|')'
newline|'\n'
name|'if'
name|'s'
op|'.'
name|'find'
op|'('
string|'\'<result name="response" numFound="3" start="0">\''
op|')'
op|'=='
op|'-'
number|'1'
op|':'
newline|'\n'
indent|'      '
name|'print'
op|'('
string|"'FAILED: response is:\\n%s'"
op|'%'
name|'s'
op|')'
newline|'\n'
name|'raise'
name|'RuntimeError'
op|'('
string|"'query on solr example instance failed'"
op|')'
newline|'\n'
dedent|''
dedent|''
name|'finally'
op|':'
newline|'\n'
comment|'# Stop server:'
nl|'\n'
indent|'    '
name|'print'
op|'('
string|"'      stop server using: bin/solr stop -p 8983'"
op|')'
newline|'\n'
name|'if'
name|'isSrc'
op|':'
newline|'\n'
indent|'      '
name|'os'
op|'.'
name|'chdir'
op|'('
name|'unpackPath'
op|'+'
string|"'/solr'"
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'      '
name|'os'
op|'.'
name|'chdir'
op|'('
name|'unpackPath'
op|')'
newline|'\n'
dedent|''
name|'subprocess'
op|'.'
name|'call'
op|'('
op|'['
string|"'bin/solr'"
op|','
string|"'stop'"
op|','
string|"'-p'"
op|','
string|"'8983'"
op|']'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'if'
name|'isSrc'
op|':'
newline|'\n'
indent|'    '
name|'os'
op|'.'
name|'chdir'
op|'('
name|'unpackPath'
op|'+'
string|"'/solr'"
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'    '
name|'os'
op|'.'
name|'chdir'
op|'('
name|'unpackPath'
op|')'
newline|'\n'
nl|'\n'
comment|'# the weaker check: we can use this on java6 for some checks,'
nl|'\n'
comment|'# but its generated HTML is hopelessly broken so we cannot run'
nl|'\n'
comment|'# the link checking that checkJavadocpathFull does.'
nl|'\n'
DECL|function|checkJavadocpath
dedent|''
dedent|''
name|'def'
name|'checkJavadocpath'
op|'('
name|'path'
op|','
name|'failOnMissing'
op|'='
name|'True'
op|')'
op|':'
newline|'\n'
comment|"# check for level='package'"
nl|'\n'
comment|'# we fail here if its screwed up'
nl|'\n'
indent|'  '
name|'if'
name|'failOnMissing'
name|'and'
name|'checkJavaDocs'
op|'.'
name|'checkPackageSummaries'
op|'('
name|'path'
op|','
string|"'package'"
op|')'
op|':'
newline|'\n'
indent|'    '
name|'raise'
name|'RuntimeError'
op|'('
string|"'missing javadocs package summaries!'"
op|')'
newline|'\n'
nl|'\n'
comment|"# now check for level='class'"
nl|'\n'
dedent|''
name|'if'
name|'checkJavaDocs'
op|'.'
name|'checkPackageSummaries'
op|'('
name|'path'
op|')'
op|':'
newline|'\n'
comment|'# disabled: RM cannot fix all this, see LUCENE-3887'
nl|'\n'
comment|"# raise RuntimeError('javadoc problems')"
nl|'\n'
indent|'    '
name|'print'
op|'('
string|"'\\n***WARNING***: javadocs want to fail!\\n'"
op|')'
newline|'\n'
nl|'\n'
comment|'# full checks'
nl|'\n'
DECL|function|checkJavadocpathFull
dedent|''
dedent|''
name|'def'
name|'checkJavadocpathFull'
op|'('
name|'path'
op|','
name|'failOnMissing'
op|'='
name|'True'
op|')'
op|':'
newline|'\n'
comment|'# check for missing, etc'
nl|'\n'
indent|'  '
name|'checkJavadocpath'
op|'('
name|'path'
op|','
name|'failOnMissing'
op|')'
newline|'\n'
nl|'\n'
comment|'# also validate html/check for broken links'
nl|'\n'
name|'if'
name|'checkJavadocLinks'
op|'.'
name|'checkAll'
op|'('
name|'path'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'raise'
name|'RuntimeError'
op|'('
string|"'broken javadocs links found!'"
op|')'
newline|'\n'
nl|'\n'
DECL|function|testDemo
dedent|''
dedent|''
name|'def'
name|'testDemo'
op|'('
name|'run_java'
op|','
name|'isSrc'
op|','
name|'version'
op|','
name|'jdk'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'if'
name|'os'
op|'.'
name|'path'
op|'.'
name|'exists'
op|'('
string|"'index'"
op|')'
op|':'
newline|'\n'
indent|'    '
name|'shutil'
op|'.'
name|'rmtree'
op|'('
string|"'index'"
op|')'
comment|'# nuke any index from any previous iteration'
newline|'\n'
nl|'\n'
dedent|''
name|'print'
op|'('
string|"'    test demo with %s...'"
op|'%'
name|'jdk'
op|')'
newline|'\n'
name|'sep'
op|'='
string|"';'"
name|'if'
name|'cygwin'
name|'else'
string|"':'"
newline|'\n'
name|'if'
name|'isSrc'
op|':'
newline|'\n'
indent|'    '
name|'cp'
op|'='
string|"'build/core/classes/java{0}build/demo/classes/java{0}build/analysis/common/classes/java{0}build/queryparser/classes/java'"
op|'.'
name|'format'
op|'('
name|'sep'
op|')'
newline|'\n'
name|'docsDir'
op|'='
string|"'core/src'"
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'    '
name|'cp'
op|'='
string|"'core/lucene-core-{0}.jar{1}demo/lucene-demo-{0}.jar{1}analysis/common/lucene-analyzers-common-{0}.jar{1}queryparser/lucene-queryparser-{0}.jar'"
op|'.'
name|'format'
op|'('
name|'version'
op|','
name|'sep'
op|')'
newline|'\n'
name|'docsDir'
op|'='
string|"'docs'"
newline|'\n'
dedent|''
name|'run_java'
op|'('
string|'\'java -cp "%s" org.apache.lucene.demo.IndexFiles -index index -docs %s\''
op|'%'
op|'('
name|'cp'
op|','
name|'docsDir'
op|')'
op|','
string|"'index.log'"
op|')'
newline|'\n'
name|'run_java'
op|'('
string|'\'java -cp "%s" org.apache.lucene.demo.SearchFiles -index index -query lucene\''
op|'%'
name|'cp'
op|','
string|"'search.log'"
op|')'
newline|'\n'
name|'reMatchingDocs'
op|'='
name|'re'
op|'.'
name|'compile'
op|'('
string|"'(\\d+) total matching documents'"
op|')'
newline|'\n'
name|'m'
op|'='
name|'reMatchingDocs'
op|'.'
name|'search'
op|'('
name|'open'
op|'('
string|"'search.log'"
op|','
name|'encoding'
op|'='
string|"'UTF-8'"
op|')'
op|'.'
name|'read'
op|'('
op|')'
op|')'
newline|'\n'
name|'if'
name|'m'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'    '
name|'raise'
name|'RuntimeError'
op|'('
string|"'lucene demo\\'s SearchFiles found no results'"
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'    '
name|'numHits'
op|'='
name|'int'
op|'('
name|'m'
op|'.'
name|'group'
op|'('
number|'1'
op|')'
op|')'
newline|'\n'
name|'if'
name|'numHits'
op|'<'
number|'100'
op|':'
newline|'\n'
indent|'      '
name|'raise'
name|'RuntimeError'
op|'('
string|"'lucene demo\\'s SearchFiles found too few results: %s'"
op|'%'
name|'numHits'
op|')'
newline|'\n'
dedent|''
name|'print'
op|'('
string|'\'      got %d hits for query "lucene"\''
op|'%'
name|'numHits'
op|')'
newline|'\n'
dedent|''
name|'print'
op|'('
string|"'    checkindex with %s...'"
op|'%'
name|'jdk'
op|')'
newline|'\n'
name|'run_java'
op|'('
string|'\'java -ea -cp "%s" org.apache.lucene.index.CheckIndex index\''
op|'%'
name|'cp'
op|','
string|"'checkindex.log'"
op|')'
newline|'\n'
name|'s'
op|'='
name|'open'
op|'('
string|"'checkindex.log'"
op|')'
op|'.'
name|'read'
op|'('
op|')'
newline|'\n'
name|'m'
op|'='
name|'re'
op|'.'
name|'search'
op|'('
string|"r'^\\s+version=(.*?)$'"
op|','
name|'s'
op|','
name|'re'
op|'.'
name|'MULTILINE'
op|')'
newline|'\n'
name|'if'
name|'m'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'    '
name|'raise'
name|'RuntimeError'
op|'('
string|"'unable to locate version=NNN output from CheckIndex; see checkindex.log'"
op|')'
newline|'\n'
dedent|''
name|'actualVersion'
op|'='
name|'m'
op|'.'
name|'group'
op|'('
number|'1'
op|')'
newline|'\n'
name|'if'
name|'removeTrailingZeros'
op|'('
name|'actualVersion'
op|')'
op|'!='
name|'removeTrailingZeros'
op|'('
name|'version'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'raise'
name|'RuntimeError'
op|'('
string|'\'wrong version from CheckIndex: got "%s" but expected "%s"\''
op|'%'
op|'('
name|'actualVersion'
op|','
name|'version'
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|function|removeTrailingZeros
dedent|''
dedent|''
name|'def'
name|'removeTrailingZeros'
op|'('
name|'version'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'return'
name|'re'
op|'.'
name|'sub'
op|'('
string|"r'(\\.0)*$'"
op|','
string|"''"
op|','
name|'version'
op|')'
newline|'\n'
nl|'\n'
DECL|function|checkMaven
dedent|''
name|'def'
name|'checkMaven'
op|'('
name|'solrSrcUnpackPath'
op|','
name|'baseURL'
op|','
name|'tmpDir'
op|','
name|'gitRevision'
op|','
name|'version'
op|','
name|'isSigned'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'POMtemplates'
op|'='
name|'defaultdict'
op|'('
op|')'
newline|'\n'
name|'getPOMtemplates'
op|'('
name|'solrSrcUnpackPath'
op|','
name|'POMtemplates'
op|','
name|'tmpDir'
op|')'
newline|'\n'
name|'print'
op|'('
string|"'    download artifacts'"
op|')'
newline|'\n'
name|'artifacts'
op|'='
op|'{'
string|"'lucene'"
op|':'
op|'['
op|']'
op|','
string|"'solr'"
op|':'
op|'['
op|']'
op|'}'
newline|'\n'
name|'for'
name|'project'
name|'in'
op|'('
string|"'lucene'"
op|','
string|"'solr'"
op|')'
op|':'
newline|'\n'
indent|'    '
name|'artifactsURL'
op|'='
string|"'%s/%s/maven/org/apache/%s/'"
op|'%'
op|'('
name|'baseURL'
op|','
name|'project'
op|','
name|'project'
op|')'
newline|'\n'
name|'targetDir'
op|'='
string|"'%s/maven/org/apache/%s'"
op|'%'
op|'('
name|'tmpDir'
op|','
name|'project'
op|')'
newline|'\n'
name|'if'
name|'not'
name|'os'
op|'.'
name|'path'
op|'.'
name|'exists'
op|'('
name|'targetDir'
op|')'
op|':'
newline|'\n'
indent|'      '
name|'os'
op|'.'
name|'makedirs'
op|'('
name|'targetDir'
op|')'
newline|'\n'
dedent|''
name|'crawl'
op|'('
name|'artifacts'
op|'['
name|'project'
op|']'
op|','
name|'artifactsURL'
op|','
name|'targetDir'
op|')'
newline|'\n'
dedent|''
name|'print'
op|'('
op|')'
newline|'\n'
name|'verifyPOMperBinaryArtifact'
op|'('
name|'artifacts'
op|','
name|'version'
op|')'
newline|'\n'
name|'verifyArtifactPerPOMtemplate'
op|'('
name|'POMtemplates'
op|','
name|'artifacts'
op|','
name|'tmpDir'
op|','
name|'version'
op|')'
newline|'\n'
name|'verifyMavenDigests'
op|'('
name|'artifacts'
op|')'
newline|'\n'
name|'checkJavadocAndSourceArtifacts'
op|'('
name|'artifacts'
op|','
name|'version'
op|')'
newline|'\n'
name|'verifyDeployedPOMsCoordinates'
op|'('
name|'artifacts'
op|','
name|'version'
op|')'
newline|'\n'
name|'if'
name|'isSigned'
op|':'
newline|'\n'
indent|'    '
name|'verifyMavenSigs'
op|'('
name|'baseURL'
op|','
name|'tmpDir'
op|','
name|'artifacts'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'distFiles'
op|'='
name|'getBinaryDistFilesForMavenChecks'
op|'('
name|'tmpDir'
op|','
name|'version'
op|','
name|'baseURL'
op|')'
newline|'\n'
name|'checkIdenticalMavenArtifacts'
op|'('
name|'distFiles'
op|','
name|'artifacts'
op|','
name|'version'
op|')'
newline|'\n'
nl|'\n'
name|'checkAllJARs'
op|'('
string|"'%s/maven/org/apache/lucene'"
op|'%'
name|'tmpDir'
op|','
string|"'lucene'"
op|','
name|'gitRevision'
op|','
name|'version'
op|','
name|'tmpDir'
op|','
name|'baseURL'
op|')'
newline|'\n'
name|'checkAllJARs'
op|'('
string|"'%s/maven/org/apache/solr'"
op|'%'
name|'tmpDir'
op|','
string|"'solr'"
op|','
name|'gitRevision'
op|','
name|'version'
op|','
name|'tmpDir'
op|','
name|'baseURL'
op|')'
newline|'\n'
nl|'\n'
DECL|function|getBinaryDistFilesForMavenChecks
dedent|''
name|'def'
name|'getBinaryDistFilesForMavenChecks'
op|'('
name|'tmpDir'
op|','
name|'version'
op|','
name|'baseURL'
op|')'
op|':'
newline|'\n'
comment|'# TODO: refactor distribution unpacking so that it only happens once per distribution per smoker run'
nl|'\n'
indent|'  '
name|'distFiles'
op|'='
name|'defaultdict'
op|'('
op|')'
newline|'\n'
name|'for'
name|'project'
name|'in'
op|'('
string|"'lucene'"
op|','
string|"'solr'"
op|')'
op|':'
newline|'\n'
indent|'    '
name|'distFiles'
op|'['
name|'project'
op|']'
op|'='
name|'getBinaryDistFiles'
op|'('
name|'project'
op|','
name|'tmpDir'
op|','
name|'version'
op|','
name|'baseURL'
op|')'
newline|'\n'
dedent|''
name|'return'
name|'distFiles'
newline|'\n'
nl|'\n'
DECL|function|getBinaryDistFiles
dedent|''
name|'def'
name|'getBinaryDistFiles'
op|'('
name|'project'
op|','
name|'tmpDir'
op|','
name|'version'
op|','
name|'baseURL'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'distribution'
op|'='
string|"'%s-%s.tgz'"
op|'%'
op|'('
name|'project'
op|','
name|'version'
op|')'
newline|'\n'
name|'if'
name|'not'
name|'os'
op|'.'
name|'path'
op|'.'
name|'exists'
op|'('
string|"'%s/%s'"
op|'%'
op|'('
name|'tmpDir'
op|','
name|'distribution'
op|')'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'distURL'
op|'='
string|"'%s/%s/%s'"
op|'%'
op|'('
name|'baseURL'
op|','
name|'project'
op|','
name|'distribution'
op|')'
newline|'\n'
name|'print'
op|'('
string|"'    download %s...'"
op|'%'
name|'distribution'
op|','
name|'end'
op|'='
string|"' '"
op|')'
newline|'\n'
name|'download'
op|'('
name|'distribution'
op|','
name|'distURL'
op|','
name|'tmpDir'
op|')'
newline|'\n'
dedent|''
name|'destDir'
op|'='
string|"'%s/unpack-%s-getBinaryDistFiles'"
op|'%'
op|'('
name|'tmpDir'
op|','
name|'project'
op|')'
newline|'\n'
name|'if'
name|'os'
op|'.'
name|'path'
op|'.'
name|'exists'
op|'('
name|'destDir'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'shutil'
op|'.'
name|'rmtree'
op|'('
name|'destDir'
op|')'
newline|'\n'
dedent|''
name|'os'
op|'.'
name|'makedirs'
op|'('
name|'destDir'
op|')'
newline|'\n'
name|'os'
op|'.'
name|'chdir'
op|'('
name|'destDir'
op|')'
newline|'\n'
name|'print'
op|'('
string|"'    unpack %s...'"
op|'%'
name|'distribution'
op|')'
newline|'\n'
name|'unpackLogFile'
op|'='
string|"'%s/unpack-%s-getBinaryDistFiles.log'"
op|'%'
op|'('
name|'tmpDir'
op|','
name|'distribution'
op|')'
newline|'\n'
name|'run'
op|'('
string|"'tar xzf %s/%s'"
op|'%'
op|'('
name|'tmpDir'
op|','
name|'distribution'
op|')'
op|','
name|'unpackLogFile'
op|')'
newline|'\n'
name|'distributionFiles'
op|'='
op|'['
op|']'
newline|'\n'
name|'for'
name|'root'
op|','
name|'dirs'
op|','
name|'files'
name|'in'
name|'os'
op|'.'
name|'walk'
op|'('
name|'destDir'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'distributionFiles'
op|'.'
name|'extend'
op|'('
op|'['
name|'os'
op|'.'
name|'path'
op|'.'
name|'join'
op|'('
name|'root'
op|','
name|'file'
op|')'
name|'for'
name|'file'
name|'in'
name|'files'
op|']'
op|')'
newline|'\n'
dedent|''
name|'return'
name|'distributionFiles'
newline|'\n'
nl|'\n'
DECL|function|checkJavadocAndSourceArtifacts
dedent|''
name|'def'
name|'checkJavadocAndSourceArtifacts'
op|'('
name|'artifacts'
op|','
name|'version'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'print'
op|'('
string|"'    check for javadoc and sources artifacts...'"
op|')'
newline|'\n'
name|'for'
name|'project'
name|'in'
op|'('
string|"'lucene'"
op|','
string|"'solr'"
op|')'
op|':'
newline|'\n'
indent|'    '
name|'for'
name|'artifact'
name|'in'
name|'artifacts'
op|'['
name|'project'
op|']'
op|':'
newline|'\n'
indent|'      '
name|'if'
name|'artifact'
op|'.'
name|'endswith'
op|'('
name|'version'
op|'+'
string|"'.jar'"
op|')'
op|':'
newline|'\n'
indent|'        '
name|'javadocJar'
op|'='
name|'artifact'
op|'['
op|':'
op|'-'
number|'4'
op|']'
op|'+'
string|"'-javadoc.jar'"
newline|'\n'
name|'if'
name|'javadocJar'
name|'not'
name|'in'
name|'artifacts'
op|'['
name|'project'
op|']'
op|':'
newline|'\n'
indent|'          '
name|'raise'
name|'RuntimeError'
op|'('
string|"'missing: %s'"
op|'%'
name|'javadocJar'
op|')'
newline|'\n'
dedent|''
name|'sourcesJar'
op|'='
name|'artifact'
op|'['
op|':'
op|'-'
number|'4'
op|']'
op|'+'
string|"'-sources.jar'"
newline|'\n'
name|'if'
name|'sourcesJar'
name|'not'
name|'in'
name|'artifacts'
op|'['
name|'project'
op|']'
op|':'
newline|'\n'
indent|'          '
name|'raise'
name|'RuntimeError'
op|'('
string|"'missing: %s'"
op|'%'
name|'sourcesJar'
op|')'
newline|'\n'
nl|'\n'
DECL|function|getZipFileEntries
dedent|''
dedent|''
dedent|''
dedent|''
dedent|''
name|'def'
name|'getZipFileEntries'
op|'('
name|'fileName'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'entries'
op|'='
op|'['
op|']'
newline|'\n'
name|'with'
name|'zipfile'
op|'.'
name|'ZipFile'
op|'('
name|'fileName'
op|')'
name|'as'
name|'zf'
op|':'
newline|'\n'
indent|'    '
name|'for'
name|'zi'
name|'in'
name|'zf'
op|'.'
name|'infolist'
op|'('
op|')'
op|':'
newline|'\n'
indent|'      '
name|'entries'
op|'.'
name|'append'
op|'('
name|'zi'
op|'.'
name|'filename'
op|')'
newline|'\n'
comment|'# Sort by name:'
nl|'\n'
dedent|''
dedent|''
name|'entries'
op|'.'
name|'sort'
op|'('
op|')'
newline|'\n'
name|'return'
name|'entries'
newline|'\n'
nl|'\n'
DECL|function|checkIdenticalMavenArtifacts
dedent|''
name|'def'
name|'checkIdenticalMavenArtifacts'
op|'('
name|'distFiles'
op|','
name|'artifacts'
op|','
name|'version'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'print'
op|'('
string|"'    verify that Maven artifacts are same as in the binary distribution...'"
op|')'
newline|'\n'
name|'reJarWar'
op|'='
name|'re'
op|'.'
name|'compile'
op|'('
string|"r'%s\\.[wj]ar$'"
op|'%'
name|'version'
op|')'
comment|'# exclude *-javadoc.jar and *-sources.jar'
newline|'\n'
name|'for'
name|'project'
name|'in'
op|'('
string|"'lucene'"
op|','
string|"'solr'"
op|')'
op|':'
newline|'\n'
indent|'    '
name|'distFilenames'
op|'='
name|'dict'
op|'('
op|')'
newline|'\n'
name|'for'
name|'file'
name|'in'
name|'distFiles'
op|'['
name|'project'
op|']'
op|':'
newline|'\n'
indent|'      '
name|'baseName'
op|'='
name|'os'
op|'.'
name|'path'
op|'.'
name|'basename'
op|'('
name|'file'
op|')'
newline|'\n'
name|'distFilenames'
op|'['
name|'baseName'
op|']'
op|'='
name|'file'
newline|'\n'
dedent|''
name|'for'
name|'artifact'
name|'in'
name|'artifacts'
op|'['
name|'project'
op|']'
op|':'
newline|'\n'
indent|'      '
name|'if'
name|'reJarWar'
op|'.'
name|'search'
op|'('
name|'artifact'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'artifactFilename'
op|'='
name|'os'
op|'.'
name|'path'
op|'.'
name|'basename'
op|'('
name|'artifact'
op|')'
newline|'\n'
name|'if'
name|'artifactFilename'
name|'not'
name|'in'
name|'distFilenames'
op|':'
newline|'\n'
indent|'          '
name|'raise'
name|'RuntimeError'
op|'('
string|"'Maven artifact %s is not present in %s binary distribution'"
nl|'\n'
op|'%'
op|'('
name|'artifact'
op|','
name|'project'
op|')'
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'          '
name|'identical'
op|'='
name|'filecmp'
op|'.'
name|'cmp'
op|'('
name|'artifact'
op|','
name|'distFilenames'
op|'['
name|'artifactFilename'
op|']'
op|','
name|'shallow'
op|'='
name|'False'
op|')'
newline|'\n'
name|'if'
name|'not'
name|'identical'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'RuntimeError'
op|'('
string|"'Maven artifact %s is not identical to %s in %s binary distribution'"
nl|'\n'
op|'%'
op|'('
name|'artifact'
op|','
name|'distFilenames'
op|'['
name|'artifactFilename'
op|']'
op|','
name|'project'
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|function|verifyMavenDigests
dedent|''
dedent|''
dedent|''
dedent|''
dedent|''
dedent|''
name|'def'
name|'verifyMavenDigests'
op|'('
name|'artifacts'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'print'
op|'('
string|'"    verify Maven artifacts\' md5/sha1 digests..."'
op|')'
newline|'\n'
name|'reJarWarPom'
op|'='
name|'re'
op|'.'
name|'compile'
op|'('
string|"r'\\.(?:[wj]ar|pom)$'"
op|')'
newline|'\n'
name|'for'
name|'project'
name|'in'
op|'('
string|"'lucene'"
op|','
string|"'solr'"
op|')'
op|':'
newline|'\n'
indent|'    '
name|'for'
name|'artifactFile'
name|'in'
op|'['
name|'a'
name|'for'
name|'a'
name|'in'
name|'artifacts'
op|'['
name|'project'
op|']'
name|'if'
name|'reJarWarPom'
op|'.'
name|'search'
op|'('
name|'a'
op|')'
op|']'
op|':'
newline|'\n'
indent|'      '
name|'if'
name|'artifactFile'
op|'+'
string|"'.md5'"
name|'not'
name|'in'
name|'artifacts'
op|'['
name|'project'
op|']'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'RuntimeError'
op|'('
string|"'missing: MD5 digest for %s'"
op|'%'
name|'artifactFile'
op|')'
newline|'\n'
dedent|''
name|'if'
name|'artifactFile'
op|'+'
string|"'.sha1'"
name|'not'
name|'in'
name|'artifacts'
op|'['
name|'project'
op|']'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'RuntimeError'
op|'('
string|"'missing: SHA1 digest for %s'"
op|'%'
name|'artifactFile'
op|')'
newline|'\n'
dedent|''
name|'with'
name|'open'
op|'('
name|'artifactFile'
op|'+'
string|"'.md5'"
op|','
name|'encoding'
op|'='
string|"'UTF-8'"
op|')'
name|'as'
name|'md5File'
op|':'
newline|'\n'
indent|'        '
name|'md5Expected'
op|'='
name|'md5File'
op|'.'
name|'read'
op|'('
op|')'
op|'.'
name|'strip'
op|'('
op|')'
newline|'\n'
dedent|''
name|'with'
name|'open'
op|'('
name|'artifactFile'
op|'+'
string|"'.sha1'"
op|','
name|'encoding'
op|'='
string|"'UTF-8'"
op|')'
name|'as'
name|'sha1File'
op|':'
newline|'\n'
indent|'        '
name|'sha1Expected'
op|'='
name|'sha1File'
op|'.'
name|'read'
op|'('
op|')'
op|'.'
name|'strip'
op|'('
op|')'
newline|'\n'
dedent|''
name|'md5'
op|'='
name|'hashlib'
op|'.'
name|'md5'
op|'('
op|')'
newline|'\n'
name|'sha1'
op|'='
name|'hashlib'
op|'.'
name|'sha1'
op|'('
op|')'
newline|'\n'
name|'inputFile'
op|'='
name|'open'
op|'('
name|'artifactFile'
op|','
string|"'rb'"
op|')'
newline|'\n'
name|'while'
name|'True'
op|':'
newline|'\n'
indent|'        '
name|'bytes'
op|'='
name|'inputFile'
op|'.'
name|'read'
op|'('
number|'65536'
op|')'
newline|'\n'
name|'if'
name|'len'
op|'('
name|'bytes'
op|')'
op|'=='
number|'0'
op|':'
newline|'\n'
indent|'          '
name|'break'
newline|'\n'
dedent|''
name|'md5'
op|'.'
name|'update'
op|'('
name|'bytes'
op|')'
newline|'\n'
name|'sha1'
op|'.'
name|'update'
op|'('
name|'bytes'
op|')'
newline|'\n'
dedent|''
name|'inputFile'
op|'.'
name|'close'
op|'('
op|')'
newline|'\n'
name|'md5Actual'
op|'='
name|'md5'
op|'.'
name|'hexdigest'
op|'('
op|')'
newline|'\n'
name|'sha1Actual'
op|'='
name|'sha1'
op|'.'
name|'hexdigest'
op|'('
op|')'
newline|'\n'
name|'if'
name|'md5Actual'
op|'!='
name|'md5Expected'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'RuntimeError'
op|'('
string|"'MD5 digest mismatch for %s: expected %s but got %s'"
nl|'\n'
op|'%'
op|'('
name|'artifactFile'
op|','
name|'md5Expected'
op|','
name|'md5Actual'
op|')'
op|')'
newline|'\n'
dedent|''
name|'if'
name|'sha1Actual'
op|'!='
name|'sha1Expected'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'RuntimeError'
op|'('
string|"'SHA1 digest mismatch for %s: expected %s but got %s'"
nl|'\n'
op|'%'
op|'('
name|'artifactFile'
op|','
name|'sha1Expected'
op|','
name|'sha1Actual'
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|function|getPOMcoordinate
dedent|''
dedent|''
dedent|''
dedent|''
name|'def'
name|'getPOMcoordinate'
op|'('
name|'treeRoot'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'namespace'
op|'='
string|"'{http://maven.apache.org/POM/4.0.0}'"
newline|'\n'
name|'groupId'
op|'='
name|'treeRoot'
op|'.'
name|'find'
op|'('
string|"'%sgroupId'"
op|'%'
name|'namespace'
op|')'
newline|'\n'
name|'if'
name|'groupId'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'    '
name|'groupId'
op|'='
name|'treeRoot'
op|'.'
name|'find'
op|'('
string|"'{0}parent/{0}groupId'"
op|'.'
name|'format'
op|'('
name|'namespace'
op|')'
op|')'
newline|'\n'
dedent|''
name|'groupId'
op|'='
name|'groupId'
op|'.'
name|'text'
op|'.'
name|'strip'
op|'('
op|')'
newline|'\n'
name|'artifactId'
op|'='
name|'treeRoot'
op|'.'
name|'find'
op|'('
string|"'%sartifactId'"
op|'%'
name|'namespace'
op|')'
op|'.'
name|'text'
op|'.'
name|'strip'
op|'('
op|')'
newline|'\n'
name|'version'
op|'='
name|'treeRoot'
op|'.'
name|'find'
op|'('
string|"'%sversion'"
op|'%'
name|'namespace'
op|')'
newline|'\n'
name|'if'
name|'version'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'    '
name|'version'
op|'='
name|'treeRoot'
op|'.'
name|'find'
op|'('
string|"'{0}parent/{0}version'"
op|'.'
name|'format'
op|'('
name|'namespace'
op|')'
op|')'
newline|'\n'
dedent|''
name|'version'
op|'='
name|'version'
op|'.'
name|'text'
op|'.'
name|'strip'
op|'('
op|')'
newline|'\n'
name|'packaging'
op|'='
name|'treeRoot'
op|'.'
name|'find'
op|'('
string|"'%spackaging'"
op|'%'
name|'namespace'
op|')'
newline|'\n'
name|'packaging'
op|'='
string|"'jar'"
name|'if'
name|'packaging'
name|'is'
name|'None'
name|'else'
name|'packaging'
op|'.'
name|'text'
op|'.'
name|'strip'
op|'('
op|')'
newline|'\n'
name|'return'
name|'groupId'
op|','
name|'artifactId'
op|','
name|'packaging'
op|','
name|'version'
newline|'\n'
nl|'\n'
DECL|function|verifyMavenSigs
dedent|''
name|'def'
name|'verifyMavenSigs'
op|'('
name|'baseURL'
op|','
name|'tmpDir'
op|','
name|'artifacts'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'print'
op|'('
string|"'    verify maven artifact sigs'"
op|','
name|'end'
op|'='
string|"' '"
op|')'
newline|'\n'
name|'for'
name|'project'
name|'in'
op|'('
string|"'lucene'"
op|','
string|"'solr'"
op|')'
op|':'
newline|'\n'
indent|'    '
name|'keysFile'
op|'='
string|"'%s/%s.KEYS'"
op|'%'
op|'('
name|'tmpDir'
op|','
name|'project'
op|')'
newline|'\n'
name|'if'
name|'not'
name|'os'
op|'.'
name|'path'
op|'.'
name|'exists'
op|'('
name|'keysFile'
op|')'
op|':'
newline|'\n'
indent|'      '
name|'keysURL'
op|'='
string|"'%s/%s/KEYS'"
op|'%'
op|'('
name|'baseURL'
op|','
name|'project'
op|')'
newline|'\n'
name|'download'
op|'('
string|"'%s.KEYS'"
op|'%'
name|'project'
op|','
name|'keysURL'
op|','
name|'tmpDir'
op|','
name|'quiet'
op|'='
name|'True'
op|')'
newline|'\n'
nl|'\n'
comment|'# Set up clean gpg world; import keys file:'
nl|'\n'
dedent|''
name|'gpgHomeDir'
op|'='
string|"'%s/%s.gpg'"
op|'%'
op|'('
name|'tmpDir'
op|','
name|'project'
op|')'
newline|'\n'
name|'if'
name|'os'
op|'.'
name|'path'
op|'.'
name|'exists'
op|'('
name|'gpgHomeDir'
op|')'
op|':'
newline|'\n'
indent|'      '
name|'shutil'
op|'.'
name|'rmtree'
op|'('
name|'gpgHomeDir'
op|')'
newline|'\n'
dedent|''
name|'os'
op|'.'
name|'makedirs'
op|'('
name|'gpgHomeDir'
op|','
number|'0o700'
op|')'
newline|'\n'
name|'run'
op|'('
string|"'gpg --homedir %s --import %s'"
op|'%'
op|'('
name|'gpgHomeDir'
op|','
name|'keysFile'
op|')'
op|','
nl|'\n'
string|"'%s/%s.gpg.import.log'"
op|'%'
op|'('
name|'tmpDir'
op|','
name|'project'
op|')'
op|')'
newline|'\n'
nl|'\n'
name|'reArtifacts'
op|'='
name|'re'
op|'.'
name|'compile'
op|'('
string|"r'\\.(?:pom|[jw]ar)$'"
op|')'
newline|'\n'
name|'for'
name|'artifactFile'
name|'in'
op|'['
name|'a'
name|'for'
name|'a'
name|'in'
name|'artifacts'
op|'['
name|'project'
op|']'
name|'if'
name|'reArtifacts'
op|'.'
name|'search'
op|'('
name|'a'
op|')'
op|']'
op|':'
newline|'\n'
indent|'      '
name|'artifact'
op|'='
name|'os'
op|'.'
name|'path'
op|'.'
name|'basename'
op|'('
name|'artifactFile'
op|')'
newline|'\n'
name|'sigFile'
op|'='
string|"'%s.asc'"
op|'%'
name|'artifactFile'
newline|'\n'
comment|'# Test sig (this is done with a clean brand-new GPG world)'
nl|'\n'
name|'logFile'
op|'='
string|"'%s/%s.%s.gpg.verify.log'"
op|'%'
op|'('
name|'tmpDir'
op|','
name|'project'
op|','
name|'artifact'
op|')'
newline|'\n'
name|'run'
op|'('
string|"'gpg --homedir %s --verify %s %s'"
op|'%'
op|'('
name|'gpgHomeDir'
op|','
name|'sigFile'
op|','
name|'artifactFile'
op|')'
op|','
nl|'\n'
name|'logFile'
op|')'
newline|'\n'
comment|"# Forward any GPG warnings, except the expected one (since it's a clean world)"
nl|'\n'
name|'f'
op|'='
name|'open'
op|'('
name|'logFile'
op|','
name|'encoding'
op|'='
string|"'UTF-8'"
op|')'
newline|'\n'
name|'for'
name|'line'
name|'in'
name|'f'
op|'.'
name|'readlines'
op|'('
op|')'
op|':'
newline|'\n'
indent|'        '
name|'if'
name|'line'
op|'.'
name|'lower'
op|'('
op|')'
op|'.'
name|'find'
op|'('
string|"'warning'"
op|')'
op|'!='
op|'-'
number|'1'
name|'and'
name|'line'
op|'.'
name|'find'
op|'('
string|"'WARNING: This key is not certified with a trusted signature'"
op|')'
op|'=='
op|'-'
number|'1'
name|'and'
name|'line'
op|'.'
name|'find'
op|'('
string|"'WARNING: using insecure memory'"
op|')'
op|'=='
op|'-'
number|'1'
op|':'
newline|'\n'
indent|'          '
name|'print'
op|'('
string|"'      GPG: %s'"
op|'%'
name|'line'
op|'.'
name|'strip'
op|'('
op|')'
op|')'
newline|'\n'
dedent|''
dedent|''
name|'f'
op|'.'
name|'close'
op|'('
op|')'
newline|'\n'
nl|'\n'
comment|'# Test trust (this is done with the real users config)'
nl|'\n'
name|'run'
op|'('
string|"'gpg --import %s'"
op|'%'
name|'keysFile'
op|','
nl|'\n'
string|"'%s/%s.gpg.trust.import.log'"
op|'%'
op|'('
name|'tmpDir'
op|','
name|'project'
op|')'
op|')'
newline|'\n'
name|'logFile'
op|'='
string|"'%s/%s.%s.gpg.trust.log'"
op|'%'
op|'('
name|'tmpDir'
op|','
name|'project'
op|','
name|'artifact'
op|')'
newline|'\n'
name|'run'
op|'('
string|"'gpg --verify %s %s'"
op|'%'
op|'('
name|'sigFile'
op|','
name|'artifactFile'
op|')'
op|','
name|'logFile'
op|')'
newline|'\n'
comment|'# Forward any GPG warnings:'
nl|'\n'
name|'f'
op|'='
name|'open'
op|'('
name|'logFile'
op|','
name|'encoding'
op|'='
string|"'UTF-8'"
op|')'
newline|'\n'
name|'for'
name|'line'
name|'in'
name|'f'
op|'.'
name|'readlines'
op|'('
op|')'
op|':'
newline|'\n'
indent|'        '
name|'if'
name|'line'
op|'.'
name|'lower'
op|'('
op|')'
op|'.'
name|'find'
op|'('
string|"'warning'"
op|')'
op|'!='
op|'-'
number|'1'
name|'and'
name|'line'
op|'.'
name|'find'
op|'('
string|"'WARNING: This key is not certified with a trusted signature'"
op|')'
op|'=='
op|'-'
number|'1'
name|'and'
name|'line'
op|'.'
name|'find'
op|'('
string|"'WARNING: using insecure memory'"
op|')'
op|'=='
op|'-'
number|'1'
op|':'
newline|'\n'
indent|'          '
name|'print'
op|'('
string|"'      GPG: %s'"
op|'%'
name|'line'
op|'.'
name|'strip'
op|'('
op|')'
op|')'
newline|'\n'
dedent|''
dedent|''
name|'f'
op|'.'
name|'close'
op|'('
op|')'
newline|'\n'
nl|'\n'
name|'sys'
op|'.'
name|'stdout'
op|'.'
name|'write'
op|'('
string|"'.'"
op|')'
newline|'\n'
dedent|''
dedent|''
name|'print'
op|'('
op|')'
newline|'\n'
nl|'\n'
DECL|function|verifyPOMperBinaryArtifact
dedent|''
name|'def'
name|'verifyPOMperBinaryArtifact'
op|'('
name|'artifacts'
op|','
name|'version'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'print'
op|'('
string|"'    verify that each binary artifact has a deployed POM...'"
op|')'
newline|'\n'
name|'reBinaryJarWar'
op|'='
name|'re'
op|'.'
name|'compile'
op|'('
string|"r'%s\\.[jw]ar$'"
op|'%'
name|'re'
op|'.'
name|'escape'
op|'('
name|'version'
op|')'
op|')'
newline|'\n'
name|'for'
name|'project'
name|'in'
op|'('
string|"'lucene'"
op|','
string|"'solr'"
op|')'
op|':'
newline|'\n'
indent|'    '
name|'for'
name|'artifact'
name|'in'
op|'['
name|'a'
name|'for'
name|'a'
name|'in'
name|'artifacts'
op|'['
name|'project'
op|']'
name|'if'
name|'reBinaryJarWar'
op|'.'
name|'search'
op|'('
name|'a'
op|')'
op|']'
op|':'
newline|'\n'
indent|'      '
name|'POM'
op|'='
name|'artifact'
op|'['
op|':'
op|'-'
number|'4'
op|']'
op|'+'
string|"'.pom'"
newline|'\n'
name|'if'
name|'POM'
name|'not'
name|'in'
name|'artifacts'
op|'['
name|'project'
op|']'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'RuntimeError'
op|'('
string|"'missing: POM for %s'"
op|'%'
name|'artifact'
op|')'
newline|'\n'
nl|'\n'
DECL|function|verifyDeployedPOMsCoordinates
dedent|''
dedent|''
dedent|''
dedent|''
name|'def'
name|'verifyDeployedPOMsCoordinates'
op|'('
name|'artifacts'
op|','
name|'version'
op|')'
op|':'
newline|'\n'
indent|'  '
string|'"""\n  verify that each POM\'s coordinate (drawn from its content) matches\n  its filepath, and verify that the corresponding artifact exists.\n  """'
newline|'\n'
name|'print'
op|'('
string|'"    verify deployed POMs\' coordinates..."'
op|')'
newline|'\n'
name|'for'
name|'project'
name|'in'
op|'('
string|"'lucene'"
op|','
string|"'solr'"
op|')'
op|':'
newline|'\n'
indent|'    '
name|'for'
name|'POM'
name|'in'
op|'['
name|'a'
name|'for'
name|'a'
name|'in'
name|'artifacts'
op|'['
name|'project'
op|']'
name|'if'
name|'a'
op|'.'
name|'endswith'
op|'('
string|"'.pom'"
op|')'
op|']'
op|':'
newline|'\n'
indent|'      '
name|'treeRoot'
op|'='
name|'ET'
op|'.'
name|'parse'
op|'('
name|'POM'
op|')'
op|'.'
name|'getroot'
op|'('
op|')'
newline|'\n'
name|'groupId'
op|','
name|'artifactId'
op|','
name|'packaging'
op|','
name|'POMversion'
op|'='
name|'getPOMcoordinate'
op|'('
name|'treeRoot'
op|')'
newline|'\n'
name|'POMpath'
op|'='
string|"'%s/%s/%s/%s-%s.pom'"
op|'%'
op|'('
name|'groupId'
op|'.'
name|'replace'
op|'('
string|"'.'"
op|','
string|"'/'"
op|')'
op|','
name|'artifactId'
op|','
name|'version'
op|','
name|'artifactId'
op|','
name|'version'
op|')'
newline|'\n'
name|'if'
name|'not'
name|'POM'
op|'.'
name|'endswith'
op|'('
name|'POMpath'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'RuntimeError'
op|'('
string|'"Mismatch between POM coordinate %s:%s:%s and filepath: %s"'
nl|'\n'
op|'%'
op|'('
name|'groupId'
op|','
name|'artifactId'
op|','
name|'POMversion'
op|','
name|'POM'
op|')'
op|')'
newline|'\n'
comment|'# Verify that the corresponding artifact exists'
nl|'\n'
dedent|''
name|'artifact'
op|'='
name|'POM'
op|'['
op|':'
op|'-'
number|'3'
op|']'
op|'+'
name|'packaging'
newline|'\n'
name|'if'
name|'artifact'
name|'not'
name|'in'
name|'artifacts'
op|'['
name|'project'
op|']'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'RuntimeError'
op|'('
string|"'Missing corresponding .%s artifact for POM %s'"
op|'%'
op|'('
name|'packaging'
op|','
name|'POM'
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|function|verifyArtifactPerPOMtemplate
dedent|''
dedent|''
dedent|''
dedent|''
name|'def'
name|'verifyArtifactPerPOMtemplate'
op|'('
name|'POMtemplates'
op|','
name|'artifacts'
op|','
name|'tmpDir'
op|','
name|'version'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'print'
op|'('
string|"'    verify that there is an artifact for each POM template...'"
op|')'
newline|'\n'
name|'namespace'
op|'='
string|"'{http://maven.apache.org/POM/4.0.0}'"
newline|'\n'
name|'xpathPlugin'
op|'='
string|"'{0}build/{0}plugins/{0}plugin'"
op|'.'
name|'format'
op|'('
name|'namespace'
op|')'
newline|'\n'
name|'xpathSkipConfiguration'
op|'='
string|"'{0}configuration/{0}skip'"
op|'.'
name|'format'
op|'('
name|'namespace'
op|')'
newline|'\n'
name|'for'
name|'project'
name|'in'
op|'('
string|"'lucene'"
op|','
string|"'solr'"
op|')'
op|':'
newline|'\n'
indent|'    '
name|'for'
name|'POMtemplate'
name|'in'
name|'POMtemplates'
op|'['
name|'project'
op|']'
op|':'
newline|'\n'
indent|'      '
name|'treeRoot'
op|'='
name|'ET'
op|'.'
name|'parse'
op|'('
name|'POMtemplate'
op|')'
op|'.'
name|'getroot'
op|'('
op|')'
newline|'\n'
name|'skipDeploy'
op|'='
name|'False'
newline|'\n'
name|'for'
name|'plugin'
name|'in'
name|'treeRoot'
op|'.'
name|'findall'
op|'('
name|'xpathPlugin'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'artifactId'
op|'='
name|'plugin'
op|'.'
name|'find'
op|'('
string|"'%sartifactId'"
op|'%'
name|'namespace'
op|')'
op|'.'
name|'text'
op|'.'
name|'strip'
op|'('
op|')'
newline|'\n'
name|'if'
name|'artifactId'
op|'=='
string|"'maven-deploy-plugin'"
op|':'
newline|'\n'
indent|'          '
name|'skip'
op|'='
name|'plugin'
op|'.'
name|'find'
op|'('
name|'xpathSkipConfiguration'
op|')'
newline|'\n'
name|'if'
name|'skip'
name|'is'
name|'not'
name|'None'
op|':'
name|'skipDeploy'
op|'='
op|'('
name|'skip'
op|'.'
name|'text'
op|'.'
name|'strip'
op|'('
op|')'
op|'.'
name|'lower'
op|'('
op|')'
op|'=='
string|"'true'"
op|')'
newline|'\n'
dedent|''
dedent|''
name|'if'
name|'not'
name|'skipDeploy'
op|':'
newline|'\n'
indent|'        '
name|'groupId'
op|','
name|'artifactId'
op|','
name|'packaging'
op|','
name|'POMversion'
op|'='
name|'getPOMcoordinate'
op|'('
name|'treeRoot'
op|')'
newline|'\n'
comment|'# Ignore POMversion, since its value will not have been interpolated'
nl|'\n'
name|'artifact'
op|'='
string|"'%s/maven/%s/%s/%s/%s-%s.%s'"
op|'%'
op|'('
name|'tmpDir'
op|','
name|'groupId'
op|'.'
name|'replace'
op|'('
string|"'.'"
op|','
string|"'/'"
op|')'
op|','
name|'artifactId'
op|','
nl|'\n'
name|'version'
op|','
name|'artifactId'
op|','
name|'version'
op|','
name|'packaging'
op|')'
newline|'\n'
name|'if'
name|'artifact'
name|'not'
name|'in'
name|'artifacts'
op|'['
string|"'lucene'"
op|']'
name|'and'
name|'artifact'
name|'not'
name|'in'
name|'artifacts'
op|'['
string|"'solr'"
op|']'
op|':'
newline|'\n'
indent|'          '
name|'raise'
name|'RuntimeError'
op|'('
string|"'Missing artifact %s'"
op|'%'
name|'artifact'
op|')'
newline|'\n'
nl|'\n'
DECL|function|getPOMtemplates
dedent|''
dedent|''
dedent|''
dedent|''
dedent|''
name|'def'
name|'getPOMtemplates'
op|'('
name|'solrSrcUnpackPath'
op|','
name|'POMtemplates'
op|','
name|'tmpDir'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'print'
op|'('
string|"'    find pom.xml.template files in the unpacked Solr source distribution'"
op|')'
newline|'\n'
name|'allPOMtemplates'
op|'='
op|'['
op|']'
newline|'\n'
name|'rePOMtemplate'
op|'='
name|'re'
op|'.'
name|'compile'
op|'('
string|"r'^pom\\.xml\\.template$'"
op|')'
newline|'\n'
name|'for'
name|'root'
op|','
name|'dirs'
op|','
name|'files'
name|'in'
name|'os'
op|'.'
name|'walk'
op|'('
name|'solrSrcUnpackPath'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'allPOMtemplates'
op|'.'
name|'extend'
op|'('
op|'['
name|'os'
op|'.'
name|'path'
op|'.'
name|'join'
op|'('
name|'root'
op|','
name|'f'
op|')'
name|'for'
name|'f'
name|'in'
name|'files'
name|'if'
name|'rePOMtemplate'
op|'.'
name|'search'
op|'('
name|'f'
op|')'
op|']'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'reLucenePOMtemplate'
op|'='
name|'re'
op|'.'
name|'compile'
op|'('
string|"r'.*/maven/lucene.*/pom\\.xml\\.template$'"
op|')'
newline|'\n'
name|'POMtemplates'
op|'['
string|"'lucene'"
op|']'
op|'='
op|'['
name|'p'
name|'for'
name|'p'
name|'in'
name|'allPOMtemplates'
name|'if'
name|'reLucenePOMtemplate'
op|'.'
name|'search'
op|'('
name|'p'
op|')'
op|']'
newline|'\n'
name|'if'
name|'POMtemplates'
op|'['
string|"'lucene'"
op|']'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'    '
name|'raise'
name|'RuntimeError'
op|'('
string|"'No Lucene POMs found at %s'"
op|'%'
name|'solrSrcUnpackPath'
op|')'
newline|'\n'
dedent|''
name|'reSolrPOMtemplate'
op|'='
name|'re'
op|'.'
name|'compile'
op|'('
string|"r'.*/maven/solr.*/pom\\.xml\\.template$'"
op|')'
newline|'\n'
name|'POMtemplates'
op|'['
string|"'solr'"
op|']'
op|'='
op|'['
name|'p'
name|'for'
name|'p'
name|'in'
name|'allPOMtemplates'
name|'if'
name|'reSolrPOMtemplate'
op|'.'
name|'search'
op|'('
name|'p'
op|')'
op|']'
newline|'\n'
name|'if'
name|'POMtemplates'
op|'['
string|"'solr'"
op|']'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'    '
name|'raise'
name|'RuntimeError'
op|'('
string|"'No Solr POMs found at %s'"
op|'%'
name|'solrSrcUnpackPath'
op|')'
newline|'\n'
dedent|''
name|'POMtemplates'
op|'['
string|"'grandfather'"
op|']'
op|'='
op|'['
name|'p'
name|'for'
name|'p'
name|'in'
name|'allPOMtemplates'
name|'if'
string|"'/maven/pom.xml.template'"
name|'in'
name|'p'
op|']'
newline|'\n'
name|'if'
name|'len'
op|'('
name|'POMtemplates'
op|'['
string|"'grandfather'"
op|']'
op|')'
op|'=='
number|'0'
op|':'
newline|'\n'
indent|'    '
name|'raise'
name|'RuntimeError'
op|'('
string|"'No Lucene/Solr grandfather POM found at %s'"
op|'%'
name|'solrSrcUnpackPath'
op|')'
newline|'\n'
nl|'\n'
DECL|function|crawl
dedent|''
dedent|''
name|'def'
name|'crawl'
op|'('
name|'downloadedFiles'
op|','
name|'urlString'
op|','
name|'targetDir'
op|','
name|'exclusions'
op|'='
name|'set'
op|'('
op|')'
op|')'
op|':'
newline|'\n'
indent|'  '
name|'for'
name|'text'
op|','
name|'subURL'
name|'in'
name|'getDirEntries'
op|'('
name|'urlString'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'if'
name|'text'
name|'not'
name|'in'
name|'exclusions'
op|':'
newline|'\n'
indent|'      '
name|'path'
op|'='
name|'os'
op|'.'
name|'path'
op|'.'
name|'join'
op|'('
name|'targetDir'
op|','
name|'text'
op|')'
newline|'\n'
name|'if'
name|'text'
op|'.'
name|'endswith'
op|'('
string|"'/'"
op|')'
op|':'
newline|'\n'
indent|'        '
name|'if'
name|'not'
name|'os'
op|'.'
name|'path'
op|'.'
name|'exists'
op|'('
name|'path'
op|')'
op|':'
newline|'\n'
indent|'          '
name|'os'
op|'.'
name|'makedirs'
op|'('
name|'path'
op|')'
newline|'\n'
dedent|''
name|'crawl'
op|'('
name|'downloadedFiles'
op|','
name|'subURL'
op|','
name|'path'
op|','
name|'exclusions'
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'        '
name|'if'
name|'not'
name|'os'
op|'.'
name|'path'
op|'.'
name|'exists'
op|'('
name|'path'
op|')'
name|'or'
name|'FORCE_CLEAN'
op|':'
newline|'\n'
indent|'          '
name|'download'
op|'('
name|'text'
op|','
name|'subURL'
op|','
name|'targetDir'
op|','
name|'quiet'
op|'='
name|'True'
op|')'
newline|'\n'
dedent|''
name|'downloadedFiles'
op|'.'
name|'append'
op|'('
name|'path'
op|')'
newline|'\n'
name|'sys'
op|'.'
name|'stdout'
op|'.'
name|'write'
op|'('
string|"'.'"
op|')'
newline|'\n'
nl|'\n'
DECL|function|make_java_config
dedent|''
dedent|''
dedent|''
dedent|''
name|'def'
name|'make_java_config'
op|'('
name|'parser'
op|','
name|'java8_home'
op|')'
op|':'
newline|'\n'
DECL|function|_make_runner
indent|'  '
name|'def'
name|'_make_runner'
op|'('
name|'java_home'
op|','
name|'version'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'print'
op|'('
string|"'Java %s JAVA_HOME=%s'"
op|'%'
op|'('
name|'version'
op|','
name|'java_home'
op|')'
op|')'
newline|'\n'
name|'if'
name|'cygwin'
op|':'
newline|'\n'
indent|'      '
name|'java_home'
op|'='
name|'subprocess'
op|'.'
name|'check_output'
op|'('
string|'\'cygpath -u "%s"\''
op|'%'
name|'java_home'
op|')'
op|'.'
name|'read'
op|'('
op|')'
op|'.'
name|'decode'
op|'('
string|"'utf-8'"
op|')'
op|'.'
name|'strip'
op|'('
op|')'
newline|'\n'
dedent|''
name|'cmd_prefix'
op|'='
string|'\'export JAVA_HOME="%s" PATH="%s/bin:$PATH" JAVACMD="%s/bin/java"\''
op|'%'
op|'('
name|'java_home'
op|','
name|'java_home'
op|','
name|'java_home'
op|')'
newline|'\n'
name|'s'
op|'='
name|'subprocess'
op|'.'
name|'check_output'
op|'('
string|"'%s; java -version'"
op|'%'
name|'cmd_prefix'
op|','
nl|'\n'
name|'shell'
op|'='
name|'True'
op|','
name|'stderr'
op|'='
name|'subprocess'
op|'.'
name|'STDOUT'
op|')'
op|'.'
name|'decode'
op|'('
string|"'utf-8'"
op|')'
newline|'\n'
name|'if'
name|'s'
op|'.'
name|'find'
op|'('
string|'\' version "%s.\''
op|'%'
name|'version'
op|')'
op|'=='
op|'-'
number|'1'
op|':'
newline|'\n'
indent|'      '
name|'parser'
op|'.'
name|'error'
op|'('
string|"'got wrong version for java %s:\\n%s'"
op|'%'
op|'('
name|'version'
op|','
name|'s'
op|')'
op|')'
newline|'\n'
DECL|function|run_java
dedent|''
name|'def'
name|'run_java'
op|'('
name|'cmd'
op|','
name|'logfile'
op|')'
op|':'
newline|'\n'
indent|'      '
name|'run'
op|'('
string|"'%s; %s'"
op|'%'
op|'('
name|'cmd_prefix'
op|','
name|'cmd'
op|')'
op|','
name|'logfile'
op|')'
newline|'\n'
dedent|''
name|'return'
name|'run_java'
newline|'\n'
dedent|''
name|'java8_home'
op|'='
name|'os'
op|'.'
name|'environ'
op|'.'
name|'get'
op|'('
string|"'JAVA_HOME'"
op|')'
newline|'\n'
name|'if'
name|'java8_home'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'    '
name|'parser'
op|'.'
name|'error'
op|'('
string|"'JAVA_HOME must be set'"
op|')'
newline|'\n'
dedent|''
name|'run_java8'
op|'='
name|'_make_runner'
op|'('
name|'java8_home'
op|','
string|"'1.8'"
op|')'
newline|'\n'
nl|'\n'
name|'jc'
op|'='
name|'namedtuple'
op|'('
string|"'JavaConfig'"
op|','
string|"'run_java8 java8_home'"
op|')'
newline|'\n'
name|'return'
name|'jc'
op|'('
name|'run_java8'
op|','
name|'java8_home'
op|')'
newline|'\n'
nl|'\n'
DECL|variable|version_re
dedent|''
name|'version_re'
op|'='
name|'re'
op|'.'
name|'compile'
op|'('
string|"r'(\\d+\\.\\d+\\.\\d+(-ALPHA|-BETA)?)'"
op|')'
newline|'\n'
DECL|variable|revision_re
name|'revision_re'
op|'='
name|'re'
op|'.'
name|'compile'
op|'('
string|"r'rev([a-f\\d]+)'"
op|')'
newline|'\n'
DECL|function|parse_config
name|'def'
name|'parse_config'
op|'('
op|')'
op|':'
newline|'\n'
indent|'  '
name|'epilogue'
op|'='
name|'textwrap'
op|'.'
name|'dedent'
op|'('
string|"'''\n    Example usage:\n    python3 -u dev-tools/scripts/smokeTestRelease.py https://dist.apache.org/repos/dist/dev/lucene/lucene-solr-6.0.1-RC2-revc7510a0...\n  '''"
op|')'
newline|'\n'
name|'description'
op|'='
string|"'Utility to test a release.'"
newline|'\n'
name|'parser'
op|'='
name|'argparse'
op|'.'
name|'ArgumentParser'
op|'('
name|'description'
op|'='
name|'description'
op|','
name|'epilog'
op|'='
name|'epilogue'
op|','
nl|'\n'
name|'formatter_class'
op|'='
name|'argparse'
op|'.'
name|'RawDescriptionHelpFormatter'
op|')'
newline|'\n'
name|'parser'
op|'.'
name|'add_argument'
op|'('
string|"'--tmp-dir'"
op|','
name|'metavar'
op|'='
string|"'PATH'"
op|','
nl|'\n'
name|'help'
op|'='
string|"'Temporary directory to test inside, defaults to /tmp/smoke_lucene_$version_$revision'"
op|')'
newline|'\n'
name|'parser'
op|'.'
name|'add_argument'
op|'('
string|"'--not-signed'"
op|','
name|'dest'
op|'='
string|"'is_signed'"
op|','
name|'action'
op|'='
string|"'store_false'"
op|','
name|'default'
op|'='
name|'True'
op|','
nl|'\n'
name|'help'
op|'='
string|"'Indicates the release is not signed'"
op|')'
newline|'\n'
name|'parser'
op|'.'
name|'add_argument'
op|'('
string|"'--revision'"
op|','
nl|'\n'
name|'help'
op|'='
string|"'GIT revision number that release was built with, defaults to that in URL'"
op|')'
newline|'\n'
name|'parser'
op|'.'
name|'add_argument'
op|'('
string|"'--version'"
op|','
name|'metavar'
op|'='
string|"'X.Y.Z(-ALPHA|-BETA)?'"
op|','
nl|'\n'
name|'help'
op|'='
string|"'Version of the release, defaults to that in URL'"
op|')'
newline|'\n'
name|'parser'
op|'.'
name|'add_argument'
op|'('
string|"'--test-java8'"
op|','
name|'metavar'
op|'='
string|"'JAVA8_HOME'"
op|','
nl|'\n'
name|'help'
op|'='
string|"'Path to Java8 home directory, to run tests with if specified'"
op|')'
newline|'\n'
name|'parser'
op|'.'
name|'add_argument'
op|'('
string|"'url'"
op|','
name|'help'
op|'='
string|"'Url pointing to release to test'"
op|')'
newline|'\n'
name|'parser'
op|'.'
name|'add_argument'
op|'('
string|"'test_args'"
op|','
name|'nargs'
op|'='
name|'argparse'
op|'.'
name|'REMAINDER'
op|','
nl|'\n'
name|'help'
op|'='
string|"'Arguments to pass to ant for testing, e.g. -Dwhat=ever.'"
op|')'
newline|'\n'
name|'c'
op|'='
name|'parser'
op|'.'
name|'parse_args'
op|'('
op|')'
newline|'\n'
nl|'\n'
name|'if'
name|'c'
op|'.'
name|'version'
name|'is'
name|'not'
name|'None'
op|':'
newline|'\n'
indent|'    '
name|'if'
name|'not'
name|'version_re'
op|'.'
name|'match'
op|'('
name|'c'
op|'.'
name|'version'
op|')'
op|':'
newline|'\n'
indent|'      '
name|'parser'
op|'.'
name|'error'
op|'('
string|'\'version "%s" does not match format X.Y.Z[-ALPHA|-BETA]\''
op|'%'
name|'c'
op|'.'
name|'version'
op|')'
newline|'\n'
dedent|''
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'    '
name|'version_match'
op|'='
name|'version_re'
op|'.'
name|'search'
op|'('
name|'c'
op|'.'
name|'url'
op|')'
newline|'\n'
name|'if'
name|'version_match'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'      '
name|'parser'
op|'.'
name|'error'
op|'('
string|"'Could not find version in URL'"
op|')'
newline|'\n'
dedent|''
name|'c'
op|'.'
name|'version'
op|'='
name|'version_match'
op|'.'
name|'group'
op|'('
number|'1'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'if'
name|'c'
op|'.'
name|'revision'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'    '
name|'revision_match'
op|'='
name|'revision_re'
op|'.'
name|'search'
op|'('
name|'c'
op|'.'
name|'url'
op|')'
newline|'\n'
name|'if'
name|'revision_match'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'      '
name|'parser'
op|'.'
name|'error'
op|'('
string|"'Could not find revision in URL'"
op|')'
newline|'\n'
dedent|''
name|'c'
op|'.'
name|'revision'
op|'='
name|'revision_match'
op|'.'
name|'group'
op|'('
number|'1'
op|')'
newline|'\n'
name|'print'
op|'('
string|"'Revision: %s'"
op|'%'
name|'c'
op|'.'
name|'revision'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'c'
op|'.'
name|'java'
op|'='
name|'make_java_config'
op|'('
name|'parser'
op|','
name|'c'
op|'.'
name|'test_java8'
op|')'
newline|'\n'
nl|'\n'
name|'if'
name|'c'
op|'.'
name|'tmp_dir'
op|':'
newline|'\n'
indent|'    '
name|'c'
op|'.'
name|'tmp_dir'
op|'='
name|'os'
op|'.'
name|'path'
op|'.'
name|'abspath'
op|'('
name|'c'
op|'.'
name|'tmp_dir'
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'    '
name|'tmp'
op|'='
string|"'/tmp/smoke_lucene_%s_%s'"
op|'%'
op|'('
name|'c'
op|'.'
name|'version'
op|','
name|'c'
op|'.'
name|'revision'
op|')'
newline|'\n'
name|'c'
op|'.'
name|'tmp_dir'
op|'='
name|'tmp'
newline|'\n'
name|'i'
op|'='
number|'1'
newline|'\n'
name|'while'
name|'os'
op|'.'
name|'path'
op|'.'
name|'exists'
op|'('
name|'c'
op|'.'
name|'tmp_dir'
op|')'
op|':'
newline|'\n'
indent|'      '
name|'c'
op|'.'
name|'tmp_dir'
op|'='
name|'tmp'
op|'+'
string|"'_%d'"
op|'%'
name|'i'
newline|'\n'
name|'i'
op|'+='
number|'1'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'return'
name|'c'
newline|'\n'
nl|'\n'
DECL|variable|reVersion1
dedent|''
name|'reVersion1'
op|'='
name|'re'
op|'.'
name|'compile'
op|'('
string|"r'\\>(\\d+)\\.(\\d+)\\.(\\d+)(-alpha|-beta)?/\\<'"
op|','
name|'re'
op|'.'
name|'IGNORECASE'
op|')'
newline|'\n'
DECL|variable|reVersion2
name|'reVersion2'
op|'='
name|'re'
op|'.'
name|'compile'
op|'('
string|"r'-(\\d+)\\.(\\d+)\\.(\\d+)(-alpha|-beta)?\\.'"
op|','
name|'re'
op|'.'
name|'IGNORECASE'
op|')'
newline|'\n'
nl|'\n'
DECL|function|getAllLuceneReleases
name|'def'
name|'getAllLuceneReleases'
op|'('
op|')'
op|':'
newline|'\n'
indent|'  '
name|'s'
op|'='
name|'load'
op|'('
string|"'https://archive.apache.org/dist/lucene/java'"
op|')'
newline|'\n'
nl|'\n'
name|'releases'
op|'='
name|'set'
op|'('
op|')'
newline|'\n'
name|'for'
name|'r'
name|'in'
name|'reVersion1'
op|','
name|'reVersion2'
op|':'
newline|'\n'
indent|'    '
name|'for'
name|'tup'
name|'in'
name|'r'
op|'.'
name|'findall'
op|'('
name|'s'
op|')'
op|':'
newline|'\n'
indent|'      '
name|'if'
name|'tup'
op|'['
op|'-'
number|'1'
op|']'
op|'.'
name|'lower'
op|'('
op|')'
op|'=='
string|"'-alpha'"
op|':'
newline|'\n'
indent|'        '
name|'tup'
op|'='
name|'tup'
op|'['
op|':'
number|'3'
op|']'
op|'+'
op|'('
string|"'0'"
op|','
op|')'
newline|'\n'
dedent|''
name|'elif'
name|'tup'
op|'['
op|'-'
number|'1'
op|']'
op|'.'
name|'lower'
op|'('
op|')'
op|'=='
string|"'-beta'"
op|':'
newline|'\n'
indent|'        '
name|'tup'
op|'='
name|'tup'
op|'['
op|':'
number|'3'
op|']'
op|'+'
op|'('
string|"'1'"
op|','
op|')'
newline|'\n'
dedent|''
name|'elif'
name|'tup'
op|'['
op|'-'
number|'1'
op|']'
op|'=='
string|"''"
op|':'
newline|'\n'
indent|'        '
name|'tup'
op|'='
name|'tup'
op|'['
op|':'
number|'3'
op|']'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'RuntimeError'
op|'('
string|"'failed to parse version: %s'"
op|'%'
name|'tup'
op|'['
op|'-'
number|'1'
op|']'
op|')'
newline|'\n'
dedent|''
name|'releases'
op|'.'
name|'add'
op|'('
name|'tuple'
op|'('
name|'int'
op|'('
name|'x'
op|')'
name|'for'
name|'x'
name|'in'
name|'tup'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'l'
op|'='
name|'list'
op|'('
name|'releases'
op|')'
newline|'\n'
name|'l'
op|'.'
name|'sort'
op|'('
op|')'
newline|'\n'
name|'return'
name|'l'
newline|'\n'
nl|'\n'
DECL|function|confirmAllReleasesAreTestedForBackCompat
dedent|''
name|'def'
name|'confirmAllReleasesAreTestedForBackCompat'
op|'('
name|'smokeVersion'
op|','
name|'unpackPath'
op|')'
op|':'
newline|'\n'
nl|'\n'
indent|'  '
name|'print'
op|'('
string|"'    find all past Lucene releases...'"
op|')'
newline|'\n'
name|'allReleases'
op|'='
name|'getAllLuceneReleases'
op|'('
op|')'
newline|'\n'
comment|'#for tup in allReleases:'
nl|'\n'
comment|"#  print('  %s' % '.'.join(str(x) for x in tup))"
nl|'\n'
nl|'\n'
name|'testedIndices'
op|'='
name|'set'
op|'('
op|')'
newline|'\n'
nl|'\n'
name|'os'
op|'.'
name|'chdir'
op|'('
name|'unpackPath'
op|')'
newline|'\n'
nl|'\n'
name|'print'
op|'('
string|"'    run TestBackwardsCompatibility..'"
op|')'
newline|'\n'
name|'command'
op|'='
string|"'ant test -Dtestcase=TestBackwardsCompatibility -Dtests.verbose=true'"
newline|'\n'
name|'p'
op|'='
name|'subprocess'
op|'.'
name|'Popen'
op|'('
name|'command'
op|','
name|'shell'
op|'='
name|'True'
op|','
name|'stdout'
op|'='
name|'subprocess'
op|'.'
name|'PIPE'
op|','
name|'stderr'
op|'='
name|'subprocess'
op|'.'
name|'STDOUT'
op|')'
newline|'\n'
name|'stdout'
op|','
name|'stderr'
op|'='
name|'p'
op|'.'
name|'communicate'
op|'('
op|')'
newline|'\n'
name|'if'
name|'p'
op|'.'
name|'returncode'
name|'is'
name|'not'
number|'0'
op|':'
newline|'\n'
comment|'# Not good: the test failed!'
nl|'\n'
indent|'    '
name|'raise'
name|'RuntimeError'
op|'('
string|"'%s failed:\\n%s'"
op|'%'
op|'('
name|'command'
op|','
name|'stdout'
op|')'
op|')'
newline|'\n'
dedent|''
name|'stdout'
op|'='
name|'stdout'
op|'.'
name|'decode'
op|'('
string|"'utf-8'"
op|')'
newline|'\n'
nl|'\n'
name|'if'
name|'stderr'
name|'is'
name|'not'
name|'None'
op|':'
newline|'\n'
comment|'# Should not happen since we redirected stderr to stdout:'
nl|'\n'
indent|'    '
name|'raise'
name|'RuntimeError'
op|'('
string|"'stderr non-empty'"
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'reIndexName'
op|'='
name|'re'
op|'.'
name|'compile'
op|'('
string|"r'TEST: index[\\s*=\\s*](.*?)(-cfs|-nocfs)$'"
op|','
name|'re'
op|'.'
name|'MULTILINE'
op|')'
newline|'\n'
name|'for'
name|'name'
op|','
name|'cfsPart'
name|'in'
name|'reIndexName'
op|'.'
name|'findall'
op|'('
name|'stdout'
op|')'
op|':'
newline|'\n'
comment|"# Fragile: decode the inconsistent naming schemes we've used in TestBWC's indices:"
nl|'\n'
comment|"#print('parse name %s' % name)"
nl|'\n'
indent|'    '
name|'tup'
op|'='
name|'tuple'
op|'('
name|'name'
op|'.'
name|'split'
op|'('
string|"'.'"
op|')'
op|')'
newline|'\n'
name|'if'
name|'len'
op|'('
name|'tup'
op|')'
op|'=='
number|'3'
op|':'
newline|'\n'
comment|'# ok'
nl|'\n'
indent|'      '
name|'tup'
op|'='
name|'tuple'
op|'('
name|'int'
op|'('
name|'x'
op|')'
name|'for'
name|'x'
name|'in'
name|'tup'
op|')'
newline|'\n'
dedent|''
name|'elif'
name|'tup'
op|'=='
op|'('
string|"'4'"
op|','
string|"'0'"
op|','
string|"'0'"
op|','
string|"'1'"
op|')'
op|':'
newline|'\n'
comment|'# CONFUSING: this is the 4.0.0-alpha index??'
nl|'\n'
indent|'      '
name|'tup'
op|'='
number|'4'
op|','
number|'0'
op|','
number|'0'
op|','
number|'0'
newline|'\n'
dedent|''
name|'elif'
name|'tup'
op|'=='
op|'('
string|"'4'"
op|','
string|"'0'"
op|','
string|"'0'"
op|','
string|"'2'"
op|')'
op|':'
newline|'\n'
comment|'# CONFUSING: this is the 4.0.0-beta index??'
nl|'\n'
indent|'      '
name|'tup'
op|'='
number|'4'
op|','
number|'0'
op|','
number|'0'
op|','
number|'1'
newline|'\n'
dedent|''
name|'elif'
name|'name'
op|'=='
string|"'5x-with-4x-segments'"
op|':'
newline|'\n'
comment|'# Mixed version test case; ignore it for our purposes because we only'
nl|'\n'
comment|'# tally up the "tests single Lucene version" indices'
nl|'\n'
indent|'      '
name|'continue'
newline|'\n'
dedent|''
name|'elif'
name|'name'
op|'=='
string|"'5.0.0.singlesegment'"
op|':'
newline|'\n'
indent|'      '
name|'tup'
op|'='
number|'5'
op|','
number|'0'
op|','
number|'0'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'      '
name|'raise'
name|'RuntimeError'
op|'('
string|"'could not parse version %s'"
op|'%'
name|'name'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'testedIndices'
op|'.'
name|'add'
op|'('
name|'tup'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'l'
op|'='
name|'list'
op|'('
name|'testedIndices'
op|')'
newline|'\n'
name|'l'
op|'.'
name|'sort'
op|'('
op|')'
newline|'\n'
name|'if'
name|'False'
op|':'
newline|'\n'
indent|'    '
name|'for'
name|'release'
name|'in'
name|'l'
op|':'
newline|'\n'
indent|'      '
name|'print'
op|'('
string|"'  %s'"
op|'%'
string|"'.'"
op|'.'
name|'join'
op|'('
name|'str'
op|'('
name|'x'
op|')'
name|'for'
name|'x'
name|'in'
name|'release'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'allReleases'
op|'='
name|'set'
op|'('
name|'allReleases'
op|')'
newline|'\n'
nl|'\n'
name|'for'
name|'x'
name|'in'
name|'testedIndices'
op|':'
newline|'\n'
indent|'    '
name|'if'
name|'x'
name|'not'
name|'in'
name|'allReleases'
op|':'
newline|'\n'
comment|"# Curious: we test 1.9.0 index but it's not in the releases (I think it was pulled because of nasty bug?)"
nl|'\n'
indent|'      '
name|'if'
name|'x'
op|'!='
op|'('
number|'1'
op|','
number|'9'
op|','
number|'0'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'RuntimeError'
op|'('
string|"'tested version=%s but it was not released?'"
op|'%'
string|"'.'"
op|'.'
name|'join'
op|'('
name|'str'
op|'('
name|'y'
op|')'
name|'for'
name|'y'
name|'in'
name|'x'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
dedent|''
name|'notTested'
op|'='
op|'['
op|']'
newline|'\n'
name|'for'
name|'x'
name|'in'
name|'allReleases'
op|':'
newline|'\n'
indent|'    '
name|'if'
name|'x'
name|'not'
name|'in'
name|'testedIndices'
op|':'
newline|'\n'
indent|'      '
name|'releaseVersion'
op|'='
string|"'.'"
op|'.'
name|'join'
op|'('
name|'str'
op|'('
name|'y'
op|')'
name|'for'
name|'y'
name|'in'
name|'x'
op|')'
newline|'\n'
name|'if'
name|'releaseVersion'
name|'in'
op|'('
string|"'1.4.3'"
op|','
string|"'1.9.1'"
op|','
string|"'2.3.1'"
op|','
string|"'2.3.2'"
op|')'
op|':'
newline|'\n'
comment|'# Exempt the dark ages indices'
nl|'\n'
indent|'        '
name|'continue'
newline|'\n'
dedent|''
name|'if'
name|'x'
op|'>='
name|'tuple'
op|'('
name|'int'
op|'('
name|'y'
op|')'
name|'for'
name|'y'
name|'in'
name|'smokeVersion'
op|'.'
name|'split'
op|'('
string|"'.'"
op|')'
op|')'
op|':'
newline|'\n'
comment|'# Exempt versions not less than the one being smoke tested'
nl|'\n'
indent|'        '
name|'print'
op|'('
string|"'      Backcompat testing not required for release %s because it\\'s not less than %s'"
nl|'\n'
op|'%'
op|'('
name|'releaseVersion'
op|','
name|'smokeVersion'
op|')'
op|')'
newline|'\n'
name|'continue'
newline|'\n'
dedent|''
name|'notTested'
op|'.'
name|'append'
op|'('
name|'x'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'if'
name|'len'
op|'('
name|'notTested'
op|')'
op|'>'
number|'0'
op|':'
newline|'\n'
indent|'    '
name|'notTested'
op|'.'
name|'sort'
op|'('
op|')'
newline|'\n'
name|'print'
op|'('
string|"'Releases that don\\'t seem to be tested:'"
op|')'
newline|'\n'
name|'failed'
op|'='
name|'True'
newline|'\n'
name|'for'
name|'x'
name|'in'
name|'notTested'
op|':'
newline|'\n'
indent|'      '
name|'print'
op|'('
string|"'  %s'"
op|'%'
string|"'.'"
op|'.'
name|'join'
op|'('
name|'str'
op|'('
name|'y'
op|')'
name|'for'
name|'y'
name|'in'
name|'x'
op|')'
op|')'
newline|'\n'
dedent|''
name|'raise'
name|'RuntimeError'
op|'('
string|"'some releases are not tested by TestBackwardsCompatibility?'"
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'    '
name|'print'
op|'('
string|"'    success!'"
op|')'
newline|'\n'
nl|'\n'
DECL|function|getScriptVersion
dedent|''
dedent|''
name|'def'
name|'getScriptVersion'
op|'('
op|')'
op|':'
newline|'\n'
indent|'  '
name|'topLevelDir'
op|'='
string|"'../..'"
comment|'# Assumption: this script is in dev-tools/scripts/ of a checkout'
newline|'\n'
name|'m'
op|'='
name|'re'
op|'.'
name|'compile'
op|'('
string|"r'(.*)/'"
op|')'
op|'.'
name|'match'
op|'('
name|'sys'
op|'.'
name|'argv'
op|'['
number|'0'
op|']'
op|')'
comment|"# Get this script's directory"
newline|'\n'
name|'if'
name|'m'
name|'is'
name|'not'
name|'None'
name|'and'
name|'m'
op|'.'
name|'group'
op|'('
number|'1'
op|')'
op|'!='
string|"'.'"
op|':'
newline|'\n'
indent|'    '
name|'origCwd'
op|'='
name|'os'
op|'.'
name|'getcwd'
op|'('
op|')'
newline|'\n'
name|'os'
op|'.'
name|'chdir'
op|'('
name|'m'
op|'.'
name|'group'
op|'('
number|'1'
op|')'
op|')'
newline|'\n'
name|'os'
op|'.'
name|'chdir'
op|'('
string|"'../..'"
op|')'
newline|'\n'
name|'topLevelDir'
op|'='
name|'os'
op|'.'
name|'getcwd'
op|'('
op|')'
newline|'\n'
name|'os'
op|'.'
name|'chdir'
op|'('
name|'origCwd'
op|')'
newline|'\n'
dedent|''
name|'reBaseVersion'
op|'='
name|'re'
op|'.'
name|'compile'
op|'('
string|"r'version\\.base\\s*=\\s*(\\d+\\.\\d+)'"
op|')'
newline|'\n'
name|'return'
name|'reBaseVersion'
op|'.'
name|'search'
op|'('
name|'open'
op|'('
string|"'%s/lucene/version.properties'"
op|'%'
name|'topLevelDir'
op|')'
op|'.'
name|'read'
op|'('
op|')'
op|')'
op|'.'
name|'group'
op|'('
number|'1'
op|')'
newline|'\n'
nl|'\n'
DECL|function|main
dedent|''
name|'def'
name|'main'
op|'('
op|')'
op|':'
newline|'\n'
indent|'  '
name|'c'
op|'='
name|'parse_config'
op|'('
op|')'
newline|'\n'
nl|'\n'
name|'scriptVersion'
op|'='
name|'getScriptVersion'
op|'('
op|')'
newline|'\n'
name|'if'
name|'not'
name|'c'
op|'.'
name|'version'
op|'.'
name|'startswith'
op|'('
name|'scriptVersion'
op|'+'
string|"'.'"
op|')'
op|':'
newline|'\n'
indent|'    '
name|'raise'
name|'RuntimeError'
op|'('
string|"'smokeTestRelease.py for %s.X is incompatible with a %s release.'"
op|'%'
op|'('
name|'scriptVersion'
op|','
name|'c'
op|'.'
name|'version'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'print'
op|'('
string|"'NOTE: output encoding is %s'"
op|'%'
name|'sys'
op|'.'
name|'stdout'
op|'.'
name|'encoding'
op|')'
newline|'\n'
name|'smokeTest'
op|'('
name|'c'
op|'.'
name|'java'
op|','
name|'c'
op|'.'
name|'url'
op|','
name|'c'
op|'.'
name|'revision'
op|','
name|'c'
op|'.'
name|'version'
op|','
name|'c'
op|'.'
name|'tmp_dir'
op|','
name|'c'
op|'.'
name|'is_signed'
op|','
string|"' '"
op|'.'
name|'join'
op|'('
name|'c'
op|'.'
name|'test_args'
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|function|smokeTest
dedent|''
name|'def'
name|'smokeTest'
op|'('
name|'java'
op|','
name|'baseURL'
op|','
name|'gitRevision'
op|','
name|'version'
op|','
name|'tmpDir'
op|','
name|'isSigned'
op|','
name|'testArgs'
op|')'
op|':'
newline|'\n'
nl|'\n'
indent|'  '
name|'startTime'
op|'='
name|'datetime'
op|'.'
name|'datetime'
op|'.'
name|'now'
op|'('
op|')'
newline|'\n'
nl|'\n'
name|'if'
name|'FORCE_CLEAN'
op|':'
newline|'\n'
indent|'    '
name|'if'
name|'os'
op|'.'
name|'path'
op|'.'
name|'exists'
op|'('
name|'tmpDir'
op|')'
op|':'
newline|'\n'
indent|'      '
name|'raise'
name|'RuntimeError'
op|'('
string|"'temp dir %s exists; please remove first'"
op|'%'
name|'tmpDir'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'if'
name|'not'
name|'os'
op|'.'
name|'path'
op|'.'
name|'exists'
op|'('
name|'tmpDir'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'os'
op|'.'
name|'makedirs'
op|'('
name|'tmpDir'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'lucenePath'
op|'='
name|'None'
newline|'\n'
name|'solrPath'
op|'='
name|'None'
newline|'\n'
name|'print'
op|'('
op|')'
newline|'\n'
name|'print'
op|'('
string|'\'Load release URL "%s"...\''
op|'%'
name|'baseURL'
op|')'
newline|'\n'
name|'newBaseURL'
op|'='
name|'unshortenURL'
op|'('
name|'baseURL'
op|')'
newline|'\n'
name|'if'
name|'newBaseURL'
op|'!='
name|'baseURL'
op|':'
newline|'\n'
indent|'    '
name|'print'
op|'('
string|"'  unshortened: %s'"
op|'%'
name|'newBaseURL'
op|')'
newline|'\n'
name|'baseURL'
op|'='
name|'newBaseURL'
newline|'\n'
nl|'\n'
dedent|''
name|'for'
name|'text'
op|','
name|'subURL'
name|'in'
name|'getDirEntries'
op|'('
name|'baseURL'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'if'
name|'text'
op|'.'
name|'lower'
op|'('
op|')'
op|'.'
name|'find'
op|'('
string|"'lucene'"
op|')'
op|'!='
op|'-'
number|'1'
op|':'
newline|'\n'
indent|'      '
name|'lucenePath'
op|'='
name|'subURL'
newline|'\n'
dedent|''
name|'elif'
name|'text'
op|'.'
name|'lower'
op|'('
op|')'
op|'.'
name|'find'
op|'('
string|"'solr'"
op|')'
op|'!='
op|'-'
number|'1'
op|':'
newline|'\n'
indent|'      '
name|'solrPath'
op|'='
name|'subURL'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'if'
name|'lucenePath'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'    '
name|'raise'
name|'RuntimeError'
op|'('
string|"'could not find lucene subdir'"
op|')'
newline|'\n'
dedent|''
name|'if'
name|'solrPath'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'    '
name|'raise'
name|'RuntimeError'
op|'('
string|"'could not find solr subdir'"
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'print'
op|'('
op|')'
newline|'\n'
name|'print'
op|'('
string|"'Test Lucene...'"
op|')'
newline|'\n'
name|'checkSigs'
op|'('
string|"'lucene'"
op|','
name|'lucenePath'
op|','
name|'version'
op|','
name|'tmpDir'
op|','
name|'isSigned'
op|')'
newline|'\n'
name|'for'
name|'artifact'
name|'in'
op|'('
string|"'lucene-%s.tgz'"
op|'%'
name|'version'
op|','
string|"'lucene-%s.zip'"
op|'%'
name|'version'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'unpackAndVerify'
op|'('
name|'java'
op|','
string|"'lucene'"
op|','
name|'tmpDir'
op|','
name|'artifact'
op|','
name|'gitRevision'
op|','
name|'version'
op|','
name|'testArgs'
op|','
name|'baseURL'
op|')'
newline|'\n'
dedent|''
name|'unpackAndVerify'
op|'('
name|'java'
op|','
string|"'lucene'"
op|','
name|'tmpDir'
op|','
string|"'lucene-%s-src.tgz'"
op|'%'
name|'version'
op|','
name|'gitRevision'
op|','
name|'version'
op|','
name|'testArgs'
op|','
name|'baseURL'
op|')'
newline|'\n'
nl|'\n'
name|'print'
op|'('
op|')'
newline|'\n'
name|'print'
op|'('
string|"'Test Solr...'"
op|')'
newline|'\n'
name|'checkSigs'
op|'('
string|"'solr'"
op|','
name|'solrPath'
op|','
name|'version'
op|','
name|'tmpDir'
op|','
name|'isSigned'
op|')'
newline|'\n'
name|'for'
name|'artifact'
name|'in'
op|'('
string|"'solr-%s.tgz'"
op|'%'
name|'version'
op|','
string|"'solr-%s.zip'"
op|'%'
name|'version'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'unpackAndVerify'
op|'('
name|'java'
op|','
string|"'solr'"
op|','
name|'tmpDir'
op|','
name|'artifact'
op|','
name|'gitRevision'
op|','
name|'version'
op|','
name|'testArgs'
op|','
name|'baseURL'
op|')'
newline|'\n'
dedent|''
name|'solrSrcUnpackPath'
op|'='
name|'unpackAndVerify'
op|'('
name|'java'
op|','
string|"'solr'"
op|','
name|'tmpDir'
op|','
string|"'solr-%s-src.tgz'"
op|'%'
name|'version'
op|','
nl|'\n'
name|'gitRevision'
op|','
name|'version'
op|','
name|'testArgs'
op|','
name|'baseURL'
op|')'
newline|'\n'
nl|'\n'
name|'print'
op|'('
op|')'
newline|'\n'
name|'print'
op|'('
string|"'Test Maven artifacts for Lucene and Solr...'"
op|')'
newline|'\n'
name|'checkMaven'
op|'('
name|'solrSrcUnpackPath'
op|','
name|'baseURL'
op|','
name|'tmpDir'
op|','
name|'gitRevision'
op|','
name|'version'
op|','
name|'isSigned'
op|')'
newline|'\n'
nl|'\n'
name|'print'
op|'('
string|"'\\nSUCCESS! [%s]\\n'"
op|'%'
op|'('
name|'datetime'
op|'.'
name|'datetime'
op|'.'
name|'now'
op|'('
op|')'
op|'-'
name|'startTime'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'if'
name|'__name__'
op|'=='
string|"'__main__'"
op|':'
newline|'\n'
indent|'  '
name|'try'
op|':'
newline|'\n'
indent|'    '
name|'main'
op|'('
op|')'
newline|'\n'
dedent|''
name|'except'
name|'KeyboardInterrupt'
op|':'
newline|'\n'
indent|'    '
name|'print'
op|'('
string|"'Keyboard interrupt...exiting'"
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
endmarker|''
end_unit
