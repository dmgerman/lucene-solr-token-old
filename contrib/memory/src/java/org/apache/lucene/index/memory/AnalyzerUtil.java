...
return|;
block|}
comment|/**    * Analyzer wrapper that caches all tokens generated by the underlying child analyzer's    * token streams, and delivers those cached tokens on subsequent calls to     *<code>tokenStream(String fieldName, Reader reader)</code>,     * if the fieldName has been seen before, altogether ignoring the Reader parameter.    *<p>    * If Analyzer / TokenFilter chains are expensive in terms of I/O or CPU, such caching can     * help improve performance if the same document is added to multiple Lucene indexes,     * because the text analysis phase need not be performed more than once.    *<p>    * Caveats:     *<ul>    *<li>Caching the tokens of large Lucene documents can lead to out of memory exceptions.</li>     *<li>The Token instances delivered by the underlying child analyzer must be immutable.</li>    *<li>A caching analyzer instance must not be used for more than one document, unless     *<code>clear()</code> is called before each new document.</li>    *</ul>    */
DECL|class|TokenCachingAnalyzer
specifier|public
specifier|static
class|class
name|TokenCachingAnalyzer
extends|extends
name|Analyzer
block|{
DECL|field|child
...
argument_list|()
decl_stmt|;
comment|/**      * Creates and returns a new caching analyzer that wraps the given underlying child analyzer.      *       * @param child      *            the underlying child analyzer      * @return a new caching analyzer      */
DECL|method|TokenCachingAnalyzer
specifier|public
name|TokenCachingAnalyzer
parameter_list|(
name|Analyzer
...
expr_stmt|;
block|}
comment|/**      * Removes all cached data.      */
DECL|method|clear
specifier|public
name|void
name|clear
