...
block|}
block|}
comment|/** Filter duplicate points and coplanar points.    */
...
name|IllegalArgumentException
argument_list|(
literal|"polygon is degenerate: all points are identical"
argument_list|)
throw|;
block|}
...
name|IllegalArgumentException
argument_list|(
literal|"polygon has fewer than three non-identical points"
argument_list|)
throw|;
block|}
comment|// Next step: remove coplanar points and backtracks.  For this, we use a strategy that is similar but we assess whether the points
comment|// are on the same plane, taking the first and last points on the same plane only.
specifier|final
name|List
argument_list|<
name|GeoPoint
argument_list|>
name|nonCoplanarPoints
init|=
operator|new
name|ArrayList
argument_list|<>
...
name|IllegalArgumentException
argument_list|(
literal|"polygon is degenerate: all points are coplanar"
argument_list|)
throw|;
block|}
comment|// Now we can start the process of walking around, removing duplicate points.
name|int
name|currentPlaneIndex
init|=
name|startPlaneIndex
decl_stmt|;
while|while
...
break|break;
block|}
comment|// Check if the next point is off plane
name|nextPlaneIndex
operator|=
name|getLegalIndex
argument_list|(
...
condition|)
block|{
comment|// We will want to add the point at currentPlaneIndex to the list (last on of the series)
break|break;
block|}
block|}
if|if
