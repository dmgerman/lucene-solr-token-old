...
comment|// nocommit: instead use the NumericUtils termsenum stuff to test this directly...
comment|/*       NumericDocValues fci = FieldCache.DEFAULT.getNumerics(SlowCompositeReaderWrapper.wrap(searcher.getIndexReader()), "trieInt", FieldCache.NUMERIC_UTILS_INT_PARSER, false);       int maxDoc = searcher.getIndexReader().maxDoc();       for(int doc=0;doc<maxDoc;doc++) {         long val = fci.get(doc);         assertTrue("value in id bounds", val>= 0&& val< 35);       }              NumericDocValues fcl = FieldCache.DEFAULT.getNumerics(SlowCompositeReaderWrapper.wrap(searcher.getIndexReader()), "trieLong", FieldCache.NUMERIC_UTILS_LONG_PARSER, false);       for(int doc=0;doc<maxDoc;doc++) {         long val = fcl.get(doc);         assertTrue("value in id bounds", val>= 0L&& val< 35L);       }       */
name|reader
operator|.
name|close
argument_list|()
