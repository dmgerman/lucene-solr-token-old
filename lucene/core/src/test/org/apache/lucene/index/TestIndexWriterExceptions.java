...
expr_stmt|;
block|}
DECL|method|testIllegalPositions
specifier|public
name|void
name|testIllegalPositions
parameter_list|()
throws|throws
name|Exception
block|{
...
expr_stmt|;
block|}
DECL|method|testLegalbutVeryLargePositions
specifier|public
name|void
name|testLegalbutVeryLargePositions
parameter_list|()
throws|throws
name|Exception
block|{
...
name|docid
expr_stmt|;
comment|// record that we're updating that document
name|updatingValue
operator|=
name|value
expr_stmt|;
comment|// and its updating value
if|if
condition|(
name|random
argument_list|()
...
expr_stmt|;
block|}
comment|// record that we successfully updated the document. this is
comment|// important when we later assert the value of the DV fields of
comment|// that document - since we update two fields that depend on each
comment|// other, could be that one of the fields successfully updates,
comment|// while the other fails (since we turn on random exceptions).
comment|// while this is supported, it makes the test raise false alarms.
name|updatingDocID
operator|=
operator|-
literal|1
...
condition|)
block|{
comment|// Updating this document did not succeed. Since the fields we assert on
comment|// depend on each other, and the update may have gone through halfway,
comment|// replay the update on both numeric and binary DV fields, so later
comment|// asserts succeed.
name|Term
name|idTerm
init|=
operator|new
