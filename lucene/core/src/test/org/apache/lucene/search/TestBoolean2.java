...
name|searcher
decl_stmt|;
DECL|field|singleSegmentSearcher
specifier|private
specifier|static
name|IndexSearcher
name|singleSegmentSearcher
decl_stmt|;
...
name|littleReader
decl_stmt|;
DECL|field|singleSegmentReader
specifier|private
specifier|static
name|IndexReader
name|singleSegmentReader
decl_stmt|;
comment|/** num of empty docs injected between every doc in the (main) index */
...
decl_stmt|;
comment|/** num "extra" docs containing value in "field2" added to the "big" clone of the index */
...
name|directory
decl_stmt|;
DECL|field|singleSegmentDirectory
specifier|private
specifier|static
name|Directory
...
name|Exception
block|{
comment|// in some runs, test immediate adjacency of matches - in others, force a full bucket gap betwen docs
name|NUM_FILLER_DOCS
operator|=
name|random
argument_list|()
...
argument_list|)
expr_stmt|;
comment|// make a copy of our index using a single segment
name|singleSegmentDirectory
operator|=
operator|new
name|MockDirectoryWrapper
...
argument_list|)
decl_stmt|;
comment|// we need docID order to be preserved:
name|iwc
operator|.
name|setMergePolicy
argument_list|(
...
name|Exception
block|{
comment|// adjust the expected doc numbers according to our filler docs
if|if
condition|(
literal|0
operator|<
...
argument_list|)
expr_stmt|;
comment|// Since we have no deleted docs, we should also be able to verify identical matches&
comment|// scores against an single segment copy of our index
name|collector
operator|=
name|TopScoreDocCollector
operator|.
...
argument_list|)
expr_stmt|;
comment|// sanity check expected num matches in bigSearcher
name|assertEquals
argument_list|(
name|mulFactor
operator|*
...
argument_list|)
expr_stmt|;
comment|// now check 2 diff scorers from the bigSearcher as well
name|collector
operator|=
name|TopScoreDocCollector
operator|.
...
name|scoreDocs
expr_stmt|;
comment|// NOTE: just comparing results, not vetting against expDocNrs
comment|// since we have dups in bigSearcher
name|CheckHits
operator|.
name|checkEqual
argument_list|(
...
comment|// restore
block|}
comment|// check diff (randomized) scorers (from AssertingSearcher) produce the same results
...
argument_list|)
expr_stmt|;
comment|// test diff (randomized) scorers produce the same results on bigSearcher as well
name|collector
operator|=
name|TopFieldCollector
operator|.
