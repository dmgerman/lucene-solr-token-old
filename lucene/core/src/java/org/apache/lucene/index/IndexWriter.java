...
name|IllegalArgumentException
argument_list|(
literal|"applyAllDeletes must be true when writeAllDeletes=true"
argument_list|)
throw|;
block|}
specifier|final
...
condition|)
block|{
comment|// We cannot both open from a commit point and create:
if|if
condition|(
name|mode
operator|==
...
condition|)
block|{
comment|// Init from an existing already opened NRT or non-NRT reader:
if|if
condition|(
name|reader
operator|.
...
name|IllegalArgumentException
argument_list|(
literal|"IndexCommit's reader must have the same directory as the IndexCommit"
argument_list|)
throw|;
block|}
if|if
...
name|IllegalArgumentException
argument_list|(
literal|"IndexCommit's reader must have the same directory passed to IndexWriter"
argument_list|)
throw|;
block|}
if|if
...
condition|)
block|{
comment|// TODO: maybe we could allow this?  It's tricky...
throw|throw
operator|new
name|IllegalArgumentException
argument_list|(
literal|"index must already have an initial commit to open from reader"
argument_list|)
throw|;
block|}
comment|// Must clone because we don't want the incoming NRT reader to "see" any changes this writer now makes:
name|segmentInfos
operator|=
name|reader
operator|.
...
condition|)
block|{
comment|// The old writer better be closed (we have the write lock now!):
assert|assert
name|reader
operator|.
name|writer
...
name|closed
assert|;
comment|// In case the old writer wrote further segments (which we are now dropping),
comment|// update SIS metadata so we remain write-once:
name|segmentInfos
operator|.
...
comment|// NOTE: this is correct even for an NRT reader because we'll pull FieldInfos even for the un-committed segments:
name|globalFieldNumberMap
operator|=
name|getFieldNumberMap
argument_list|()
expr_stmt|;
name|validateIndexSort
argument_list|()
expr_stmt|;
name|config
operator|.
...
argument_list|)
expr_stmt|;
comment|// We incRef all files when we return an NRT reader from IW, so all files must exist even in the NRT case:
assert|assert
name|create
operator|||
name|filesExist
...
condition|)
block|{
comment|// Pre-enroll all segment readers into the reader pool; this is necessary so
comment|// any in-memory NRT live docs are correctly carried over, and so NRT readers
comment|// pulled from this IW share the same segment reader:
name|List
argument_list|<
...
expr_stmt|;
block|}
comment|// We always assume we are carrying over incoming changes when opening from reader:
name|segmentInfos
operator|.
name|changed
argument_list|()
...
name|IllegalArgumentException
argument_list|(
literal|"cannot change previous indexSort="
operator|+
name|segmentIndexSort
operator|+
literal|" (from segment="
operator|+
name|info
operator|+
literal|") to new indexSort="
operator|+
name|indexSort
argument_list|)
throw|;
...
block|}
block|}
comment|// reads latest field infos for the commit
comment|// this is used on IW init and addIndexes(Dir) to create/update the global field map.
comment|// TODO: fix tests abusing this method!
...
condition|)
block|{
comment|// there are updates, we read latest (always outside of CFS)
specifier|final
name|String
name|segmentSuffix
init|=
...
literal|"IW"
argument_list|,
literal|"MMapDirectory.UNMAP_SUPPORTED="
operator|+
name|unmapInfo
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|/** Just like {@link #forceMergeDeletes()}, except you can    *  specify whether the call should block until the    *  operation completes.  This is only meaningful with a    *  {@link MergeScheduler} that is able to run merges in    *  background threads. */
...
condition|)
block|{
comment|// TODO: we could make this smarter, e.g. if the incoming indexSort is congruent with our sort ("starts with") then it's OK
throw|throw
operator|new
name|IllegalArgumentException
argument_list|(
literal|"cannot change index sort from "
operator|+
name|segmentIndexSort
operator|+
literal|" to "
...
argument_list|)
decl_stmt|;
comment|// TODO: unlike merge, on exception we arent sniping any trash cfs files here?
comment|// createCompoundFile tries to cleanup, but it might not always be able to...
try|try
block|{
...
name|IOException
block|{}
comment|/**<p>Expert: prepare for commit.  This does the    *  first phase of 2-phase commit. This method does all    *  steps necessary to commit changes since this writer    *  was opened: flushes pending added and deleted docs,    *  syncs the index files, writes most of next segments_N    *  file.  After calling this you must call either {@link    *  #commit()} to finish the commit, or {@link    *  #rollback()} to revert the commit and undo all changes    *  done since the writer was opened.</p>    *    *<p>You can also just call {@link #commit()} directly    *  without prepareCommit first in which case that method    *  will internally call prepareCommit.    *    * @return The<a href="#sequence_number">sequence number</a>    * of the last operation in the commit.  All sequence numbers&lt;= this value    * will be reflected in the commit, and all others will not.    */
annotation|@
name|Override
...
argument_list|)
expr_stmt|;
comment|// we committed, if anything goes wrong after this, we are screwed and it's a tragedy:
name|commitCompleted
operator|=
literal|true
expr_stmt|;
...
condition|)
block|{
comment|// exc happened in finishCommit: not a tragedy
name|deleter
operator|.
name|decRefWhileHandlingException
argument_list|(
...
literal|"IW"
argument_list|,
literal|"hit exception during finishCommit: "
operator|+
name|t
operator|.
name|getMessage
...
literal|"IW"
argument_list|,
literal|"now apply all deletes for all segments maxDoc="
operator|+
operator|(
name|docWriter
operator|.
...
condition|)
block|{
comment|// if the document was deleted before, it better still be deleted!
assert|assert
name|currentLiveDocs
operator|.
name|get
...
condition|)
block|{
comment|// the document was deleted while we were merging:
if|if
condition|(
name|holder
operator|.
...
argument_list|)
expr_stmt|;
comment|// On IBM J9 JVM this is better than java.version which is just 1.7.0 (no update level):
name|diagnostics
operator|.
name|put
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Hotspot version, e.g. 2.8 for J9:
name|diagnostics
operator|.
name|put
argument_list|(
...
comment|// Let the merge wrap readers
name|List
argument_list|<
name|CodecReader
argument_list|>
...
name|ROOT
argument_list|,
literal|"%.1f sec (%.1f sec stopped, %.1f sec paused) to merge segment [%.2f MB, %.2f MB/sec]"
argument_list|,
name|sec
argument_list|,
name|stoppedSec
...
literal|"IW"
argument_list|,
literal|"skip merging fully deleted segments"
argument_list|)
expr_stmt|;
block|}
block|}
...
condition|)
block|{
comment|// Merge would produce a 0-doc segment, so we do nothing except commit the merge to remove all the 0-doc segments that we "merged":
assert|assert
name|merge
operator|.
name|info
...
name|IOException
block|{
comment|// maybe this check is not needed, but why take the risk?
if|if
condition|(
operator|!
name|directory
...
name|IllegalStateException
argument_list|(
literal|"pass a clean trackingdir for CFS creation"
argument_list|)
throw|;
block|}
if|if
...
name|IOException
block|{
comment|// TODO: this really should be a tragic
name|Collection
argument_list|<
name|String
argument_list|>
...
literal|"IW"
argument_list|,
literal|"incRefDeleter for NRT reader version="
operator|+
name|segmentInfos
operator|.
name|getVersion
...
literal|"IW"
argument_list|,
literal|"decRefDeleter for NRT reader version="
operator|+
name|segmentInfos
operator|.
name|getVersion
