...
condition|)
block|{
comment|// Make sure we only write del docs and updates for a live segment:
assert|assert
name|infoIsLive
argument_list|(
name|info
...
argument_list|)
expr_stmt|;
comment|// we wrote liveDocs and field updates, reopen the reader
name|rld
operator|.
name|reopenReader
argument_list|(
...
block|}
block|}
comment|/**    * Updates a document's NumericDocValue for<code>field</code> to the given    *<code>value</code>. This method can be used to 'unset' a document's value    * by passing {@code null} as the new value. Also, you can only update fields    * that already exist in the index, not add new fields through this method.    *     *<p>    *<b>NOTE:</b> it is currently not allowed to update the value of documents    * in a segment where the field does not exist (even though it may exist in    * other segments). If you try that, you will hit an    * {@link UnsupportedOperationException} when the segment is later flushed    * (following an NRT reader reopen, commit, forceMerge etc.).    *     *<p>    *<b>NOTE</b>: if this method hits an OutOfMemoryError you should immediately    * close the writer. See<a href="#OOME">above</a> for details.    *</p>    *     * @param term    *          the term to identify the document(s) to be updated    * @param field    *          field name of the NumericDocValues field    * @param value    *          new value for the field    * @throws CorruptIndexException    *           if the index is corrupt    * @throws IOException    *           if there is a low-level IO error    */
comment|// TODO (DVU_FIELDINFOS_GEN) remove the paragraph on updating segments without the field not allowed
...
condition|)
block|{
comment|// document isn't deleted, check if it has updates
name|Map
argument_list|<
name|String
argument_list|,
...
argument_list|()
assert|;
comment|// set any updates that came while the segment was merging
if|if
condition|(
operator|!
name|mergedUpdates
...
argument_list|()
operator|+
literal|" new deletes since merge started and "
operator|+
name|mergedDeletes
operator|.
...
argument_list|()
operator|+
literal|" new field updates since merge started"
argument_list|)
expr_stmt|;
block|}
block|}
