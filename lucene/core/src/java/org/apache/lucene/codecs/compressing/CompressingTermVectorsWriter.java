...
end_import
begin_comment
comment|/**  * {@link TermVectorsWriter} for {@link CompressingTermVectorsFormat}.  * @lucene.experimental  */
end_comment
begin_class
DECL|class|CompressingTermVectorsWriter
specifier|public
specifier|final
class|class
...
DECL|method|CompressingTermVectorsWriter
specifier|public
name|CompressingTermVectorsWriter
parameter_list|(
name|Directory
...
condition|)
block|{
comment|// we can only bulk-copy if the matching reader is also a CompressingTermVectorsReader
if|if
condition|(
name|vectorsReader
operator|!=
...
control|)
block|{
comment|// We make sure to move the checksum input in any case, otherwise the final
comment|// integrity check might need to read the whole file a second time
specifier|final
name|long
name|startPointer
init|=
