...
comment|// To compute this default value, I created a rather dense set (0.1% load
comment|// factor, which is close to the worst case regarding both compression and
comment|// speed for this DocIdSet impl since sequences are going to be short) and I
comment|// started with interval=1 and doubled it at each iteration until advance
comment|// became slower
...
block|}
decl_stmt|;
comment|/** Same as {@link #copyOf(DocIdSetIterator, int)} with the default index interval. */
DECL|method|copyOf
specifier|public
specifier|static
name|WAH8DocIdSet
...
return|;
block|}
comment|/** Return a copy of the provided iterator. */
DECL|method|copyOf
specifier|public
specifier|static
name|WAH8DocIdSet
...
expr_stmt|;
block|}
comment|/** Set the index interval. Smaller index intervals improve performance of      *  {@link DocIdSetIterator#advance(int)} but make the {@link DocIdSet}      *  larger. An index interval<code>i</code> makes the index add an overhead      *  which is at most<code>4/i</code>, but likely much less.The default index      *  interval is<code>16</code>, meaning the index has an overhead of at most      *  25%. To disable indexing, you can pass {@link Integer#MAX_VALUE} as an      *  index interval. */
...
block|{
comment|// From the tests I ran, there is no need to expose acceptableOverheadRatio, these packed ints are never the bottleneck
specifier|final
name|PackedInts
operator|.
name|Mutable
...
argument_list|()
return|;
comment|// good estimation of the cost of iterating over all docs
block|}
block|}
comment|/** Return the number of documents in this {@link DocIdSet}. This method    *  runs in linear time but is much faster than counting documents. */
