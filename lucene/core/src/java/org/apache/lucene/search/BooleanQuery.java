...
name|disableCoord
decl_stmt|;
DECL|field|termConjunction
specifier|private
specifier|final
name|boolean
name|termConjunction
decl_stmt|;
...
condition|)
block|{
comment|// specialized scorer for term conjunctions
return|return
name|createConjunctionTermScorer
argument_list|(
name|context
argument_list|,
name|acceptDocs
...
return|;
block|}
DECL|method|createConjunctionTermScorer
specifier|private
name|Scorer
name|createConjunctionTermScorer
parameter_list|(
...
name|IOException
block|{
comment|// TODO: fix scorer API to specify "needsScores" up
comment|// front, so we can do match-only if caller doesn't
comment|// needs scores
specifier|final
name|DocsAndFreqs
index|[]
name|docsAndFreqs
...
condition|)
block|{
comment|// TODO: we could carry over TermState from the
comment|// terms we already seek'd to, to save re-seeking
comment|// to make the match-only scorer, but it's likely
comment|// rare that BQ mixes terms from omitTf and
comment|// non-omitTF fields:
comment|// At least one sub cannot provide freqs; abort
comment|// and fallback to full match-only scorer:
return|return
name|createMatchOnlyConjunctionTermScorer
argument_list|(
name|context
argument_list|,
name|acceptDocs
...
return|;
block|}
DECL|method|createMatchOnlyConjunctionTermScorer
specifier|private
name|Scorer
name|createMatchOnlyConjunctionTermScorer
parameter_list|(
