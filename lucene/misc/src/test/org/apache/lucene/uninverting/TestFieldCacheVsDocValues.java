...
argument_list|()
decl_stmt|;
comment|// compare values for all live docs:
for|for
control|(
name|int
name|i
...
condition|)
block|{
comment|// Don't check deleted docs
continue|continue;
block|}
name|expected
operator|.
...
expr_stmt|;
block|}
comment|// Make sure both have same number of non-deleted values:
name|assertEquals
argument_list|(
name|liveOrdsExpected
operator|.
...
expr_stmt|;
block|}
comment|/** Does termsEnum.next() but then skips over deleted ords. */
DECL|method|next
specifier|private
specifier|static
name|BytesRef
...
return|;
block|}
comment|/** Does termsEnum.seekCeil() but then skips over deleted ords. */
