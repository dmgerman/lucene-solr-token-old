...
argument_list|)
decl_stmt|;
comment|/** Get if a random codec per field should be used */
comment|// only use random per field if no explicit codec is set
DECL|field|TEST_RANDOM_CODEC_PER_FIELD
specifier|static
specifier|final
name|boolean
name|TEST_RANDOM_CODEC_PER_FIELD
init|=
literal|"random"
operator|.
name|equals
...
name|getProperty
argument_list|(
literal|"tests.randomCodecPerField"
argument_list|,
literal|"true"
argument_list|)
argument_list|)
...
name|seed
decl_stmt|;
DECL|field|randomCodecProvider
specifier|protected
specifier|static
name|CodecProvider
name|randomCodecProvider
...
block|}
block|}
comment|/** Returns the codec for the given field uses in this testcase */
DECL|method|getRandomFieldCodec
specifier|public
specifier|static
name|String
...
condition|)
block|{
comment|/*          * We randomly swap in a exclusive PreFlexCodec to provide random test coverage          * for preFlex indexes. In realistic a PreFlex-Environment newer codecs don't occur.              */
name|this
operator|.
name|codecs
operator|=
...
argument_list|)
expr_stmt|;
comment|// register this for read support
block|}
for|for
control|(
name|int
...
condition|)
block|{
comment|// select a codec at random
name|setFieldCodec
argument_list|(
name|name
argument_list|,
...
block|{
return|return
literal|"RandomCodecProvider [perFieldMap="
operator|+
name|perFieldMap
operator|+
literal|"]"
