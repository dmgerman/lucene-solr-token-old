...
parameter_list|)
block|{
comment|// return empty token before first
name|bytes
operator|.
name|length
operator|=
...
parameter_list|)
block|{
comment|// this attribute has no contents to copy
block|}
annotation|@
name|Override
DECL|method|clone
...
parameter_list|()
block|{
comment|// cannot throw CloneNotSupportedException (checked)
throw|throw
operator|new
...
argument_list|)
expr_stmt|;
comment|// we must do this after the super call :(
operator|(
operator|(
name|NumericAttributeFactory
operator|)
...
condition|)
block|{
comment|// reset so the attribute still works after exhausted stream
name|shift
operator|-=
name|precisionStep
expr_stmt|;
...
argument_list|()
expr_stmt|;
comment|// the TermToBytesRefAttribute is directly accessing shift& value.
name|typeAtt
operator|.
name|setType
argument_list|(
