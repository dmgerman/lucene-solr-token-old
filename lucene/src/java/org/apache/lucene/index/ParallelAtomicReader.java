...
end_import
begin_comment
comment|/** An {@link AtomicReader} which reads multiple, parallel indexes.  Each index added  * must have the same number of documents, but typically each contains  * different fields.  Each document contains the union of the fields of all  * documents with the same document number.  When searching, matches for a  * query term are from the first index added that has the field.  *  *<p>This is useful, e.g., with collections that have large fields which  * change rarely and small fields that change more frequently.  The smaller  * fields may be re-indexed in a new index and both indexes may be searched  * together.  *   *<p>To create instances of {@code ParallelAtomicReader}, use the provided  * {@link ParallelAtomicReader.Builder}.  *  *<p><strong>Warning:</strong> It is up to you to make sure all indexes  * are created and modified the same way. For example, if you add  * documents to one index, you need to add the same documents in the  * same order to the other indexes.<em>Failure to do so will result in  * undefined behavior</em>.  */
end_comment
begin_class
...
return|;
block|}
comment|/**    * Builder implementation to create instances of {@link ParallelAtomicReader}.    */
DECL|class|Builder
specifier|public
specifier|static
specifier|final
...
expr_stmt|;
block|}
comment|/** Add an AtomicReader.      * @throws IOException if there is a low-level IO error      */
DECL|method|add
specifier|public
name|Builder
name|add
...
return|;
block|}
comment|/**      * Build the {@link ParallelAtomicReader} instance from the settings.      */
DECL|method|build
specifier|public
name|ParallelAtomicReader
name|build
