...
return|;
block|}
comment|// Used for all SegmentReaders we open
...
argument_list|)
decl_stmt|;
DECL|method|getReaderFinishedListeners
name|Collection
argument_list|<
name|IndexReader
operator|.
...
argument_list|()
decl_stmt|;
comment|/** Forcefully clear changes for the specified segments.  This is called on successful merge. */
DECL|method|clear
...
name|codecs
decl_stmt|;
comment|/**    * Constructs a new IndexWriter per the settings given in<code>conf</code>.    * Note that the passed in {@link IndexWriterConfig} is cloned and thus making    * changes to it after IndexWriter has been instantiated will not affect    * IndexWriter. Additionally, calling {@link #getConfig()} and changing the    * parameters does not affect that IndexWriter instance.    *<p>    *<b>NOTE:</b> by default, {@link IndexWriterConfig#getMaxFieldLength()}    * returns {@link IndexWriterConfig#UNLIMITED_FIELD_LENGTH}. Pay attention to    * whether this setting fits your application.    *    * @param d    *          the index directory. The index is either created or appended    *          according<code>conf.getOpenMode()</code>.    * @param conf    *          the configuration settings according to which IndexWriter should    *          be initialized.    * @throws CorruptIndexException    *           if the index is corrupt    * @throws LockObtainFailedException    *           if another writer has this index open (<code>write.lock</code>    *           could not be obtained)    * @throws IOException    *           if the directory cannot be read/written to, or if it does not    *           exist and<code>conf.getOpenMode()</code> is    *<code>OpenMode.APPEND</code> or if there is any other low-level    *           IO error    */
...
expr_stmt|;
comment|// nocommit
comment|//return docWriter.bytesUsed() + bufferedDeletesStream.bytesUsed();
return|return
literal|0
return|;
block|}
...
condition|)
block|{
comment|// Reader was skipped because it was 100% deletions
continue|continue;
block|}
specifier|final
name|Bits
...
comment|//        long used = bufferedDeletesStream.bytesUsed() + docWriter.bytesUsed();
...
comment|//
comment|//          used = bufferedDeletesStream.bytesUsed() + docWriter.bytesUsed();
comment|//          if (used>= limit) {
