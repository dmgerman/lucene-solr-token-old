...
name|ReaderPool
block|{
DECL|class|SegmentCacheKey
specifier|final
class|class
name|SegmentCacheKey
block|{
DECL|field|si
specifier|public
specifier|final
...
condition|)
block|{
comment|// Also drop the READ reader if present: we don't
comment|// need its deletes since they've been carried
comment|// over to the merged segment
name|cacheKey
operator|=
operator|new
name|SegmentCacheKey
...
comment|// near real-time reader using this SR is still open
name|reader
operator|.
name|decRef
argument_list|()
...
expr_stmt|;
block|}
comment|// create CFS only if the source segment is not CFS, and MP agrees it
comment|// should be CFS.
name|boolean
name|createCFS
decl_stmt|;
synchronized|synchronized
...
name|info
argument_list|)
comment|// optimize case only for segments that don't share doc stores
operator|&&
name|versionComparator
operator|.
name|compare
...
block|}
block|}
comment|/** Copies the segment into the IndexWriter's directory, as a compound segment. */
DECL|method|copySegmentIntoCFS
specifier|private
name|void
name|copySegmentIntoCFS
parameter_list|(
...
block|}
block|}
DECL|method|getLiveDocsClone
specifier|private
specifier|synchronized
name|BitVector
name|getLiveDocsClone
parameter_list|(
name|SegmentInfo
name|info
parameter_list|,
...
condition|)
block|{
comment|// We clone the del docs because other
comment|// deletes may come in while we're merging.  We
comment|// need frozen deletes while merging, and then
comment|// we carry over any new deletions in
comment|// commitMergedDeletes.
name|liveDocs
operator|=
operator|(
name|BitVector
