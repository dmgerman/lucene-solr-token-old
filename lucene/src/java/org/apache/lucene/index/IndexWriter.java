...
return|;
block|}
comment|/**      * Release the segment reader (i.e. decRef it and close if there      * are no more references.      * @return true if this release altered the index (eg      * the SegmentReader had pending changes to del docs and      * was closed).  Caller must call checkpoint() if so.      * @param sr      * @throws IOException      */
DECL|method|release
specifier|public
specifier|synchronized
name|boolean
...
block|}
block|}
DECL|method|getCurrentFieldInfos
specifier|private
name|FieldInfos
name|getCurrentFieldInfos
parameter_list|()
...
block|{
comment|// Already a 4.0 index; just seed the FieldInfos
...
comment|// if this call is removed in the future we need to make
comment|// sure that info.clearFiles() is called here
name|info
operator|.
name|setDocStore
argument_list|(
...
block|}
block|}
comment|/**    * Flush all in-memory buffered udpates (adds and deletes)    * to the Directory.    * @param triggerMerge if true, we may merge segments (if    *  deletes or docs were flushed) if necessary    * @param flushDeletes whether pending deletes should also    */
DECL|method|flush
specifier|protected
specifier|final
name|void
...
argument_list|)
assert|;
comment|// We may be flushing because it was triggered by doc
comment|// count, del count, ram usage (in which case flush
comment|// pending is already set), or we may be flushing
comment|// due to external event eg getReader or commit is
comment|// called (in which case we now set it, and this will
comment|// pause all threads):
name|flushControl
operator|.
name|setFlushPendingNoWait
argument_list|(
literal|"explicit flush"
argument_list|)
expr_stmt|;
name|boolean
name|success
...
expr_stmt|;
comment|// nocommit
comment|//return docWriter.bytesUsed() + bufferedDeletes.bytesUsed();
return|return
literal|0
return|;
block|}
...
argument_list|)
expr_stmt|;
comment|// remove pending deletes of the segments
comment|// that were merged, moving them onto the segment just
comment|// before the merged segment
...
condition|)
block|{
comment|// Must checkpoint after releasing the
comment|// mergedReader since it may have written a new
...
return|;
block|}
comment|// decides when flushes happen
DECL|class|FlushControl
specifier|final
class|class
name|FlushControl
block|{
...
name|message
argument_list|(
literal|"now trigger flush reason="
operator|+
name|reason
argument_list|)
expr_stmt|;
...
block|}
block|}
DECL|method|setFlushPendingNoWait
specifier|public
specifier|synchronized
name|void
name|setFlushPendingNoWait
...
expr_stmt|;
block|}
DECL|method|getFlushPending
specifier|public
specifier|synchronized
name|boolean
name|getFlushPending
parameter_list|()
block|{
return|return
name|flushPending
...
block|}
block|}
comment|// skipWait is only used when a thread is BOTH adding
comment|// a doc and buffering a del term, and, the adding of
comment|// the doc already triggered a flush
if|if
condition|(
name|skipWait
condition|)
...
argument_list|(
literal|"add delete/doc"
argument_list|)
return|;
block|}
DECL|method|flushByRAMUsage
specifier|public
specifier|synchronized
name|boolean
name|flushByRAMUsage
...
block|{
comment|// nocommit
comment|//      final double ramBufferSizeMB = config.getRAMBufferSizeMB();
comment|//      if (ramBufferSizeMB != IndexWriterConfig.DISABLE_AUTO_FLUSH) {
comment|//        final long limit = (long) (ramBufferSizeMB*1024*1024);
comment|//        long used = bufferedDeletes.bytesUsed() + docWriter.bytesUsed();
comment|//        if (used>= limit) {
comment|//
comment|//          // DocumentsWriter may be able to free up some
comment|//          // RAM:
comment|//          // Lock order: FC -> DW
comment|//          docWriter.balanceRAM();
comment|//
comment|//          used = bufferedDeletes.bytesUsed() + docWriter.bytesUsed();
comment|//          if (used>= limit) {
comment|//            return setFlushPending("ram full: " + reason, false);
comment|//          }
comment|//        }
comment|//      }
return|return
