...
argument_list|()
expr_stmt|;
comment|// It's possible another flush (that did not close
comment|// the open do stores) snuck in after the flush we
comment|// just did, so we remove any tail segments
comment|// referencing the open doc store from the
comment|// SegmentInfos we are about to sync (the main
comment|// SegmentInfos will keep them):
name|toSync
operator|=
operator|(
name|SegmentInfos
