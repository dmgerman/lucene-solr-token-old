...
argument_list|()
expr_stmt|;
comment|// Count both "Publish Date" and "Author" dimensions, in
comment|// drill-down:
name|FacetSearchParams
name|fsp
init|=
...
name|assertEquals
argument_list|(
literal|"Publish Date: 2012=1 2010=1"
argument_list|,
name|toString
argument_list|(
name|r
...
name|assertEquals
argument_list|(
literal|"Author: Lisa=2 Frank=1 Susan=1 Bob=1"
argument_list|,
name|toString
argument_list|(
name|r
...
name|assertEquals
argument_list|(
literal|"Publish Date: 2010=2 2012=1"
argument_list|,
name|toString
argument_list|(
name|r
...
argument_list|()
expr_stmt|;
comment|// Two requests against the same dim:
name|FacetSearchParams
name|fsp
init|=
operator|new
...
argument_list|()
decl_stmt|;
comment|// TestRuleSetupAndRestoreClassEnv can sometimes
comment|// randomly pick the non-general Facet42DocValuesFormat:
name|DocValuesFormat
name|dvf
init|=
name|Codec
...
expr_stmt|;
block|}
comment|/*     aChance = .01;     bChance = 0.5;     cChance = 1.0;     */
name|double
name|sum
init|=
name|aChance
...
argument_list|)
expr_stmt|;
comment|//s = _TestUtil.randomSimpleString(random());
comment|// We cannot include this character else we hit
comment|// IllegalArgExc:
if|if
condition|(
name|s
operator|.
...
control|)
block|{
comment|// LUCENE-4915: sometimes don't request facet
comment|// counts on the dim(s) we drill down on
if|if
condition|(
name|random
argument_list|()
...
name|println
argument_list|(
literal|"  do facet request on dim="
operator|+
name|i
argument_list|)
expr_stmt|;
...
name|println
argument_list|(
literal|"  skip facet request on dim="
operator|+
name|i
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|/** Just gathers counts of values under the dim. */
