...
parameter_list|)
function_decl|;
comment|// Minimum distribution of relationships
comment|// Each shape has different characteristics, so we don't expect (for instance) shapes that
comment|// are likely to be long and thin to contain as many rectangles as those that
comment|// short and fat.
comment|/** Called once by {@link #testRelateWithRectangle()} to determine the max laps to try before failing. */
...
return|;
block|}
comment|/** The minimum number of times we need to see each predicate in {@code laps} iterations. */
...
name|fail
argument_list|(
literal|"Did not find enough contains/within/intersection/disjoint/bounds cases in a reasonable number"
operator|+
literal|" of attempts. CWIDbD: "
operator|+
name|i_C
operator|+
literal|"("
