...
expr_stmt|;
block|}
comment|//  Removed for now  - NumericRangeQuery does not currently implement getEnum
comment|//  private void testEnum(int lower, int upper) throws Exception {
comment|//    NumericRangeQuery<Integer> q = NumericRangeQuery.newIntRange("field4", 4, lower, upper, true, true);
comment|//    FilteredTermEnum termEnum = q.getEnum(searcher.getIndexReader());
comment|//    try {
comment|//      int count = 0;
comment|//      do {
comment|//        final Term t = termEnum.term();
comment|//        if (t != null) {
comment|//          final int val = NumericUtils.prefixCodedToInt(t.text());
comment|//          assertTrue("value not in bounds", val>= lower&& val<= upper);
comment|//          count++;
comment|//        } else break;
comment|//      } while (termEnum.next());
comment|//      assertFalse(termEnum.next());
comment|//      System.out.println("TermEnum on 'field4' for range [" + lower + "," + upper + "] contained " + count + " terms.");
comment|//    } finally {
comment|//      termEnum.close();
comment|//    }
comment|//  }
comment|//
comment|//  public void testEnum() throws Exception {
comment|//    int count=3000;
comment|//    int lower=(distance*3/2)+startOffset, upper=lower + count*distance + (distance/3);
comment|//    // test enum with values
comment|//    testEnum(lower, upper);
comment|//    // test empty enum
comment|//    testEnum(upper, lower);
comment|//    // test empty enum outside of bounds
comment|//    lower = distance*noDocs+startOffset;
comment|//    upper = 2 * lower;
comment|//    testEnum(lower, upper);
comment|//  }
block|}
end_class
