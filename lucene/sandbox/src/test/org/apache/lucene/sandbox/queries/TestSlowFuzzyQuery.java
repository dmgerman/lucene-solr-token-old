...
argument_list|)
expr_stmt|;
comment|// TODO: cannot really be supported given the legacy scoring
comment|// system which scores negative, if the distance> min term len,
comment|// so such matches were always impossible with lucene 3.x, etc
comment|//
comment|//q = new SlowFuzzyQuery(new Term("field", "t"), 3);
comment|//hits = searcher.search(q, 10).scoreDocs;
comment|//assertEquals(1, hits.length);
comment|//assertEquals("test", searcher.doc(hits[0].doc).get("field"));
comment|// q = new SlowFuzzyQuery(new Term("field", "a"), 4f, 0, 50);
comment|// hits = searcher.search(q, 10).scoreDocs;
comment|// assertEquals(1, hits.length);
comment|// assertEquals("test", searcher.doc(hits[0].doc).get("field"));
comment|// q = new SlowFuzzyQuery(new Term("field", "a"), 6f, 0, 50);
comment|// hits = searcher.search(q, 10).scoreDocs;
comment|// assertEquals(2, hits.length);
comment|// assertEquals("test", searcher.doc(hits[0].doc).get("field"));
comment|// assertEquals("foobar", searcher.doc(hits[1].doc).get("field"));
name|reader
operator|.
name|close
argument_list|()
