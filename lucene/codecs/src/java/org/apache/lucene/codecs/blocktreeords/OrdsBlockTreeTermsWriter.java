...
name|output
decl_stmt|;
comment|//long blockTermCount = output.endOrd - output.startOrd + 1;
name|Output
name|newOutput
init|=
name|FST_OUTPUTS
...
name|IOException
block|{
comment|/*       if (DEBUG) {         int[] tmp = new int[lastTerm.length];         System.arraycopy(prefixStarts, 0, tmp, 0, tmp.length);         System.out.println("BTTW: write term=" + brToString(text) + " prefixStarts=" + Arrays.toString(tmp) + " pending.size()=" + pending.size());       }       */
name|BlockTermState
name|state
init|=
name|postingsWriter
...
condition|)
block|{
comment|// if (DEBUG) System.out.println("BTTW.finish pending.size()=" + pending.size());
