...
argument_list|()
expr_stmt|;
comment|// BlenderType.EXPONENTIAL_RECIPROCAL is using 1/(pow(1+p, exponent)) * w where w is weight and p the position of the word
name|suggester
operator|=
operator|new
name|BlendedInfixSuggester
...
expr_stmt|;
block|}
comment|/**    * Handle trailing spaces that result in no prefix token LUCENE-6093    */
DECL|method|testNullPrefixToken
specifier|public
name|void
name|testNullPrefixToken
parameter_list|()
throws|throws
name|IOException
block|{
...
expr_stmt|;
block|}
DECL|method|testBlendedInfixSuggesterDedupsOnWeightTitleAndPayload
specifier|public
name|void
name|testBlendedInfixSuggesterDedupsOnWeightTitleAndPayload
parameter_list|()
throws|throws
name|Exception
block|{
comment|//exactly same inputs
name|Input
index|[]
name|inputDocuments
init|=
...
argument_list|)
expr_stmt|;
comment|// inputs differ on payload
name|inputDocuments
operator|=
operator|new
name|Input
...
argument_list|)
expr_stmt|;
comment|//exactly same input without payloads
name|inputDocuments
operator|=
operator|new
name|Input
...
argument_list|)
expr_stmt|;
comment|//Same input with first has payloads, second does not
name|inputDocuments
operator|=
operator|new
name|Input
...
argument_list|)
expr_stmt|;
comment|/**same input, first not having a payload, the second having payload      * we would expect 2 entries out but we are getting only 1 because      * the InputArrayIterator#hasPayloads() returns false because the first      * item has no payload, therefore, when ingested, none of the 2 input has payload and become 1      */
name|inputDocuments
operator|=
operator|new
name|Input
...
argument_list|)
expr_stmt|;
comment|//exactly same inputs but different weight
name|inputDocuments
operator|=
operator|new
name|Input
...
argument_list|)
expr_stmt|;
comment|//exactly same inputs but different text
name|inputDocuments
operator|=
operator|new
name|Input
...
expr_stmt|;
block|}
DECL|method|testSuggesterCountForAllLookups
specifier|public
name|void
name|testSuggesterCountForAllLookups
parameter_list|()
throws|throws
name|IOException
block|{
...
name|Input
argument_list|(
literal|"as you sow so shall you reap"
argument_list|,
literal|1
argument_list|)
block|,     }
