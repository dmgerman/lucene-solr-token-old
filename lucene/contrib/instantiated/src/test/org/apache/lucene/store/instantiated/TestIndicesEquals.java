begin_unit
begin_package
DECL|package|org.apache.lucene.store.instantiated
package|package
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|store
operator|.
name|instantiated
package|;
end_package
begin_comment
comment|/**  * Copyright 2006 The Apache Software Foundation  *  * Licensed under the Apache License, Version 2.0 (the "License");  * you may not use this file except in compliance with the License.  * You may obtain a copy of the License at  *  *     http://www.apache.org/licenses/LICENSE-2.0  *  * Unless required by applicable law or agreed to in writing, software  * distributed under the License is distributed on an "AS IS" BASIS,  * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.  * See the License for the specific language governing permissions and  * limitations under the License.  */
end_comment
begin_import
import|import
name|java
operator|.
name|io
operator|.
name|IOException
import|;
end_import
begin_import
import|import
name|java
operator|.
name|util
operator|.
name|ArrayList
import|;
end_import
begin_import
import|import
name|java
operator|.
name|util
operator|.
name|Arrays
import|;
end_import
begin_import
import|import
name|java
operator|.
name|util
operator|.
name|Comparator
import|;
end_import
begin_import
import|import
name|java
operator|.
name|util
operator|.
name|Iterator
import|;
end_import
begin_import
import|import
name|java
operator|.
name|util
operator|.
name|List
import|;
end_import
begin_import
import|import
name|java
operator|.
name|util
operator|.
name|Random
import|;
end_import
begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|analysis
operator|.
name|Token
import|;
end_import
begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|analysis
operator|.
name|TokenStream
import|;
end_import
begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|analysis
operator|.
name|MockAnalyzer
import|;
end_import
begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|analysis
operator|.
name|tokenattributes
operator|.
name|CharTermAttribute
import|;
end_import
begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|document
operator|.
name|Document
import|;
end_import
begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|document
operator|.
name|Field
import|;
end_import
begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|document
operator|.
name|FieldType
import|;
end_import
begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|document
operator|.
name|TextField
import|;
end_import
begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|index
operator|.
name|IndexReader
import|;
end_import
begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|index
operator|.
name|IndexWriter
import|;
end_import
begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|index
operator|.
name|MultiNorms
import|;
end_import
begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|index
operator|.
name|Payload
import|;
end_import
begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|index
operator|.
name|Term
import|;
end_import
begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|index
operator|.
name|DocsEnum
import|;
end_import
begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|index
operator|.
name|DocsAndPositionsEnum
import|;
end_import
begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|index
operator|.
name|MultiFields
import|;
end_import
begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|index
operator|.
name|TermsEnum
import|;
end_import
begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|index
operator|.
name|FieldsEnum
import|;
end_import
begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|index
operator|.
name|TermFreqVector
import|;
end_import
begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|index
operator|.
name|TermPositionVector
import|;
end_import
begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|store
operator|.
name|Directory
import|;
end_import
begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|util
operator|.
name|AttributeImpl
import|;
end_import
begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|util
operator|.
name|LuceneTestCase
import|;
end_import
begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|util
operator|.
name|BytesRef
import|;
end_import
begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|util
operator|.
name|Bits
import|;
end_import
begin_comment
comment|/**  * Asserts equality of content and behaviour of two index readers.  */
end_comment
begin_class
DECL|class|TestIndicesEquals
specifier|public
class|class
name|TestIndicesEquals
extends|extends
name|LuceneTestCase
block|{
comment|//  public void test2() throws Exception {
comment|//    FSDirectory fsdir = FSDirectory.open(new File("/tmp/fatcorpus"));
comment|//    IndexReader ir = IndexReader.open(fsdir, false);
comment|//    InstantiatedIndex ii = new InstantiatedIndex(ir);
comment|//    ir.close();
comment|//    testEquals(fsdir, ii);
comment|//  }
DECL|method|testLoadIndexReader
specifier|public
name|void
name|testLoadIndexReader
parameter_list|()
throws|throws
name|Exception
block|{
name|Directory
name|dir
init|=
name|newDirectory
argument_list|()
decl_stmt|;
comment|// create dir data
name|IndexWriter
name|indexWriter
init|=
operator|new
name|IndexWriter
argument_list|(
name|dir
argument_list|,
name|newIndexWriterConfig
argument_list|(
name|TEST_VERSION_CURRENT
argument_list|,
operator|new
name|MockAnalyzer
argument_list|(
name|random
argument_list|)
argument_list|)
operator|.
name|setMergePolicy
argument_list|(
name|newLogMergePolicy
argument_list|()
argument_list|)
argument_list|)
decl_stmt|;
for|for
control|(
name|int
name|i
init|=
literal|0
init|;
name|i
operator|<
literal|20
condition|;
name|i
operator|++
control|)
block|{
name|Document
name|document
init|=
operator|new
name|Document
argument_list|()
decl_stmt|;
name|assembleDocument
argument_list|(
name|document
argument_list|,
name|i
argument_list|)
expr_stmt|;
name|indexWriter
operator|.
name|addDocument
argument_list|(
name|document
argument_list|)
expr_stmt|;
block|}
name|indexWriter
operator|.
name|close
argument_list|()
expr_stmt|;
comment|// test load ii from index reader
name|IndexReader
name|ir
init|=
name|IndexReader
operator|.
name|open
argument_list|(
name|dir
argument_list|,
literal|false
argument_list|)
decl_stmt|;
name|InstantiatedIndex
name|ii
init|=
operator|new
name|InstantiatedIndex
argument_list|(
name|ir
argument_list|)
decl_stmt|;
name|ir
operator|.
name|close
argument_list|()
expr_stmt|;
name|testEqualBehaviour
argument_list|(
name|dir
argument_list|,
name|ii
argument_list|)
expr_stmt|;
name|dir
operator|.
name|close
argument_list|()
expr_stmt|;
block|}
DECL|method|testInstantiatedIndexWriter
specifier|public
name|void
name|testInstantiatedIndexWriter
parameter_list|()
throws|throws
name|Exception
block|{
name|Directory
name|dir
init|=
name|newDirectory
argument_list|()
decl_stmt|;
name|InstantiatedIndex
name|ii
init|=
operator|new
name|InstantiatedIndex
argument_list|()
decl_stmt|;
comment|// we need to pass the "same" random to both, so they surely index the same payload data.
name|long
name|seed
init|=
name|random
operator|.
name|nextLong
argument_list|()
decl_stmt|;
comment|// create dir data
name|IndexWriter
name|indexWriter
init|=
operator|new
name|IndexWriter
argument_list|(
name|dir
argument_list|,
name|newIndexWriterConfig
argument_list|(
name|TEST_VERSION_CURRENT
argument_list|,
operator|new
name|MockAnalyzer
argument_list|(
operator|new
name|Random
argument_list|(
name|seed
argument_list|)
argument_list|)
argument_list|)
operator|.
name|setMergePolicy
argument_list|(
name|newLogMergePolicy
argument_list|()
argument_list|)
argument_list|)
decl_stmt|;
if|if
condition|(
name|VERBOSE
condition|)
block|{
name|System
operator|.
name|out
operator|.
name|println
argument_list|(
literal|"TEST: make test index"
argument_list|)
expr_stmt|;
block|}
for|for
control|(
name|int
name|i
init|=
literal|0
init|;
name|i
operator|<
literal|500
condition|;
name|i
operator|++
control|)
block|{
name|Document
name|document
init|=
operator|new
name|Document
argument_list|()
decl_stmt|;
name|assembleDocument
argument_list|(
name|document
argument_list|,
name|i
argument_list|)
expr_stmt|;
name|indexWriter
operator|.
name|addDocument
argument_list|(
name|document
argument_list|)
expr_stmt|;
block|}
name|indexWriter
operator|.
name|close
argument_list|()
expr_stmt|;
comment|// test ii writer
name|InstantiatedIndexWriter
name|instantiatedIndexWriter
init|=
name|ii
operator|.
name|indexWriterFactory
argument_list|(
operator|new
name|MockAnalyzer
argument_list|(
operator|new
name|Random
argument_list|(
name|seed
argument_list|)
argument_list|)
argument_list|,
literal|true
argument_list|)
decl_stmt|;
for|for
control|(
name|int
name|i
init|=
literal|0
init|;
name|i
operator|<
literal|500
condition|;
name|i
operator|++
control|)
block|{
name|Document
name|document
init|=
operator|new
name|Document
argument_list|()
decl_stmt|;
name|assembleDocument
argument_list|(
name|document
argument_list|,
name|i
argument_list|)
expr_stmt|;
name|instantiatedIndexWriter
operator|.
name|addDocument
argument_list|(
name|document
argument_list|)
expr_stmt|;
block|}
name|instantiatedIndexWriter
operator|.
name|close
argument_list|()
expr_stmt|;
name|testEqualBehaviour
argument_list|(
name|dir
argument_list|,
name|ii
argument_list|)
expr_stmt|;
name|dir
operator|.
name|close
argument_list|()
expr_stmt|;
block|}
DECL|method|testTermDocsSomeMore
specifier|private
name|void
name|testTermDocsSomeMore
parameter_list|(
name|Directory
name|aprioriIndex
parameter_list|,
name|InstantiatedIndex
name|testIndex
parameter_list|)
throws|throws
name|Exception
block|{
name|IndexReader
name|aprioriReader
init|=
name|IndexReader
operator|.
name|open
argument_list|(
name|aprioriIndex
argument_list|,
literal|false
argument_list|)
decl_stmt|;
name|IndexReader
name|testReader
init|=
name|testIndex
operator|.
name|indexReaderFactory
argument_list|()
decl_stmt|;
comment|// test seek
name|Term
name|t
init|=
operator|new
name|Term
argument_list|(
literal|"c"
argument_list|,
literal|"danny"
argument_list|)
decl_stmt|;
name|TermsEnum
name|aprioriTermEnum
init|=
name|MultiFields
operator|.
name|getTerms
argument_list|(
name|aprioriReader
argument_list|,
name|t
operator|.
name|field
argument_list|()
argument_list|)
operator|.
name|iterator
argument_list|()
decl_stmt|;
name|aprioriTermEnum
operator|.
name|seekCeil
argument_list|(
operator|new
name|BytesRef
argument_list|(
name|t
operator|.
name|text
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
name|TermsEnum
name|testTermEnum
init|=
name|MultiFields
operator|.
name|getTerms
argument_list|(
name|testReader
argument_list|,
name|t
operator|.
name|field
argument_list|()
argument_list|)
operator|.
name|iterator
argument_list|()
decl_stmt|;
name|testTermEnum
operator|.
name|seekCeil
argument_list|(
operator|new
name|BytesRef
argument_list|(
name|t
operator|.
name|text
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|aprioriTermEnum
operator|.
name|term
argument_list|()
argument_list|,
name|testTermEnum
operator|.
name|term
argument_list|()
argument_list|)
expr_stmt|;
name|DocsEnum
name|aprioriTermDocs
init|=
name|aprioriTermEnum
operator|.
name|docs
argument_list|(
name|MultiFields
operator|.
name|getLiveDocs
argument_list|(
name|aprioriReader
argument_list|)
argument_list|,
literal|null
argument_list|)
decl_stmt|;
name|DocsEnum
name|testTermDocs
init|=
name|testTermEnum
operator|.
name|docs
argument_list|(
name|MultiFields
operator|.
name|getLiveDocs
argument_list|(
name|testReader
argument_list|)
argument_list|,
literal|null
argument_list|)
decl_stmt|;
name|assertEquals
argument_list|(
name|aprioriTermDocs
operator|.
name|nextDoc
argument_list|()
argument_list|,
name|testTermDocs
operator|.
name|nextDoc
argument_list|()
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|aprioriTermDocs
operator|.
name|freq
argument_list|()
argument_list|,
name|testTermDocs
operator|.
name|freq
argument_list|()
argument_list|)
expr_stmt|;
if|if
condition|(
name|aprioriTermDocs
operator|.
name|advance
argument_list|(
literal|4
argument_list|)
operator|!=
name|DocsEnum
operator|.
name|NO_MORE_DOCS
condition|)
block|{
name|assertTrue
argument_list|(
name|testTermDocs
operator|.
name|advance
argument_list|(
literal|4
argument_list|)
operator|!=
name|DocsEnum
operator|.
name|NO_MORE_DOCS
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|aprioriTermDocs
operator|.
name|freq
argument_list|()
argument_list|,
name|testTermDocs
operator|.
name|freq
argument_list|()
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|aprioriTermDocs
operator|.
name|docID
argument_list|()
argument_list|,
name|testTermDocs
operator|.
name|docID
argument_list|()
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|assertEquals
argument_list|(
name|DocsEnum
operator|.
name|NO_MORE_DOCS
argument_list|,
name|testTermDocs
operator|.
name|advance
argument_list|(
literal|4
argument_list|)
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|aprioriTermDocs
operator|.
name|nextDoc
argument_list|()
operator|!=
name|DocsEnum
operator|.
name|NO_MORE_DOCS
condition|)
block|{
name|assertTrue
argument_list|(
name|testTermDocs
operator|.
name|nextDoc
argument_list|()
operator|!=
name|DocsEnum
operator|.
name|NO_MORE_DOCS
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|aprioriTermDocs
operator|.
name|freq
argument_list|()
argument_list|,
name|testTermDocs
operator|.
name|freq
argument_list|()
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|aprioriTermDocs
operator|.
name|docID
argument_list|()
argument_list|,
name|testTermDocs
operator|.
name|docID
argument_list|()
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|assertEquals
argument_list|(
name|DocsEnum
operator|.
name|NO_MORE_DOCS
argument_list|,
name|testTermDocs
operator|.
name|nextDoc
argument_list|()
argument_list|)
expr_stmt|;
block|}
comment|// beyond this point all next and skipto will return false
if|if
condition|(
name|aprioriTermDocs
operator|.
name|advance
argument_list|(
literal|100
argument_list|)
operator|!=
name|DocsEnum
operator|.
name|NO_MORE_DOCS
condition|)
block|{
name|assertTrue
argument_list|(
name|testTermDocs
operator|.
name|advance
argument_list|(
literal|100
argument_list|)
operator|!=
name|DocsEnum
operator|.
name|NO_MORE_DOCS
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|aprioriTermDocs
operator|.
name|freq
argument_list|()
argument_list|,
name|testTermDocs
operator|.
name|freq
argument_list|()
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|aprioriTermDocs
operator|.
name|docID
argument_list|()
argument_list|,
name|testTermDocs
operator|.
name|docID
argument_list|()
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|assertEquals
argument_list|(
name|DocsEnum
operator|.
name|NO_MORE_DOCS
argument_list|,
name|testTermDocs
operator|.
name|advance
argument_list|(
literal|100
argument_list|)
argument_list|)
expr_stmt|;
block|}
comment|// start using the API the way one is supposed to use it
name|t
operator|=
operator|new
name|Term
argument_list|(
literal|""
argument_list|,
literal|""
argument_list|)
expr_stmt|;
name|FieldsEnum
name|apFieldsEnum
init|=
name|MultiFields
operator|.
name|getFields
argument_list|(
name|aprioriReader
argument_list|)
operator|.
name|iterator
argument_list|()
decl_stmt|;
name|String
name|apFirstField
init|=
name|apFieldsEnum
operator|.
name|next
argument_list|()
decl_stmt|;
name|FieldsEnum
name|testFieldsEnum
init|=
name|MultiFields
operator|.
name|getFields
argument_list|(
name|testReader
argument_list|)
operator|.
name|iterator
argument_list|()
decl_stmt|;
name|String
name|testFirstField
init|=
name|testFieldsEnum
operator|.
name|next
argument_list|()
decl_stmt|;
name|assertEquals
argument_list|(
name|apFirstField
argument_list|,
name|testFirstField
argument_list|)
expr_stmt|;
name|aprioriTermEnum
operator|=
name|apFieldsEnum
operator|.
name|terms
argument_list|()
expr_stmt|;
name|testTermEnum
operator|=
name|testFieldsEnum
operator|.
name|terms
argument_list|()
expr_stmt|;
name|assertEquals
argument_list|(
name|aprioriTermEnum
operator|.
name|next
argument_list|()
argument_list|,
name|testTermEnum
operator|.
name|next
argument_list|()
argument_list|)
expr_stmt|;
name|aprioriTermDocs
operator|=
name|aprioriTermEnum
operator|.
name|docs
argument_list|(
name|MultiFields
operator|.
name|getLiveDocs
argument_list|(
name|aprioriReader
argument_list|)
argument_list|,
name|aprioriTermDocs
argument_list|)
expr_stmt|;
name|testTermDocs
operator|=
name|testTermEnum
operator|.
name|docs
argument_list|(
name|MultiFields
operator|.
name|getLiveDocs
argument_list|(
name|testReader
argument_list|)
argument_list|,
name|testTermDocs
argument_list|)
expr_stmt|;
while|while
condition|(
name|aprioriTermDocs
operator|.
name|nextDoc
argument_list|()
operator|!=
name|DocsEnum
operator|.
name|NO_MORE_DOCS
condition|)
block|{
name|assertTrue
argument_list|(
name|testTermDocs
operator|.
name|nextDoc
argument_list|()
operator|!=
name|DocsEnum
operator|.
name|NO_MORE_DOCS
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|aprioriTermDocs
operator|.
name|freq
argument_list|()
argument_list|,
name|testTermDocs
operator|.
name|freq
argument_list|()
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|aprioriTermDocs
operator|.
name|docID
argument_list|()
argument_list|,
name|testTermDocs
operator|.
name|docID
argument_list|()
argument_list|)
expr_stmt|;
block|}
name|assertEquals
argument_list|(
name|DocsEnum
operator|.
name|NO_MORE_DOCS
argument_list|,
name|testTermDocs
operator|.
name|nextDoc
argument_list|()
argument_list|)
expr_stmt|;
comment|// clean up
name|aprioriReader
operator|.
name|close
argument_list|()
expr_stmt|;
name|testReader
operator|.
name|close
argument_list|()
expr_stmt|;
block|}
DECL|method|assembleDocument
specifier|private
name|void
name|assembleDocument
parameter_list|(
name|Document
name|document
parameter_list|,
name|int
name|i
parameter_list|)
block|{
name|FieldType
name|customType
init|=
operator|new
name|FieldType
argument_list|(
name|TextField
operator|.
name|TYPE_STORED
argument_list|)
decl_stmt|;
name|customType
operator|.
name|setStoreTermVectors
argument_list|(
literal|true
argument_list|)
expr_stmt|;
name|customType
operator|.
name|setStoreTermVectorOffsets
argument_list|(
literal|true
argument_list|)
expr_stmt|;
name|customType
operator|.
name|setStoreTermVectorPositions
argument_list|(
literal|true
argument_list|)
expr_stmt|;
comment|//document.add(new Field("a", i + " Do you really want to go and live in that house all winter?", Field.Store.YES, Field.Index.ANALYZED, Field.TermVector.WITH_POSITIONS_OFFSETS));
name|document
operator|.
name|add
argument_list|(
operator|new
name|Field
argument_list|(
literal|"a"
argument_list|,
name|i
operator|+
literal|" Do you really want to go and live in that house all winter?"
argument_list|,
name|customType
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|i
operator|>
literal|0
condition|)
block|{
comment|//document.add(new Field("b0", i + " All work and no play makes Jack a dull boy", Field.Store.YES, Field.Index.ANALYZED, Field.TermVector.WITH_POSITIONS_OFFSETS));
name|document
operator|.
name|add
argument_list|(
operator|new
name|Field
argument_list|(
literal|"b0"
argument_list|,
name|i
operator|+
literal|" All work and no play makes Jack a dull boy"
argument_list|,
name|customType
argument_list|)
argument_list|)
expr_stmt|;
comment|//document.add(new Field("b1", i + " All work and no play makes Jack a dull boy", Field.Store.YES, Field.Index.NOT_ANALYZED_NO_NORMS, Field.TermVector.NO));
name|FieldType
name|customType2
init|=
operator|new
name|FieldType
argument_list|(
name|TextField
operator|.
name|TYPE_STORED
argument_list|)
decl_stmt|;
name|customType2
operator|.
name|setTokenized
argument_list|(
literal|false
argument_list|)
expr_stmt|;
name|customType2
operator|.
name|setOmitNorms
argument_list|(
literal|true
argument_list|)
expr_stmt|;
name|document
operator|.
name|add
argument_list|(
operator|new
name|Field
argument_list|(
literal|"b1"
argument_list|,
name|i
operator|+
literal|" All work and no play makes Jack a dull boy"
argument_list|,
name|customType2
argument_list|)
argument_list|)
expr_stmt|;
comment|//document.add(new Field("b2", i + " All work and no play makes Jack a dull boy", Field.Store.NO, Field.Index.NOT_ANALYZED, Field.TermVector.NO));
name|FieldType
name|customType3
init|=
operator|new
name|FieldType
argument_list|(
name|TextField
operator|.
name|TYPE_UNSTORED
argument_list|)
decl_stmt|;
name|customType3
operator|.
name|setTokenized
argument_list|(
literal|false
argument_list|)
expr_stmt|;
name|document
operator|.
name|add
argument_list|(
operator|new
name|Field
argument_list|(
literal|"b1"
argument_list|,
name|i
operator|+
literal|" All work and no play makes Jack a dull boy"
argument_list|,
name|customType3
argument_list|)
argument_list|)
expr_stmt|;
comment|//document.add(new Field("b3", i + " All work and no play makes Jack a dull boy", Field.Store.YES, Field.Index.NO, Field.TermVector.NO));
name|FieldType
name|customType4
init|=
operator|new
name|FieldType
argument_list|(
name|TextField
operator|.
name|TYPE_STORED
argument_list|)
decl_stmt|;
name|customType4
operator|.
name|setIndexed
argument_list|(
literal|false
argument_list|)
expr_stmt|;
name|customType4
operator|.
name|setTokenized
argument_list|(
literal|false
argument_list|)
expr_stmt|;
name|document
operator|.
name|add
argument_list|(
operator|new
name|Field
argument_list|(
literal|"b1"
argument_list|,
name|i
operator|+
literal|" All work and no play makes Jack a dull boy"
argument_list|,
name|customType4
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|i
operator|>
literal|1
condition|)
block|{
comment|//document.add(new Field("c", i + " Redrum redrum", Field.Store.YES, Field.Index.ANALYZED, Field.TermVector.WITH_POSITIONS_OFFSETS));
name|document
operator|.
name|add
argument_list|(
operator|new
name|Field
argument_list|(
literal|"c"
argument_list|,
name|i
operator|+
literal|" Redrum redrum"
argument_list|,
name|customType
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|i
operator|>
literal|2
condition|)
block|{
comment|//document.add(new Field("d", i + " Hello Danny, come and play with us... forever and ever. and ever.", Field.Store.YES, Field.Index.ANALYZED, Field.TermVector.WITH_POSITIONS_OFFSETS));
name|document
operator|.
name|add
argument_list|(
operator|new
name|Field
argument_list|(
literal|"d"
argument_list|,
name|i
operator|+
literal|" Hello Danny, come and play with us... forever and ever. and ever."
argument_list|,
name|customType
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|i
operator|>
literal|3
condition|)
block|{
comment|//Field f = new Field("e", i + " Heres Johnny!", Field.Store.YES, Field.Index.ANALYZED, Field.TermVector.WITH_POSITIONS_OFFSETS);
comment|//f.setOmitNorms(true);
name|FieldType
name|customType5
init|=
operator|new
name|FieldType
argument_list|(
name|TextField
operator|.
name|TYPE_UNSTORED
argument_list|)
decl_stmt|;
name|customType5
operator|.
name|setOmitNorms
argument_list|(
literal|true
argument_list|)
expr_stmt|;
name|Field
name|f
init|=
operator|new
name|Field
argument_list|(
literal|"e"
argument_list|,
name|i
operator|+
literal|" Heres Johnny!"
argument_list|,
name|customType5
argument_list|)
decl_stmt|;
name|document
operator|.
name|add
argument_list|(
name|f
argument_list|)
expr_stmt|;
if|if
condition|(
name|i
operator|>
literal|4
condition|)
block|{
specifier|final
name|List
argument_list|<
name|Token
argument_list|>
name|tokens
init|=
operator|new
name|ArrayList
argument_list|<
name|Token
argument_list|>
argument_list|(
literal|2
argument_list|)
decl_stmt|;
name|Token
name|t
init|=
name|createToken
argument_list|(
literal|"the"
argument_list|,
literal|0
argument_list|,
literal|2
argument_list|,
literal|"text"
argument_list|)
decl_stmt|;
name|t
operator|.
name|setPayload
argument_list|(
operator|new
name|Payload
argument_list|(
operator|new
name|byte
index|[]
block|{
literal|1
block|,
literal|2
block|,
literal|3
block|}
argument_list|)
argument_list|)
expr_stmt|;
name|tokens
operator|.
name|add
argument_list|(
name|t
argument_list|)
expr_stmt|;
name|t
operator|=
name|createToken
argument_list|(
literal|"end"
argument_list|,
literal|3
argument_list|,
literal|5
argument_list|,
literal|"text"
argument_list|)
expr_stmt|;
name|t
operator|.
name|setPayload
argument_list|(
operator|new
name|Payload
argument_list|(
operator|new
name|byte
index|[]
block|{
literal|2
block|}
argument_list|)
argument_list|)
expr_stmt|;
name|tokens
operator|.
name|add
argument_list|(
name|t
argument_list|)
expr_stmt|;
name|tokens
operator|.
name|add
argument_list|(
name|createToken
argument_list|(
literal|"fin"
argument_list|,
literal|7
argument_list|,
literal|9
argument_list|)
argument_list|)
expr_stmt|;
name|TokenStream
name|ts
init|=
operator|new
name|TokenStream
argument_list|(
name|Token
operator|.
name|TOKEN_ATTRIBUTE_FACTORY
argument_list|)
block|{
specifier|final
name|AttributeImpl
name|reusableToken
init|=
operator|(
name|AttributeImpl
operator|)
name|addAttribute
argument_list|(
name|CharTermAttribute
operator|.
name|class
argument_list|)
decl_stmt|;
name|Iterator
argument_list|<
name|Token
argument_list|>
name|it
init|=
name|tokens
operator|.
name|iterator
argument_list|()
decl_stmt|;
annotation|@
name|Override
specifier|public
specifier|final
name|boolean
name|incrementToken
parameter_list|()
throws|throws
name|IOException
block|{
if|if
condition|(
operator|!
name|it
operator|.
name|hasNext
argument_list|()
condition|)
block|{
return|return
literal|false
return|;
block|}
name|clearAttributes
argument_list|()
expr_stmt|;
name|it
operator|.
name|next
argument_list|()
operator|.
name|copyTo
argument_list|(
name|reusableToken
argument_list|)
expr_stmt|;
return|return
literal|true
return|;
block|}
annotation|@
name|Override
specifier|public
name|void
name|reset
parameter_list|()
throws|throws
name|IOException
block|{
name|it
operator|=
name|tokens
operator|.
name|iterator
argument_list|()
expr_stmt|;
block|}
block|}
decl_stmt|;
comment|//document.add(new Field("f", ts));
name|document
operator|.
name|add
argument_list|(
operator|new
name|TextField
argument_list|(
literal|"f"
argument_list|,
name|ts
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|}
block|}
block|}
block|}
block|}
comment|/**    * Asserts that the content of two index readers equal each other.    *    * @param aprioriIndex the index that is known to be correct    * @param testIndex    the index that is supposed to equals the apriori index.    * @throws Exception    */
DECL|method|testEqualBehaviour
specifier|protected
name|void
name|testEqualBehaviour
parameter_list|(
name|Directory
name|aprioriIndex
parameter_list|,
name|InstantiatedIndex
name|testIndex
parameter_list|)
throws|throws
name|Exception
block|{
name|testEquals
argument_list|(
name|aprioriIndex
argument_list|,
name|testIndex
argument_list|)
expr_stmt|;
comment|// delete a few documents
name|IndexReader
name|air
init|=
name|IndexReader
operator|.
name|open
argument_list|(
name|aprioriIndex
argument_list|,
literal|false
argument_list|)
decl_stmt|;
name|InstantiatedIndexReader
name|tir
init|=
name|testIndex
operator|.
name|indexReaderFactory
argument_list|()
decl_stmt|;
name|assertEquals
argument_list|(
name|air
operator|.
name|isCurrent
argument_list|()
argument_list|,
name|tir
operator|.
name|isCurrent
argument_list|()
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|air
operator|.
name|hasDeletions
argument_list|()
argument_list|,
name|tir
operator|.
name|hasDeletions
argument_list|()
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|air
operator|.
name|maxDoc
argument_list|()
argument_list|,
name|tir
operator|.
name|maxDoc
argument_list|()
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|air
operator|.
name|numDocs
argument_list|()
argument_list|,
name|tir
operator|.
name|numDocs
argument_list|()
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|air
operator|.
name|numDeletedDocs
argument_list|()
argument_list|,
name|tir
operator|.
name|numDeletedDocs
argument_list|()
argument_list|)
expr_stmt|;
name|air
operator|.
name|deleteDocument
argument_list|(
literal|3
argument_list|)
expr_stmt|;
name|tir
operator|.
name|deleteDocument
argument_list|(
literal|3
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|air
operator|.
name|isCurrent
argument_list|()
argument_list|,
name|tir
operator|.
name|isCurrent
argument_list|()
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|air
operator|.
name|hasDeletions
argument_list|()
argument_list|,
name|tir
operator|.
name|hasDeletions
argument_list|()
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|air
operator|.
name|maxDoc
argument_list|()
argument_list|,
name|tir
operator|.
name|maxDoc
argument_list|()
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|air
operator|.
name|numDocs
argument_list|()
argument_list|,
name|tir
operator|.
name|numDocs
argument_list|()
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|air
operator|.
name|numDeletedDocs
argument_list|()
argument_list|,
name|tir
operator|.
name|numDeletedDocs
argument_list|()
argument_list|)
expr_stmt|;
name|air
operator|.
name|deleteDocument
argument_list|(
literal|8
argument_list|)
expr_stmt|;
name|tir
operator|.
name|deleteDocument
argument_list|(
literal|8
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|air
operator|.
name|isCurrent
argument_list|()
argument_list|,
name|tir
operator|.
name|isCurrent
argument_list|()
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|air
operator|.
name|hasDeletions
argument_list|()
argument_list|,
name|tir
operator|.
name|hasDeletions
argument_list|()
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|air
operator|.
name|maxDoc
argument_list|()
argument_list|,
name|tir
operator|.
name|maxDoc
argument_list|()
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|air
operator|.
name|numDocs
argument_list|()
argument_list|,
name|tir
operator|.
name|numDocs
argument_list|()
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|air
operator|.
name|numDeletedDocs
argument_list|()
argument_list|,
name|tir
operator|.
name|numDeletedDocs
argument_list|()
argument_list|)
expr_stmt|;
comment|// this (in 3.0) commits the deletions
name|air
operator|.
name|close
argument_list|()
expr_stmt|;
name|tir
operator|.
name|close
argument_list|()
expr_stmt|;
name|air
operator|=
name|IndexReader
operator|.
name|open
argument_list|(
name|aprioriIndex
argument_list|,
literal|false
argument_list|)
expr_stmt|;
name|tir
operator|=
name|testIndex
operator|.
name|indexReaderFactory
argument_list|()
expr_stmt|;
name|assertEquals
argument_list|(
name|air
operator|.
name|isCurrent
argument_list|()
argument_list|,
name|tir
operator|.
name|isCurrent
argument_list|()
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|air
operator|.
name|hasDeletions
argument_list|()
argument_list|,
name|tir
operator|.
name|hasDeletions
argument_list|()
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|air
operator|.
name|maxDoc
argument_list|()
argument_list|,
name|tir
operator|.
name|maxDoc
argument_list|()
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|air
operator|.
name|numDocs
argument_list|()
argument_list|,
name|tir
operator|.
name|numDocs
argument_list|()
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|air
operator|.
name|numDeletedDocs
argument_list|()
argument_list|,
name|tir
operator|.
name|numDeletedDocs
argument_list|()
argument_list|)
expr_stmt|;
specifier|final
name|Bits
name|aLiveDocs
init|=
name|MultiFields
operator|.
name|getLiveDocs
argument_list|(
name|air
argument_list|)
decl_stmt|;
specifier|final
name|Bits
name|tLiveDocs
init|=
name|MultiFields
operator|.
name|getLiveDocs
argument_list|(
name|tir
argument_list|)
decl_stmt|;
name|assertTrue
argument_list|(
operator|(
name|aLiveDocs
operator|!=
literal|null
operator|&&
name|tLiveDocs
operator|!=
literal|null
operator|)
operator|||
operator|(
name|aLiveDocs
operator|==
literal|null
operator|&&
name|tLiveDocs
operator|==
literal|null
operator|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|aLiveDocs
operator|!=
literal|null
condition|)
block|{
for|for
control|(
name|int
name|d
init|=
literal|0
init|;
name|d
operator|<
name|air
operator|.
name|maxDoc
argument_list|()
condition|;
name|d
operator|++
control|)
block|{
name|assertEquals
argument_list|(
name|aLiveDocs
operator|.
name|get
argument_list|(
name|d
argument_list|)
argument_list|,
name|tLiveDocs
operator|.
name|get
argument_list|(
name|d
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|}
name|air
operator|.
name|close
argument_list|()
expr_stmt|;
name|tir
operator|.
name|close
argument_list|()
expr_stmt|;
comment|// make sure they still equal
name|testEquals
argument_list|(
name|aprioriIndex
argument_list|,
name|testIndex
argument_list|)
expr_stmt|;
block|}
DECL|method|testEquals
specifier|protected
name|void
name|testEquals
parameter_list|(
name|Directory
name|aprioriIndex
parameter_list|,
name|InstantiatedIndex
name|testIndex
parameter_list|)
throws|throws
name|Exception
block|{
if|if
condition|(
name|VERBOSE
condition|)
block|{
name|System
operator|.
name|out
operator|.
name|println
argument_list|(
literal|"TEST: testEquals"
argument_list|)
expr_stmt|;
block|}
name|testTermDocsSomeMore
argument_list|(
name|aprioriIndex
argument_list|,
name|testIndex
argument_list|)
expr_stmt|;
name|IndexReader
name|aprioriReader
init|=
name|IndexReader
operator|.
name|open
argument_list|(
name|aprioriIndex
argument_list|,
literal|false
argument_list|)
decl_stmt|;
name|IndexReader
name|testReader
init|=
name|testIndex
operator|.
name|indexReaderFactory
argument_list|()
decl_stmt|;
name|assertEquals
argument_list|(
name|aprioriReader
operator|.
name|numDocs
argument_list|()
argument_list|,
name|testReader
operator|.
name|numDocs
argument_list|()
argument_list|)
expr_stmt|;
comment|// assert field options
name|assertEquals
argument_list|(
name|aprioriReader
operator|.
name|getFieldNames
argument_list|(
name|IndexReader
operator|.
name|FieldOption
operator|.
name|INDEXED
argument_list|)
argument_list|,
name|testReader
operator|.
name|getFieldNames
argument_list|(
name|IndexReader
operator|.
name|FieldOption
operator|.
name|INDEXED
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|aprioriReader
operator|.
name|getFieldNames
argument_list|(
name|IndexReader
operator|.
name|FieldOption
operator|.
name|INDEXED_NO_TERMVECTOR
argument_list|)
argument_list|,
name|testReader
operator|.
name|getFieldNames
argument_list|(
name|IndexReader
operator|.
name|FieldOption
operator|.
name|INDEXED_NO_TERMVECTOR
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|aprioriReader
operator|.
name|getFieldNames
argument_list|(
name|IndexReader
operator|.
name|FieldOption
operator|.
name|INDEXED_WITH_TERMVECTOR
argument_list|)
argument_list|,
name|testReader
operator|.
name|getFieldNames
argument_list|(
name|IndexReader
operator|.
name|FieldOption
operator|.
name|INDEXED_WITH_TERMVECTOR
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|aprioriReader
operator|.
name|getFieldNames
argument_list|(
name|IndexReader
operator|.
name|FieldOption
operator|.
name|STORES_PAYLOADS
argument_list|)
argument_list|,
name|testReader
operator|.
name|getFieldNames
argument_list|(
name|IndexReader
operator|.
name|FieldOption
operator|.
name|STORES_PAYLOADS
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|aprioriReader
operator|.
name|getFieldNames
argument_list|(
name|IndexReader
operator|.
name|FieldOption
operator|.
name|TERMVECTOR
argument_list|)
argument_list|,
name|testReader
operator|.
name|getFieldNames
argument_list|(
name|IndexReader
operator|.
name|FieldOption
operator|.
name|TERMVECTOR
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|aprioriReader
operator|.
name|getFieldNames
argument_list|(
name|IndexReader
operator|.
name|FieldOption
operator|.
name|TERMVECTOR_WITH_OFFSET
argument_list|)
argument_list|,
name|testReader
operator|.
name|getFieldNames
argument_list|(
name|IndexReader
operator|.
name|FieldOption
operator|.
name|TERMVECTOR_WITH_OFFSET
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|aprioriReader
operator|.
name|getFieldNames
argument_list|(
name|IndexReader
operator|.
name|FieldOption
operator|.
name|TERMVECTOR_WITH_POSITION
argument_list|)
argument_list|,
name|testReader
operator|.
name|getFieldNames
argument_list|(
name|IndexReader
operator|.
name|FieldOption
operator|.
name|TERMVECTOR_WITH_POSITION
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|aprioriReader
operator|.
name|getFieldNames
argument_list|(
name|IndexReader
operator|.
name|FieldOption
operator|.
name|TERMVECTOR_WITH_POSITION_OFFSET
argument_list|)
argument_list|,
name|testReader
operator|.
name|getFieldNames
argument_list|(
name|IndexReader
operator|.
name|FieldOption
operator|.
name|TERMVECTOR_WITH_POSITION_OFFSET
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|aprioriReader
operator|.
name|getFieldNames
argument_list|(
name|IndexReader
operator|.
name|FieldOption
operator|.
name|UNINDEXED
argument_list|)
argument_list|,
name|testReader
operator|.
name|getFieldNames
argument_list|(
name|IndexReader
operator|.
name|FieldOption
operator|.
name|UNINDEXED
argument_list|)
argument_list|)
expr_stmt|;
for|for
control|(
name|Object
name|field
range|:
name|aprioriReader
operator|.
name|getFieldNames
argument_list|(
name|IndexReader
operator|.
name|FieldOption
operator|.
name|ALL
argument_list|)
control|)
block|{
comment|// test norms as used by normal use
name|byte
index|[]
name|aprioriNorms
init|=
name|MultiNorms
operator|.
name|norms
argument_list|(
name|aprioriReader
argument_list|,
operator|(
name|String
operator|)
name|field
argument_list|)
decl_stmt|;
name|byte
index|[]
name|testNorms
init|=
name|MultiNorms
operator|.
name|norms
argument_list|(
name|testReader
argument_list|,
operator|(
name|String
operator|)
name|field
argument_list|)
decl_stmt|;
if|if
condition|(
name|aprioriNorms
operator|!=
literal|null
condition|)
block|{
name|assertEquals
argument_list|(
name|aprioriNorms
operator|.
name|length
argument_list|,
name|testNorms
operator|.
name|length
argument_list|)
expr_stmt|;
for|for
control|(
name|int
name|i
init|=
literal|0
init|;
name|i
operator|<
name|aprioriNorms
operator|.
name|length
condition|;
name|i
operator|++
control|)
block|{
name|assertEquals
argument_list|(
literal|"norms does not equals for field "
operator|+
name|field
operator|+
literal|" in document "
operator|+
name|i
argument_list|,
name|aprioriNorms
index|[
name|i
index|]
argument_list|,
name|testNorms
index|[
name|i
index|]
argument_list|)
expr_stmt|;
block|}
block|}
block|}
specifier|final
name|Bits
name|apLiveDocs
init|=
name|MultiFields
operator|.
name|getLiveDocs
argument_list|(
name|aprioriReader
argument_list|)
decl_stmt|;
specifier|final
name|Bits
name|testLiveDocs
init|=
name|MultiFields
operator|.
name|getLiveDocs
argument_list|(
name|testReader
argument_list|)
decl_stmt|;
name|assertTrue
argument_list|(
operator|(
name|apLiveDocs
operator|!=
literal|null
operator|&&
name|testLiveDocs
operator|!=
literal|null
operator|)
operator|||
operator|(
name|apLiveDocs
operator|==
literal|null
operator|&&
name|testLiveDocs
operator|==
literal|null
operator|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|apLiveDocs
operator|!=
literal|null
condition|)
block|{
for|for
control|(
name|int
name|docIndex
init|=
literal|0
init|;
name|docIndex
operator|<
name|aprioriReader
operator|.
name|numDocs
argument_list|()
condition|;
name|docIndex
operator|++
control|)
block|{
name|assertEquals
argument_list|(
name|apLiveDocs
operator|.
name|get
argument_list|(
name|docIndex
argument_list|)
argument_list|,
name|testLiveDocs
operator|.
name|get
argument_list|(
name|docIndex
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|}
comment|// compare term enumeration stepping
name|FieldsEnum
name|aprioriFieldsEnum
init|=
name|MultiFields
operator|.
name|getFields
argument_list|(
name|aprioriReader
argument_list|)
operator|.
name|iterator
argument_list|()
decl_stmt|;
name|FieldsEnum
name|testFieldsEnum
init|=
name|MultiFields
operator|.
name|getFields
argument_list|(
name|testReader
argument_list|)
operator|.
name|iterator
argument_list|()
decl_stmt|;
name|String
name|aprioriField
decl_stmt|;
while|while
condition|(
operator|(
name|aprioriField
operator|=
name|aprioriFieldsEnum
operator|.
name|next
argument_list|()
operator|)
operator|!=
literal|null
condition|)
block|{
name|String
name|testField
init|=
name|testFieldsEnum
operator|.
name|next
argument_list|()
decl_stmt|;
if|if
condition|(
name|VERBOSE
condition|)
block|{
name|System
operator|.
name|out
operator|.
name|println
argument_list|(
literal|"TEST: verify field="
operator|+
name|testField
argument_list|)
expr_stmt|;
block|}
name|assertEquals
argument_list|(
name|aprioriField
argument_list|,
name|testField
argument_list|)
expr_stmt|;
name|TermsEnum
name|aprioriTermEnum
init|=
name|aprioriFieldsEnum
operator|.
name|terms
argument_list|()
decl_stmt|;
name|TermsEnum
name|testTermEnum
init|=
name|testFieldsEnum
operator|.
name|terms
argument_list|()
decl_stmt|;
name|BytesRef
name|aprioriText
decl_stmt|;
while|while
condition|(
operator|(
name|aprioriText
operator|=
name|aprioriTermEnum
operator|.
name|next
argument_list|()
operator|)
operator|!=
literal|null
condition|)
block|{
name|assertEquals
argument_list|(
name|aprioriText
argument_list|,
name|testTermEnum
operator|.
name|next
argument_list|()
argument_list|)
expr_stmt|;
if|if
condition|(
name|VERBOSE
condition|)
block|{
name|System
operator|.
name|out
operator|.
name|println
argument_list|(
literal|"TEST:   verify term="
operator|+
name|aprioriText
operator|.
name|utf8ToString
argument_list|()
argument_list|)
expr_stmt|;
block|}
name|assertTrue
argument_list|(
name|aprioriTermEnum
operator|.
name|docFreq
argument_list|()
operator|==
name|testTermEnum
operator|.
name|docFreq
argument_list|()
argument_list|)
expr_stmt|;
specifier|final
name|long
name|totalTermFreq
init|=
name|aprioriTermEnum
operator|.
name|totalTermFreq
argument_list|()
decl_stmt|;
if|if
condition|(
name|totalTermFreq
operator|!=
operator|-
literal|1
condition|)
block|{
name|assertEquals
argument_list|(
name|totalTermFreq
argument_list|,
name|testTermEnum
operator|.
name|totalTermFreq
argument_list|()
argument_list|)
expr_stmt|;
block|}
comment|// compare termDocs seeking
name|DocsEnum
name|aprioriTermDocs
init|=
name|aprioriTermEnum
operator|.
name|docs
argument_list|(
name|MultiFields
operator|.
name|getLiveDocs
argument_list|(
name|aprioriReader
argument_list|)
argument_list|,
literal|null
argument_list|)
decl_stmt|;
name|DocsEnum
name|testTermDocs
init|=
name|testTermEnum
operator|.
name|docs
argument_list|(
name|MultiFields
operator|.
name|getLiveDocs
argument_list|(
name|testReader
argument_list|)
argument_list|,
literal|null
argument_list|)
decl_stmt|;
while|while
condition|(
name|aprioriTermDocs
operator|.
name|nextDoc
argument_list|()
operator|!=
name|DocsEnum
operator|.
name|NO_MORE_DOCS
condition|)
block|{
name|assertTrue
argument_list|(
name|testTermDocs
operator|.
name|advance
argument_list|(
name|aprioriTermDocs
operator|.
name|docID
argument_list|()
argument_list|)
operator|!=
name|DocsEnum
operator|.
name|NO_MORE_DOCS
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|aprioriTermDocs
operator|.
name|docID
argument_list|()
argument_list|,
name|testTermDocs
operator|.
name|docID
argument_list|()
argument_list|)
expr_stmt|;
block|}
comment|// compare documents per term
name|assertEquals
argument_list|(
name|aprioriReader
operator|.
name|docFreq
argument_list|(
name|aprioriField
argument_list|,
name|aprioriTermEnum
operator|.
name|term
argument_list|()
argument_list|)
argument_list|,
name|testReader
operator|.
name|docFreq
argument_list|(
name|aprioriField
argument_list|,
name|testTermEnum
operator|.
name|term
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
name|aprioriTermDocs
operator|=
name|aprioriTermEnum
operator|.
name|docs
argument_list|(
name|MultiFields
operator|.
name|getLiveDocs
argument_list|(
name|aprioriReader
argument_list|)
argument_list|,
name|aprioriTermDocs
argument_list|)
expr_stmt|;
name|testTermDocs
operator|=
name|testTermEnum
operator|.
name|docs
argument_list|(
name|MultiFields
operator|.
name|getLiveDocs
argument_list|(
name|testReader
argument_list|)
argument_list|,
name|testTermDocs
argument_list|)
expr_stmt|;
while|while
condition|(
literal|true
condition|)
block|{
if|if
condition|(
name|aprioriTermDocs
operator|.
name|nextDoc
argument_list|()
operator|==
name|DocsEnum
operator|.
name|NO_MORE_DOCS
condition|)
block|{
name|assertEquals
argument_list|(
name|DocsEnum
operator|.
name|NO_MORE_DOCS
argument_list|,
name|testTermDocs
operator|.
name|nextDoc
argument_list|()
argument_list|)
expr_stmt|;
break|break;
block|}
if|if
condition|(
name|VERBOSE
condition|)
block|{
name|System
operator|.
name|out
operator|.
name|println
argument_list|(
literal|"TEST:     verify doc="
operator|+
name|aprioriTermDocs
operator|.
name|docID
argument_list|()
argument_list|)
expr_stmt|;
block|}
name|assertTrue
argument_list|(
name|testTermDocs
operator|.
name|nextDoc
argument_list|()
operator|!=
name|DocsEnum
operator|.
name|NO_MORE_DOCS
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|aprioriTermDocs
operator|.
name|docID
argument_list|()
argument_list|,
name|testTermDocs
operator|.
name|docID
argument_list|()
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|aprioriTermDocs
operator|.
name|freq
argument_list|()
argument_list|,
name|testTermDocs
operator|.
name|freq
argument_list|()
argument_list|)
expr_stmt|;
block|}
comment|// compare term positions
name|DocsAndPositionsEnum
name|aprioriTermPositions
init|=
name|aprioriTermEnum
operator|.
name|docsAndPositions
argument_list|(
name|MultiFields
operator|.
name|getLiveDocs
argument_list|(
name|aprioriReader
argument_list|)
argument_list|,
literal|null
argument_list|)
decl_stmt|;
name|DocsAndPositionsEnum
name|testTermPositions
init|=
name|testTermEnum
operator|.
name|docsAndPositions
argument_list|(
name|MultiFields
operator|.
name|getLiveDocs
argument_list|(
name|testReader
argument_list|)
argument_list|,
literal|null
argument_list|)
decl_stmt|;
if|if
condition|(
name|VERBOSE
condition|)
block|{
name|System
operator|.
name|out
operator|.
name|println
argument_list|(
literal|"TEST: enum1="
operator|+
name|aprioriTermPositions
operator|+
literal|" enum2="
operator|+
name|testTermPositions
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|aprioriTermPositions
operator|!=
literal|null
condition|)
block|{
for|for
control|(
name|int
name|docIndex
init|=
literal|0
init|;
name|docIndex
operator|<
name|aprioriReader
operator|.
name|maxDoc
argument_list|()
condition|;
name|docIndex
operator|++
control|)
block|{
name|boolean
name|hasNext
init|=
name|aprioriTermPositions
operator|.
name|nextDoc
argument_list|()
operator|!=
name|DocsEnum
operator|.
name|NO_MORE_DOCS
decl_stmt|;
if|if
condition|(
name|hasNext
condition|)
block|{
name|assertTrue
argument_list|(
name|testTermPositions
operator|.
name|nextDoc
argument_list|()
operator|!=
name|DocsEnum
operator|.
name|NO_MORE_DOCS
argument_list|)
expr_stmt|;
if|if
condition|(
name|VERBOSE
condition|)
block|{
name|System
operator|.
name|out
operator|.
name|println
argument_list|(
literal|"TEST:     verify doc="
operator|+
name|aprioriTermPositions
operator|.
name|docID
argument_list|()
argument_list|)
expr_stmt|;
block|}
name|assertEquals
argument_list|(
name|aprioriTermPositions
operator|.
name|freq
argument_list|()
argument_list|,
name|testTermPositions
operator|.
name|freq
argument_list|()
argument_list|)
expr_stmt|;
for|for
control|(
name|int
name|termPositionIndex
init|=
literal|0
init|;
name|termPositionIndex
operator|<
name|aprioriTermPositions
operator|.
name|freq
argument_list|()
condition|;
name|termPositionIndex
operator|++
control|)
block|{
name|int
name|aprioriPos
init|=
name|aprioriTermPositions
operator|.
name|nextPosition
argument_list|()
decl_stmt|;
name|int
name|testPos
init|=
name|testTermPositions
operator|.
name|nextPosition
argument_list|()
decl_stmt|;
if|if
condition|(
name|VERBOSE
condition|)
block|{
name|System
operator|.
name|out
operator|.
name|println
argument_list|(
literal|"TEST:       verify pos="
operator|+
name|aprioriPos
argument_list|)
expr_stmt|;
block|}
name|assertEquals
argument_list|(
name|aprioriPos
argument_list|,
name|testPos
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|aprioriTermPositions
operator|.
name|hasPayload
argument_list|()
argument_list|,
name|testTermPositions
operator|.
name|hasPayload
argument_list|()
argument_list|)
expr_stmt|;
if|if
condition|(
name|aprioriTermPositions
operator|.
name|hasPayload
argument_list|()
condition|)
block|{
name|BytesRef
name|apPayload
init|=
name|aprioriTermPositions
operator|.
name|getPayload
argument_list|()
decl_stmt|;
name|BytesRef
name|testPayload
init|=
name|testTermPositions
operator|.
name|getPayload
argument_list|()
decl_stmt|;
name|assertEquals
argument_list|(
name|apPayload
argument_list|,
name|testPayload
argument_list|)
expr_stmt|;
block|}
block|}
block|}
block|}
block|}
block|}
name|assertNull
argument_list|(
name|testTermEnum
operator|.
name|next
argument_list|()
argument_list|)
expr_stmt|;
block|}
name|assertNull
argument_list|(
name|testFieldsEnum
operator|.
name|next
argument_list|()
argument_list|)
expr_stmt|;
comment|// compare term vectors and position vectors
for|for
control|(
name|int
name|documentNumber
init|=
literal|0
init|;
name|documentNumber
operator|<
name|aprioriReader
operator|.
name|numDocs
argument_list|()
condition|;
name|documentNumber
operator|++
control|)
block|{
if|if
condition|(
name|documentNumber
operator|>
literal|0
condition|)
block|{
name|assertNotNull
argument_list|(
name|aprioriReader
operator|.
name|getTermFreqVector
argument_list|(
name|documentNumber
argument_list|,
literal|"b0"
argument_list|)
argument_list|)
expr_stmt|;
name|assertNull
argument_list|(
name|aprioriReader
operator|.
name|getTermFreqVector
argument_list|(
name|documentNumber
argument_list|,
literal|"b1"
argument_list|)
argument_list|)
expr_stmt|;
name|assertNotNull
argument_list|(
name|testReader
operator|.
name|getTermFreqVector
argument_list|(
name|documentNumber
argument_list|,
literal|"b0"
argument_list|)
argument_list|)
expr_stmt|;
name|assertNull
argument_list|(
name|testReader
operator|.
name|getTermFreqVector
argument_list|(
name|documentNumber
argument_list|,
literal|"b1"
argument_list|)
argument_list|)
expr_stmt|;
block|}
name|TermFreqVector
index|[]
name|aprioriFreqVectors
init|=
name|aprioriReader
operator|.
name|getTermFreqVectors
argument_list|(
name|documentNumber
argument_list|)
decl_stmt|;
name|TermFreqVector
index|[]
name|testFreqVectors
init|=
name|testReader
operator|.
name|getTermFreqVectors
argument_list|(
name|documentNumber
argument_list|)
decl_stmt|;
if|if
condition|(
name|aprioriFreqVectors
operator|!=
literal|null
operator|&&
name|testFreqVectors
operator|!=
literal|null
condition|)
block|{
name|Arrays
operator|.
name|sort
argument_list|(
name|aprioriFreqVectors
argument_list|,
operator|new
name|Comparator
argument_list|<
name|TermFreqVector
argument_list|>
argument_list|()
block|{
specifier|public
name|int
name|compare
parameter_list|(
name|TermFreqVector
name|termFreqVector
parameter_list|,
name|TermFreqVector
name|termFreqVector1
parameter_list|)
block|{
return|return
name|termFreqVector
operator|.
name|getField
argument_list|()
operator|.
name|compareTo
argument_list|(
name|termFreqVector1
operator|.
name|getField
argument_list|()
argument_list|)
return|;
block|}
block|}
argument_list|)
expr_stmt|;
name|Arrays
operator|.
name|sort
argument_list|(
name|testFreqVectors
argument_list|,
operator|new
name|Comparator
argument_list|<
name|TermFreqVector
argument_list|>
argument_list|()
block|{
specifier|public
name|int
name|compare
parameter_list|(
name|TermFreqVector
name|termFreqVector
parameter_list|,
name|TermFreqVector
name|termFreqVector1
parameter_list|)
block|{
return|return
name|termFreqVector
operator|.
name|getField
argument_list|()
operator|.
name|compareTo
argument_list|(
name|termFreqVector1
operator|.
name|getField
argument_list|()
argument_list|)
return|;
block|}
block|}
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
literal|"document "
operator|+
name|documentNumber
operator|+
literal|" vectors does not match"
argument_list|,
name|aprioriFreqVectors
operator|.
name|length
argument_list|,
name|testFreqVectors
operator|.
name|length
argument_list|)
expr_stmt|;
for|for
control|(
name|int
name|freqVectorIndex
init|=
literal|0
init|;
name|freqVectorIndex
operator|<
name|aprioriFreqVectors
operator|.
name|length
condition|;
name|freqVectorIndex
operator|++
control|)
block|{
name|assertTrue
argument_list|(
name|Arrays
operator|.
name|equals
argument_list|(
name|aprioriFreqVectors
index|[
name|freqVectorIndex
index|]
operator|.
name|getTermFrequencies
argument_list|()
argument_list|,
name|testFreqVectors
index|[
name|freqVectorIndex
index|]
operator|.
name|getTermFrequencies
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
name|assertTrue
argument_list|(
name|Arrays
operator|.
name|equals
argument_list|(
name|aprioriFreqVectors
index|[
name|freqVectorIndex
index|]
operator|.
name|getTerms
argument_list|()
argument_list|,
name|testFreqVectors
index|[
name|freqVectorIndex
index|]
operator|.
name|getTerms
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|aprioriFreqVectors
index|[
name|freqVectorIndex
index|]
operator|instanceof
name|TermPositionVector
condition|)
block|{
name|TermPositionVector
name|aprioriTermPositionVector
init|=
operator|(
name|TermPositionVector
operator|)
name|aprioriFreqVectors
index|[
name|freqVectorIndex
index|]
decl_stmt|;
name|TermPositionVector
name|testTermPositionVector
init|=
operator|(
name|TermPositionVector
operator|)
name|testFreqVectors
index|[
name|freqVectorIndex
index|]
decl_stmt|;
for|for
control|(
name|int
name|positionVectorIndex
init|=
literal|0
init|;
name|positionVectorIndex
operator|<
name|aprioriFreqVectors
index|[
name|freqVectorIndex
index|]
operator|.
name|getTerms
argument_list|()
operator|.
name|length
condition|;
name|positionVectorIndex
operator|++
control|)
block|{
if|if
condition|(
name|aprioriTermPositionVector
operator|.
name|getOffsets
argument_list|(
name|positionVectorIndex
argument_list|)
operator|!=
literal|null
condition|)
block|{
name|assertTrue
argument_list|(
name|Arrays
operator|.
name|equals
argument_list|(
name|aprioriTermPositionVector
operator|.
name|getOffsets
argument_list|(
name|positionVectorIndex
argument_list|)
argument_list|,
name|testTermPositionVector
operator|.
name|getOffsets
argument_list|(
name|positionVectorIndex
argument_list|)
argument_list|)
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|aprioriTermPositionVector
operator|.
name|getTermPositions
argument_list|(
name|positionVectorIndex
argument_list|)
operator|!=
literal|null
condition|)
block|{
name|assertTrue
argument_list|(
name|Arrays
operator|.
name|equals
argument_list|(
name|aprioriTermPositionVector
operator|.
name|getTermPositions
argument_list|(
name|positionVectorIndex
argument_list|)
argument_list|,
name|testTermPositionVector
operator|.
name|getTermPositions
argument_list|(
name|positionVectorIndex
argument_list|)
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|}
block|}
block|}
block|}
block|}
name|aprioriReader
operator|.
name|close
argument_list|()
expr_stmt|;
name|testReader
operator|.
name|close
argument_list|()
expr_stmt|;
block|}
DECL|method|createToken
specifier|private
specifier|static
name|Token
name|createToken
parameter_list|(
name|String
name|term
parameter_list|,
name|int
name|start
parameter_list|,
name|int
name|offset
parameter_list|)
block|{
return|return
operator|new
name|Token
argument_list|(
name|term
argument_list|,
name|start
argument_list|,
name|offset
argument_list|)
return|;
block|}
DECL|method|createToken
specifier|private
specifier|static
name|Token
name|createToken
parameter_list|(
name|String
name|term
parameter_list|,
name|int
name|start
parameter_list|,
name|int
name|offset
parameter_list|,
name|String
name|type
parameter_list|)
block|{
return|return
operator|new
name|Token
argument_list|(
name|term
argument_list|,
name|start
argument_list|,
name|offset
argument_list|,
name|type
argument_list|)
return|;
block|}
block|}
end_class
end_unit
