...
name|IOException
block|{
comment|// Don't overallocate the results buffers. This also serves the purpose of allowing
comment|// the user of this class to request all matches using Integer.MAX_VALUE as the number
comment|// of results.
specifier|final
name|ArrayList
argument_list|<
...
return|;
block|}
comment|/**    * Checks if the list of {@link LookupResult}s already has a<code>key</code>. If so,    * reorders that {@link LookupResult} to the first position.    *     * @return Returns<code>true<code> if and only if<code>list</code> contained<code>key</code>.    */
...
condition|)
block|{
comment|// Key found. Unless already at i==0, remove it and push up front so that the ordering
