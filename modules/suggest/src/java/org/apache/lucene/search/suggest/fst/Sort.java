...
end_import
begin_comment
comment|// TODO: the buffer is currently byte[][] which with very small arrays will terribly overallocate
end_comment
begin_comment
comment|// memory (alignments) and make GC very happy.
end_comment
begin_comment
comment|//
end_comment
begin_comment
comment|// We could move it to a single byte[] + and use custom sorting, but we'd need to check if this
end_comment
begin_comment
comment|// yields any improvement first.
end_comment
begin_comment
comment|/**  * On-disk sorting of byte arrays. Each byte array (entry) is a composed of the following  * fields:  *<ul>  *<li>(two bytes) length of the following byte array,  *<li>exactly the above count of bytes for the sequence to be sorted.  *</ul>  *   * @see #sort(File, File)  */
end_comment
begin_class
DECL|class|Sort
specifier|public
...
literal|128
decl_stmt|;
comment|/**    * Minimum slot buffer expansion.    */
DECL|field|MIN_EXPECTED_GROWTH
specifier|private
specifier|final
specifier|static
name|int
name|MIN_EXPECTED_GROWTH
init|=
literal|1000
decl_stmt|;
...
block|}
block|}
comment|/**    * byte[] in unsigned byte order.    */
...
argument_list|()
decl_stmt|;
comment|// We will be reallocating from scratch.
name|Arrays
operator|.
name|fill
argument_list|(
