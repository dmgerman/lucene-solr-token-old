...
block|}
comment|/** expert internal use only    * Gets the replica from caches by the core name (assuming the slice is unknown) or null if replica is not found.    * If the slice is known, do not use this method.    * coreNodeName is the same as replicaName    */
...
return|;
block|}
comment|/**    * For internal use only    * @return Map&lt;collectionName, Map&lt;sliceName,Slice&gt;&gt;    */
...
return|;
block|}
comment|/**    * Create ClusterState from json string that is typically stored in zookeeper.    *     * Use {@link ClusterState#load(SolrZkClient, Set, ZkStateReader)} instead, unless you want to    * do something more when getting the data - such as get the stat, set watch, etc.    * @param version zk version of the clusterstate.json file (bytes)    * @param bytes clusterstate.json as a byte array    * @param liveNodes list of live nodes    * @param stateReader The ZkStateReader for this clusterstate    * @param znode the znode from which this data is read from    * @return the ClusterState    */
DECL|method|load
specifier|public
specifier|static
name|ClusterState
...
return|;
block|}
comment|/**    * @deprecated use {@link #load(Integer, byte[], Set, ZkStateReader, String)}    */
annotation|@
name|Deprecated
DECL|method|load
specifier|public
