...
return|;
block|}
comment|/**    * Set the ACL on a single node in ZooKeeper. This will replace all existing ACL on that node.    *    * @param path path to set ACL on e.g. /solr/conf/solrconfig.xml    * @param acls a list of {@link ACL}s to be applied    * @param retryOnConnLoss true if the command should be retried on connection loss    */
DECL|method|setACL
specifier|public
name|Stat
name|setACL
...
block|}
block|}
comment|/**    * Update all ACLs for a zk tree based on our configured {@link ZkACLProvider}.    * @param root the root node to recursively update    */
DECL|method|updateACLs
specifier|public
name|void
name|updateACLs
parameter_list|(
...
parameter_list|)
block|{
comment|// If a node was deleted, don't bother trying to set ACLs on it.
return|return;
block|}
block|}
argument_list|)
...
function_decl|;
block|}
comment|/**    * Recursively visit a zk tree rooted at path and apply the given visitor to each path. Exists as a separate method    * because some of the logic can get nuanced.    *    * @param path the path to start from    * @param visitor the operation to perform on each path    */
