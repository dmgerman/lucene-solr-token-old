...
function_decl|;
block|}
comment|/**    * Annotation for test classes to prevent TEMP_DIR cleanup.    */
annotation|@
name|Documented
annotation|@
name|Inherited
...
name|TYPE
argument_list|)
DECL|interface|SuppressTempDirCleanUp
specifier|public
annotation_defn|@interface
...
argument_list|)
decl_stmt|;
DECL|field|LEAVE_TEST_TMP_DIR_ANNOTATION
specifier|private
specifier|static
name|boolean
...
name|println
argument_list|(
literal|"NOTE: per solr.test.leavetmpdir, the test tmp directory will not be removed: "
operator|+
name|rootTmpDir
operator|.
name|getAbsolutePath
...
block|{
comment|// TODO: tmp files should already get cleaned up by the test framework, but
comment|// we still do it here as well, so that we clean up as much as we can, even
comment|// when a test is the SuppressTempDirCleanUp annotation
if|if
condition|(
name|rootTmpDir
operator|!=
...
block|{
comment|// TODO: do we want to fail here with this message? Is the test framework
comment|// fail that should happen better?
name|fail
argument_list|(
name|msg
argument_list|)
