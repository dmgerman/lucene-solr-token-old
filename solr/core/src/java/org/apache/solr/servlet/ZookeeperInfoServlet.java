...
operator|)
expr_stmt|;
comment|// this is hacky but tree view requests don't come in with the wt set
try|try
block|{
name|printer
operator|.
...
block|}
block|}
comment|// pull in external collections too
if|if
condition|(
name|ZkStateReader
operator|.
...
condition|)
block|{
comment|// initialize lazily as there may not be any external collections
name|collectionStates
operator|=
operator|new
name|TreeMap
...
argument_list|()
expr_stmt|;
comment|// add the internal collections
if|if
condition|(
name|dataStr
operator|!=
...
expr_stmt|;
block|}
comment|// now add in the external collections
name|Map
argument_list|<
name|String
argument_list|,
