...
condition|)
block|{
comment|// collection not found ... but it might be an alias?
name|String
name|resolved
init|=
literal|null
...
condition|)
block|{
comment|// ok, was an alias, but if the alias points to multiple collections, then we don't support that yet
if|if
condition|(
name|resolved
operator|.
...
argument_list|)
throw|;
comment|// ok, resolved to an alias
name|fromIndex
operator|=
name|resolved
expr_stmt|;
block|}
comment|// the fromIndex is a local replica for a single-sharded collection with replicas
comment|// across all nodes that have replicas for the collection we're joining with
name|fromIndex
operator|=
