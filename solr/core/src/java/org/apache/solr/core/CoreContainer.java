...
block|}
block|}
comment|// Either we have a config file or not. If it ends in .properties, assume new-style.
if|if
condition|(
name|fconf
operator|.
...
name|info
argument_list|(
literal|"no solr.xml or solr.properties file found - using default old-style solr.xml"
argument_list|)
expr_stmt|;
try|try
block|{
...
comment|//TODO 5.0: Remove this confusing junk, the properties file is so fast to read that there's no good reason
comment|//          to add this stuff. Furthermore, it would be good to persist comments when saving.....
try|try
block|{
if|if
condition|(
...
comment|//
comment|// Maybe the right thing to do is keep all the core descriptors NOT in the SolrCore, but keep all of the
comment|// core descriptors in SolrProperties exclusively.
comment|// TODO: 5.0 move coreDescriptors out of SolrCore and keep them only once in SolrProperties
comment|//
synchronized|synchronized
init|(
name|locker
...
argument_list|)
expr_stmt|;
comment|// Now add back in any implicit properties that aren't in already. These are all "attribs" in this meaning
name|Properties
name|implicit
init|=
name|dcore
