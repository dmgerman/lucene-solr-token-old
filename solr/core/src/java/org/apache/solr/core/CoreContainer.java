...
return|;
block|}
comment|/**    * Removes all references to the oldSchema from the schema cache; places an entry    * in the schema cache for the newSchema; and replaces references to the oldSchema    * with references to the newSchema in all registered cores.    *     * @param initiatingCore The initiating core; schema references doesn't need to be changed here    * @param oldSchema The schema to remove references to    * @param newSchema The schema to add references to    */
DECL|method|replaceSchema
specifier|public
name|void
name|replaceSchema
parameter_list|(
...
condition|)
block|{
comment|// isShareSchema() == true
comment|// Remove references to the oldSchema from the schema cache
for|for
control|(
name|Iterator
argument_list|<
...
block|}
block|}
comment|// Cache the new schema
specifier|final
name|String
name|newSchemaResourceName
init|=
name|IndexSchemaFactory
operator|.
name|getResourceNameToBeUsed
...
expr_stmt|;
block|}
comment|// Replace oldSchema references with newSchema references in all active cores
for|for
control|(
name|String
name|coreName
...
end_class
begin_comment
comment|// Introducing the two new maps (transientCores and dynamicDescriptors) introduced some locking complexities. Rather
end_comment
begin_comment
comment|// than try to keep them all straight in the code, use this class you need to access any of:
end_comment
begin_comment
comment|// cores
end_comment
begin_comment
comment|// transientCores
end_comment
begin_comment
comment|// dynamicDescriptors
end_comment
begin_comment
comment|//
end_comment
...
parameter_list|)
block|{
comment|// This is expensive in the maximal case, but I think necessary. It should keep a reference open to all of the
comment|// current cores while they are saved. Remember that especially the transient core can come and go.
comment|//
comment|// TODO: 5.0. remove the possibility of storing core descirptors in solr.xml?
comment|//
synchronized|synchronized
init|(
name|locker
...
argument_list|)
expr_stmt|;
comment|// add back all the cores that aren't loaded, either in cores or transient cores
for|for
control|(
name|Map
operator|.
...
argument_list|)
expr_stmt|;
comment|// Add in any non-standard bits of data
name|Set
argument_list|<
name|String
argument_list|>
...
expr_stmt|;
block|}
comment|// Default value here is same as old code.
name|addIfNotNull
argument_list|(
name|coreAttribs
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// Emulating the old code, just overwrite shard and roles if present in the cloud descriptor
if|if
condition|(
name|cd
operator|!=
...
block|{
comment|// Another awkward bit for back-compat for SOLR-4196
name|ConfigSolrXml
operator|.
name|addPersistCore
argument_list|(
