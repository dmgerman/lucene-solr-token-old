...
argument_list|()
decl_stmt|;
DECL|field|pendingDynamicCoreLoads
specifier|protected
specifier|final
name|Set
argument_list|<
...
return|;
block|}
comment|// Keep multiple threads from loading the same core at the same time.
try|try
block|{
name|boolean
name|isPending
...
condition|)
block|{
comment|// NOTE: If, for some reason, the load failed, we'll return null here and presumably the log will show
comment|// why. We'll fail all over again next time if the problem isn't corrected.
return|return
name|getCoreFromAnyList
argument_list|(
name|name
...
argument_list|)
expr_stmt|;
comment|// This is a "permanent", although deferred-load core
block|}
block|}
catch|catch
parameter_list|(
