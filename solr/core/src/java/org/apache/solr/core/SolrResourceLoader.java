...
argument_list|()
decl_stmt|;
comment|// A static map of AnalysisSPILoaders, keyed by ClassLoader used (because it can change during Solr lifetime) and expected base class:
DECL|field|expectedTypesSPILoaders
specifier|private
specifier|static
specifier|final
name|WeakIdentityMap
...
name|trace
argument_list|(
literal|"Trying to load class from analysis SPI"
argument_list|)
expr_stmt|;
comment|// retrieve the map of classLoader -> expectedType -> SPI from cache / regenerate cache
name|Map
argument_list|<
name|Class
argument_list|<
...
condition|)
block|{
comment|// it's a correct expected type for analysis! Let's go on!
try|try
block|{
return|return
name|clazz
