...
name|managedSchemaResourceName
expr_stmt|;
comment|// Check if the non-managed schema is also present
if|if
condition|(
operator|!
name|resourceName
...
if|if
condition|(
name|nonManagedSchemaExists
argument_list|()
condition|)
block|{
comment|// Warn if the non-managed schema is present
name|log
operator|.
name|warn
argument_list|(
...
name|info
argument_list|(
literal|"SolrConfig.isManagedSchema = true, but managed schema resource "
operator|+
name|managedSchemaResourceName
operator|+
...
DECL|method|nonManagedSchemaExists
specifier|private
name|boolean
name|nonManagedSchemaExists
parameter_list|()
...
block|{
comment|// try $configDir/$resource
name|File
name|fileUnderConfigDir
init|=
operator|new
name|File
argument_list|(
...
block|{
comment|// no success with $configDir/$resource - try $CWD/$resource
if|if
condition|(
name|file
operator|.
...
try|try
block|{
comment|// Create the managed schema znode
name|zkCmdExecutor
operator|.
name|ensureExists
argument_list|(
