...
decl_stmt|;
DECL|field|uniqueCoreId
specifier|private
specifier|final
name|String
name|uniqueCoreId
decl_stmt|;
comment|// used in equals impl to uniquely identify the core that we're dependent on
DECL|field|isRemoved
specifier|private
name|boolean
name|isRemoved
init|=
...
argument_list|()
expr_stmt|;
comment|// register a CloseHook for the core this reader is linked to, so that we can de-register the listener
name|solrCore
operator|.
name|addCloseHook
argument_list|(
...
name|info
argument_list|(
literal|"Removing ZkIndexSchemaReader OnReconnect listener as core "
operator|+
name|core
operator|.
name|getName
...
block|{
return|return;
comment|// the core for this reader has already been removed, don't process this event
block|}
...
block|{
return|return
literal|"ZkIndexSchemaReader: "
operator|+
name|managedSchemaPath
operator|+
literal|", uniqueCoreId: "
operator|+
name|uniqueCoreId
return|;
block|}
...
return|;
block|}
comment|// We need the uniqueCoreId which is core name + start time nanos to be the tie breaker
comment|// as there can be multiple ZkIndexSchemaReader instances active for the same core after
comment|// a reload (one is initializing and the other is being shutdown)
DECL|method|equals
specifier|public
name|boolean
name|equals
