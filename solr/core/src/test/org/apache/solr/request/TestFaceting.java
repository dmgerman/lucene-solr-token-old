...
argument_list|)
expr_stmt|;
comment|// Now, are all the UnInvertedFields still the same? Meaning they weren't re-fetched even when a bunch were
comment|// requested at the same time?
name|assertEquals
argument_list|(
literal|"UnInvertedField coming back from the seacher should not have changed! "
argument_list|,
name|ui0
argument_list|,
name|DocValues
...
block|}
block|}
comment|// assert same instance: either same object, or both wrapping same single-valued object
...
condition|)
block|{
comment|// actually a multi-valued field
if|if
condition|(
name|dv1
operator|instanceof
...
condition|)
block|{
comment|// if we produced more than one segment, ensure the core ordinal map is the same object
name|assertTrue
argument_list|(
name|dv2
operator|instanceof
...
block|{
comment|// otherwise, same atomic instance
name|assertSame
argument_list|(
name|dv1
argument_list|,
...
block|{
comment|// just wrapping a field that is actually single-valued
if|if
condition|(
name|singleton1
operator|instanceof
