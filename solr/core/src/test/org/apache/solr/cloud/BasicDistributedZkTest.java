...
literal|1
decl_stmt|;
comment|// Could use this for 0 hits too!
while|while
condition|(
name|docTotal
operator|!=
...
block|}
block|}
comment|// We could fail here if we broke out of the above because we exceeded the time allowed.
name|assertEquals
argument_list|(
name|failureMessage
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// This should be redundant, but it caught a test error after all.
for|for
control|(
name|SolrClient
name|client
...
block|}
block|}
comment|// Insure that counts are the same for all replicas in each shard
comment|// Return the total doc count for the query.
DECL|method|checkSlicesSameCounts
specifier|private
name|long
name|checkSlicesSameCounts
parameter_list|(
...
literal|0
decl_stmt|;
comment|// total number of documents found counting only one replica per slice.
for|for
control|(
name|Slice
name|slice
...
init|)
block|{
comment|// client.setConnectionTimeout(15000);
name|Create
name|createCmd
init|=
operator|new
