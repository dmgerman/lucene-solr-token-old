...
argument_list|()
expr_stmt|;
comment|// TODO: make this a generic collector list
name|List
argument_list|<
...
control|)
block|{
comment|// TODO: perhaps use some methods rather than instanceof
if|if
condition|(
name|groupCommand
operator|instanceof
...
condition|)
block|{
comment|// TODO: can callCollectors be zero length?
name|setCollector
operator|=
operator|new
...
block|}
block|}
comment|// TODO: optionally cache docs and feed them back through rather than re-searching
name|search
argument_list|(
name|query
argument_list|,
...
argument_list|)
decl_stmt|;
comment|// nl.add("matches", groupDocs.matches);  // redundant with doclist.numFound from the doc list
name|TopDocs
name|topDocs
init|=
name|groupDocs
...
expr_stmt|;
block|}
comment|/*** values from stage 1          DocSlice docs = new DocSlice(0, 1, new int[] {group.topDoc}, null, 1, 0);          nl.add("docs", docs);           Object[] vals = new Object[collector.comparators.length];          for (int i=0; i<vals.length; i++) {          vals[i] = collector.comparators[i].value(group.comparatorSlot);          }          nl.add("groupSortValue", vals);          groupResult.add(nl);          ***/
block|}
block|}
name|qr
operator|.
