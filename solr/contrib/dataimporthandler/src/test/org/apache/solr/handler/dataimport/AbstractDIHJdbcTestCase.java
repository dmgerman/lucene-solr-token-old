...
block|{
comment|//Start with "true" so the first test run will populate the data.
...
name|executeUpdate
argument_list|(
literal|"drop table people_sports"
argument_list|)
expr_stmt|;
block|}
catch|catch
...
parameter_list|)
block|{
comment|//ignore.  dbs complain when tables do no exist.
block|}
name|s
operator|.
name|executeUpdate
...
literal|null
decl_stmt|;
comment|//One second in the future ensures a change time after the last import (DIH uses second precision only)
name|Timestamp
name|theTime
init|=
operator|new
...
name|prepareStatement
argument_list|(
literal|"insert into people (id,name,country_code,last_modified) values (?,?,'NEW',?)"
argument_list|)
expr_stmt|;
for|for
control|(
...
name|append
argument_list|(
literal|"processor=''SqlEntityProcessor'' dataSource=''hsqldb'' "
argument_list|)
expr_stmt|;
name|sb
operator|.
...
name|append
argument_list|(
literal|"<entity name=''Sports'' dataSource=''hsqldb'' "
operator|+
operator|(
name|sportsTransformer
condition|?
