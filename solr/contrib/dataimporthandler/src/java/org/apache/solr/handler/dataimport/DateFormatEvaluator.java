...
name|ENGLISH
decl_stmt|;
comment|// we default to ENGLISH for dates for full Java 9 compatibility
if|if
condition|(
name|l
operator|.
...
argument_list|()
decl_stmt|;
comment|// DWS TODO: is this the right default for us?  Deserves explanation if so.
if|if
condition|(
name|l
operator|.
...
parameter_list|)
block|{
comment|// note: DMP does not use the locale but perhaps a subclass might use it, for e.g. parsing a date in a custom
comment|// string that doesn't necessarily have date math?
comment|//TODO refactor DateMathParser.parseMath a bit to have a static method for this logic.
if|if
condition|(
name|datemathfmt
operator|.
...
argument_list|)
expr_stmt|;
comment|// thus do *not* use SolrRequestInfo
return|return
name|parser
operator|.
name|parseMath
