...
argument_list|)
throw|;
comment|// Read the last_index_time out of the dataimport.properties if available
name|String
name|cname
init|=
name|getStringFromContext
...
name|info
argument_list|(
literal|"Using lastIndexTime "
operator|+
name|lastIndexTime
operator|+
literal|" for mail import"
argument_list|)
expr_stmt|;
name|this
operator|.
...
name|info
argument_list|(
literal|"Parsed fetchMailsSince="
operator|+
name|lastIndexTime
argument_list|)
expr_stmt|;
...
argument_list|(
literal|"includeOtherUserFolders"
argument_list|)
decl_stmt|;
name|includeOtherUserFolders
operator|=
operator|(
name|tmp
operator|!=
...
name|getEntityAttribute
argument_list|(
literal|"includeSharedFolders"
argument_list|)
expr_stmt|;
name|includeSharedFolders
operator|=
operator|(
name|tmp
operator|!=
...
operator|)
expr_stmt|;
name|setProcessAttachmentConfig
argument_list|()
expr_stmt|;
name|includeContent
operator|=
...
expr_stmt|;
block|}
DECL|method|setProcessAttachmentConfig
specifier|private
name|void
name|setProcessAttachmentConfig
parameter_list|()
...
name|error
argument_list|(
literal|"Failed to convert message ["
operator|+
name|mail
operator|.
name|toString
...
name|warn
argument_list|(
literal|"Multipart content is a not an instance of Multipart! Content is: "
operator|+
operator|(
name|content
operator|!=
...
operator|)
operator|+
literal|". Typically, this is due to the Java Activation JAR being loaded by the wrong classloader."
argument_list|)
expr_stmt|;
block|}
block|}
...
block|}
block|}
DECL|method|addEnvelopeToDocument
specifier|private
name|void
name|addEnvelopeToDocument
parameter_list|(
...
parameter_list|()
block|{
comment|// this is needed to load the activation mail stuff correctly
comment|// otherwise, the JavaMail multipart support doesn't get configured
comment|// correctly, which leads to a class cast exception when processing
comment|// multipart messages: IMAPInputStream cannot be cast to
comment|// javax.mail.Multipart
name|Thread
operator|.
name|currentThread
argument_list|()
...
name|info
argument_list|(
literal|"Consider using 'gimaps' protocol instead of '"
operator|+
name|protocol
operator|+
literal|"' for enabling GMail specific extensions for "
operator|+
name|host
argument_list|)
expr_stmt|;
...
name|ENGLISH
argument_list|,
literal|"Failed to connect to %s server %s as user %s due to: %s"
argument_list|,
name|protocol
argument_list|,
name|host
...
name|warn
argument_list|(
literal|"Failed to read folders due to: "
operator|+
name|e
argument_list|)
expr_stmt|;
comment|// throw new
comment|// DataImportHandlerException(DataImportHandlerException.SEVERE,
...
block|}
block|}
DECL|method|getOtherUserFolders
specifier|private
name|void
name|getOtherUserFolders
parameter_list|()
...
name|length
operator|+
literal|" user namespace folders"
argument_list|)
expr_stmt|;
for|for
control|(
...
name|warn
argument_list|(
literal|"Messaging exception retrieving user namespaces: "
operator|+
name|me
operator|.
name|getMessage
...
name|length
operator|+
literal|" shared namespace folders"
argument_list|)
expr_stmt|;
for|for
control|(
...
name|warn
argument_list|(
literal|"Messaging exception retrieving shared namespaces: "
operator|+
name|me
operator|.
name|getMessage
...
literal|false
expr_stmt|;
comment|// Searching can still take a while even though we're only pulling
comment|// envelopes; unless you're using gmail server-side filter, which is
comment|// fast
name|LOG
operator|.
name|info
argument_list|(
literal|"Searching folder "
operator|+
name|folder
operator|.
name|getName
...
comment|// server-side
comment|// search for messages occurring on or after the fetch date (at
comment|// midnight),
comment|// which reduces the number of envelopes we need to pull from the
comment|// server
comment|// to apply the precise DateTerm filter; GMail server-side search has
comment|// date
comment|// granularity only but the local filters are also applied
if|if
condition|(
name|folder
operator|instanceof
...
name|info
argument_list|(
literal|"Added server-side gmail filter: "
operator|+
name|afterCrit
argument_list|)
expr_stmt|;
...
name|info
argument_list|(
literal|"GMail server-side filter found "
operator|+
name|afterMessages
operator|.
name|length
...
argument_list|)
expr_stmt|;
comment|// now pass in the server-side filtered messages to the local filter
name|messagesInCurBatch
operator|=
name|folder
operator|.
...
name|info
argument_list|(
literal|"Building mail filter for messages in "
operator|+
name|folder
operator|.
name|getName
...
argument_list|()
operator|+
literal|" was received at ["
operator|+
name|msgDateStr
operator|+
literal|"], since filter is ["
operator|+
name|sinceDateStr
operator|+
literal|"]"
...
literal|true
decl_stmt|;
DECL|field|includeOtherUserFolders
specifier|private
name|boolean
name|includeOtherUserFolders
init|=
literal|false
decl_stmt|;
DECL|field|includeSharedFolders
specifier|private
name|boolean
name|includeSharedFolders
init|=
