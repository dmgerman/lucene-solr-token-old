...
end_import
begin_comment
comment|/**  * This tokenizer uses regex pattern matching to construct distinct tokens  * for the input stream.  It takes two arguments:  "pattern" and "group"  *   * "pattern" is the regular expression.    * "group" says which group to extract into tokens.  *   * group=-1 (the default) is equuivolent to "split".  In this case, the tokes will  * be equivolent to the output from:  *   * http://java.sun.com/j2se/1.4.2/docs/api/java/lang/String.html#split(java.lang.String)  *   * Using group>= 0 selects the matching group as the token.  For example, it you have:  *    *  pattern = \'([^\']+)\'  *  group = 0  *  input = aaa 'bbb' 'ccc'  *   * the output will be two tokens: 'bbb' and 'ccc'  (including the ' marks).  With the same input  * but using group=1, the output would be: bbb and ccc (no ' marks)  *   *   * @author ryan  * @since solr1.2  * @version $Id:$  */
end_comment
begin_class
