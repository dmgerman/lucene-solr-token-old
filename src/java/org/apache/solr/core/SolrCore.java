...
return|;
block|}
comment|// The registry of known cores
DECL|field|cores
specifier|private
specifier|static
name|Map
...
return|;
block|}
comment|/**    * Retrieves a core instance by name.    *@param name the core name    *@return the core instance or null if none exist with that name.    */
DECL|method|getSolrCore
specifier|public
specifier|static
name|SolrCore
...
block|}
block|}
comment|/**    * Returns an unmodifieable Map containing the registered cores    */
DECL|method|getSolrCores
specifier|public
name|Map
argument_list|<
name|String
...
return|;
block|}
comment|/** The array of known core names. */
DECL|method|getSolrCoreNames
specifier|public
name|String
index|[]
name|getSolrCoreNames
parameter_list|()
block|{
synchronized|synchronized
init|(
...
return|;
block|}
comment|/** The single-core mode compatibility constructor; the core is named 'null'. */
...
expr_stmt|;
block|}
comment|/** Ensures that a name does not contain a '/' or a '\' to avoid any potential    *  issues with file pathes.    *@param name the core name to check    *@return the name    *@throws SolrException if the name is not valid    */
DECL|method|checkName
specifier|private
specifier|static
name|String
name|checkName
...
return|;
block|}
comment|/**    * Creates a new core and register it in the list of cores.    * If a core with the same name already exists, it will be stopped and replaced by this one.    *@param name the unique name of the core (null is accepted)    *@param dataDir the index directory    *@param config a solr config instance    *@param schema a solr schema instance    */
DECL|method|SolrCore
specifier|public
name|SolrCore
parameter_list|(
...
name|config
expr_stmt|;
comment|// compatibility code with pre-solr215-patch in case some custom code relies on SolrConfig.config existence.
if|if
condition|(
name|this
operator|.
...
name|info
argument_list|(
literal|"CLOSING default SolrCore!"
argument_list|)
expr_stmt|;
else|else
name|log
