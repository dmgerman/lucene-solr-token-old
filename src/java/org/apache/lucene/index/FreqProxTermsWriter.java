...
name|omitTf
decl_stmt|;
comment|// If current field omits tf then it cannot store
comment|// payloads.  We silently drop the payloads in this case:
specifier|final
name|boolean
...
condition|)
block|{
comment|// omitTf == false so we do write positions& payload
assert|assert
name|proxOut
operator|!=
literal|null
...
block|{
comment|// omitTf==true: we store only the docs, without
comment|// term freq, positions, payloads
name|freqOut
operator|.
name|writeVInt
argument_list|(
