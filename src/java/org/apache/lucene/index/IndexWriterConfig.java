...
name|matchVersion
decl_stmt|;
comment|/**    * Creates a new config that with defaults that match the specified    * {@link Version}. {@link Version} is a placeholder for future changes. The    * default settings are relevant to 3.1 and before. In the future, if    * different settings will apply to different versions, they will be    * documented here.    */
...
block|}
block|}
comment|/**    * Sets the default {@link Analyzer} to be used when indexing documents. The    * default {@link WhitespaceAnalyzer} is set for convenience (e.g. for test    * purposes or when the analyzer used does not make a difference) and it's    * recommended to override the default setting if you care about the tokens    * that end up in your index.    *<p>    *<b>NOTE:</b> the analyzer cannot be null. If<code>null</code> is passed,    * the analyzer will be set to the default.    */
