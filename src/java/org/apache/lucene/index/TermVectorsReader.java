...
name|IOException
block|{
comment|// why don't we trap the exception and at least make sure that
comment|// all streams that we can close are closed?
if|if
condition|(
name|tvx
operator|!=
...
return|;
block|}
comment|/**    * Retrieve the term vector for the given document and field    * @param docNum The document number to retrieve the vector for    * @param field The field within the document to retrieve    * @return The TermFreqVector for the document and field or null    */
DECL|method|get
specifier|synchronized
name|TermFreqVector
name|get
...
comment|//We don't need to do this in other seeks because we already have the file pointer
...
expr_stmt|;
block|}
comment|// This field, although valid in the segment, was not found in this document
if|if
condition|(
name|found
operator|!=
...
return|;
block|}
comment|/** Return all term vectors stored for this document or null if the could not be read in. */
DECL|method|get
specifier|synchronized
name|TermFreqVector
index|[]
...
comment|// If no terms - return a constant empty termvector
if|if
condition|(
name|numTerms
operator|==
